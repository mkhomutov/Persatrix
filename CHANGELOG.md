...
- **RFC 0052 — the convene / synthesis forced turns are now actually deliverable to real agents (they were refused at the receiver gate, so a convened channel produced nothing).** Booting the offline demo surfaced that the orchestrator dispatches the §B convene opener and the §D synthesis-close turn under **synthetic sender ids** (`orchestrator:convene` / `orchestrator:synthesis`) that deliberately contain a reserved `:` so they can never collide with a real participant id — but the agent's inbound `validate_channel_message_event` applied the participant-id pattern to `sender_id` and **refused the directive at the transport gate** (`sender_id is not a valid participant id`) *before* the forced-turn admit ever ran. The result was a silent no-op: `ConveneChannel` returned `202`, the receiver `503`'d the delivery, and the channel produced no opener and no synthesis — exactly the silent-failure class the RFC's safety contract exists to catch. It escaped CI because the Go acceptance suite mocks the dispatcher (never exercising the real Python receiver gate) and the live `MT-AUTONOMOUS-*` runs were still pending. The receiver now carves the two reserved sentinels out of `sender_id` validation, **exactly as it already carves `@everyone` out of mention validation** — a bounded, enumerated set that loosens nothing else about the cleartext-port sender-spoofing trust boundary (PR #248), with a Go↔Python drift guard pinning the carve-out against `ConveneDispatchSenderID` / `SynthesisDispatchSenderID`. This unblocks the whole autonomous live path (convene opener, synthesis turn, and the PR 6 anti-collapse advance turn all ride these senders). ([RFC 0052](docs/rfcs/0052-autonomous-agent-channels.md))
