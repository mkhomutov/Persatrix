...
| **[Behaviour — per-member `threshold` superseded by the flip]** The structured rung ignores the RFC 0030 score gate | Under `bid`/`plan` the bid emits **no** `score` and the per-member salience `threshold` is **inert** — the `{ should_post }` verdict *is* the silence decision (RFC 0051 OQ 7); a numeric floor can neither articulate "I'd only be agreeing" nor rescue a low-scored turn that adds real substance. So a deployment that tuned per-member `threshold` floors gets *semantic* silence instead of score-gating the moment a channel becomes governed. To keep the numeric score gate and honour those thresholds, pin `reasoning.mode: off` (the kill switch above). |
| **[Capability-gated]** `reasoning.depth: deep` is rejected at `validate` | Native extended-thinking depth (Phase 4) is not yet deployed, so an unbacked value is rejected with a 400 rather than silently downgraded. |
| **[New opt-in — Phase 5 deployed]** `reasoning.revise: 0\|1\|2` (reflexion) | The reflexion loop now ships (default `revise: 0` = single-pass, unchanged behaviour). `validate` accepts `revise ≥ 1` only under `mode: plan` (the critic re-reads the draft against the plan) and caps it at 2; a `revise ≥ 1` on any other rung, or above 2, is rejected with a 400. |
| **[Version-skew — additive]** New `reasoning_mode` / `reasoning_revise` wire fields | `ChannelMessageEvent` gains `reasoning_mode` and `reasoning_revise` fields. A pre-v0.3.10 producer leaves them empty/0, which the agent maps to `off` / single-pass, so both are additive across a mixed-version deployment. **Upgrade the orchestrator together with (or before) the agents** to arm the flip and the reflexion opt-in. |

### 🚀 Features

- **RFC 0051 go-live (PR 6): the resolved `reasoning.mode` rung reaches the agent over the wire, and the governed-channel default flips `off → bid` in lockstep with the kill switch and telemetry.** The router stamps the channel's resolved rung onto every dispatch (`ChannelMessageEvent.reasoning_mode`); the salience seam reads it to pick the bid's verdict grammar (scalar gate / structured silence verdict / plan-threaded compose). Governed channels resolve to `bid` by default at load, boot, runtime-create, and runtime-edit; an explicit `off` is preserved as a kill switch across all of them. New deliberation telemetry — `deliberation.total` (rate), `deliberation.suppressed` (silence charted by `reason_code` + `mode`), `deliberation.duration` (latency histogram), and `deliberation.budget_starved` — makes the active default safe to run.
- **RFC 0051 reflexion (PR 8, Phase 5a): an opt-in critic→revise pass that sharpens a `mode: plan` post before it ships, default off.** Under `reasoning.revise: 1|2` a cheap `fast` **critic** re-reads the composed draft against the private plan and, only if it flags weakness, a `quality` **revise** rewrites it — bounded to the round count and **fail-soft** (a parse/critic failure or an exhausted lease keeps the last good draft; the post is never blocked). The critic adds no compose unless it flags weakness, so an `N`-round post costs up to `N+1` quality composes, hard-capped at 2 and metered against the same interaction lease. The discarded drafts and critic notes are walled exactly like the plan (never a message, never persisted). The resolved count rides the wire (`ChannelMessageEvent.reasoning_revise`); `validate` gates `revise ≥ 1` to `mode: plan`. Default `revise: 0` keeps single-pass the norm.
