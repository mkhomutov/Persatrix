...
- **Ollama mode — run the whole society on a real local model, no API key, no cloud spend.** A first-class `ollama` provider ([`agents/llm_ollama.py`](agents/llm_ollama.py)) joins the `LLMProvider` lineup as a thin subclass of `OpenAIProvider` — Ollama serves the OpenAI-compatible API verbatim at `/v1`, so it reuses the cloud provider's message/tool translation and overrides only the `gen_ai.system` name (`ollama`), a localhost-default `base_url`, and an optional forced model. Set `PERSATRIX_OLLAMA=1` to route every agent to a locally-served model (offline mode wins if both flags are set), or set `provider: ollama` on a single agent. Unlike offline mode this is **real inference** — usage carries the model's actual token counts — but it runs on your own hardware, so cloud spend is `$0`.
- **`make demo-ollama`** brings the full stack up against a bundled `ollama` container via the new [`docker-compose.ollama.yaml`](docker-compose.ollama.yaml) overlay: a one-shot service pulls `PERSATRIX_OLLAMA_MODEL` (default `llama3.2`) into a persistent volume and each agent waits for that pull before starting. No `ANTHROPIC_API_KEY` required. See the README "Run a real local model — Ollama" section.

### 📝 Config / Schema

- [`schemas/agent.schema.json`](schemas/agent.schema.json) now documents the previously undocumented `provider` field (`anthropic` / `openai` / `ollama` / `mock`) and `provider_config.base_url`, so an explicit `provider: mock` / `provider: ollama` (and the existing OpenAI-compatible `base_url` for local models) validate instead of being rejected by `additionalProperties: false`. The `ollama` provider defaults `base_url` to `http://localhost:11434/v1`; the `PERSATRIX_OLLAMA` / `PERSATRIX_OLLAMA_MODEL` / `PERSATRIX_OLLAMA_BASE_URL` env knobs are documented in [`.env.example`](.env.example).
