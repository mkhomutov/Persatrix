...
- **Create channels from the browser (RFC 0048 channel-creation amendment).** The Channels panel gains a collapsed "New channel" form with a **Group / Direct** choice. *Group:* pick a name (the server derives the canonical `group:<name>` id, shown read-only), an optional description, and **persona** members (task agents are excluded) with a per-member respond policy, over the already-exposed `POST /api/v1/channels` (no new endpoint); the acting user is auto-added as a member so they can post. *Direct:* pick one persona and an opening message to open a 1:1 DM via the chat façade (`GetOrCreateDM`), landing you in the resulting `dm:` channel. It closes the Slice-1 gap where a tester could *watch* personas interact but could not *create* a channel to put them in without hand-editing `config/channels.yaml`. The affordance ships **dark** behind a new per-panel `channel_timeline.create_enabled` toggle (default `false`) and renders only when that toggle is on **and** the channel store is wired (`create.available`, runtime-derived) — the forward-compat seam that becomes capability-gated once RFC 0039 auth lands. The structural-write-before-auth carve-out is a signed-off, local-mode-only exception (the endpoint is already reachable unauthenticated on the localhost surface).
- **One conversation surface — DM ergonomics folded onto the Channels panel (RFC 0048 chat-panel-retirement amendment, PR 1 of 2).** A chat *is* a `dm:` channel server-side, so the console no longer maintains two panels over one model. The Channels panel gains a **persona entry point**: pick a persona to open a direct message — a persona header (name — role — capabilities), the synchronous chat-façade send with its abortable "thinking…" turn, and the session/epoch scope selector + "Acting as" override all carry over from the (still-present, PR 2 removes it) Chat panel. A DM **renders as a channel** (it reuses the same timeline rows + poll loop), so there is one rendering path for both DMs and group channels; the per-turn scope *annotation* is dropped in exchange (the scope selector itself still scopes each send). The old cross-panel "view in timeline" deep-link and its `nav.svelte.js` module are retired — a DM is just an in-panel selection now. No backend, endpoint, schema, or `config/ui.yaml` change.
