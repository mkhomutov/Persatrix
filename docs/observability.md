...
| `channel.dispatch` | `GRPCMessageDispatcher.Dispatch()` (Go orchestrator fan-out; ISSUE-0032) | `channel.id`, `channel.message_id`, `recipient.agent_id` (set at span start); `recipient.address` (added only after the registry resolves a real address, so a lookup miss never pollutes address-cardinality dashboards). Plus `session.id` — the resolved per-`(agent, channel)` session (RFC 0031), set from the `--session` override or the auto-binding — and `epoch.id` — the resolved run/test-isolation epoch (ISSUE-0085), set from the `--epoch` override or the boot `PERSATRIX_EPOCH`. Both are **low-cardinality-on-span, never a metric label** (RFC 0031 OQ #7), so a trace can be pivoted to the conversation / logical run it served. Each is omitted when unresolved (channels-off / pre-wiring path → persona falls back to its `legacy` / `live` construction snapshot). Status discipline: the best-effort unknown-participant drop stays `Unset` (no `RecordError` — a `channels.yaml` typo must not redden error-rate dashboards, RFC 0011 §C); registry / readiness / dial failures `RecordError` + set `Error`. |
...
Floor control (RFC 0030 Layer 2.5, the floor-control amendment) emits two `channel.conversation.*` instruments under the serialized speaker round. `channel.conversation.floor_turn{channel_type, outcome}` counts one per completed speaker turn, with `outcome ∈ {replied, timeout}` — `replied` when the speaker's reply landed and advanced the loop, `timeout` when it stayed silent past `floor_turn_timeout_seconds` (default 45 s) and the loop fail-opened to the next speaker. The `timeout` share is the stalled-floor-holder rate; `sum(rate(channel_conversation_floor_turn{outcome="timeout"}[5m])) / sum(rate(channel_conversation_floor_turn[5m]))` calibrates the per-turn timeout (amendment D2) — a high ratio means the budget is too tight (real turns being cut off) or personas are wedged. `channel.conversation.floor_round_duration{channel_type}` is the per-round wall-clock in milliseconds (one observation per round, timed from floor acquisition); because responders go serial, a round of N candidates where M stay silent can block the publish path for up to M×timeout, so a rising p95 here is the trigger to revisit the no-cap decision (amendment D4) with a configurable `max_responders_per_round`. Both labels are `channel_type` only (floor control runs on group channels), so cardinality is bounded by the channel-type set; per-channel drill-down lives in spans. Histogram exemplars link a slow round back to its trace.
...
The RFC 0030 Tier-B salience bid (the dynamic no-pile-on layer downstream of the gate) rides the *same* `channel.messages.gated` counter for the turns it suppresses, but adds `policy="low_salience"` and a `reason` attribute so a fail-closed branch is distinguishable from genuine dampening on a dashboard. `reason ∈ {below_threshold, declined, parse_failure, lease_denied, llm_error, model_unresolvable}` on the scalar gate; under RFC 0051 reasoning (`mode: bid|plan`; `bid` is the default on a governed channel since v0.3.10) the score-only `below_threshold` gives way to the semantic codes `{already_answered, only_agreeing, nothing_to_add}` and `adds_substance` is a speak verdict (never gated). A non-zero `reason="lease_denied"` or `reason="llm_error"` rate means a `fast`-model outage or wallet back-pressure is masquerading as the feature working — alert on those, not on the dampening codes. The companion `channel.messages.salience_skipped{reason="channel_too_large"}` counts the TB6 oversized-channel skip (the bid never ran), which is *not* a suppression.

`deliberation.parse_failures{mode}` (RFC 0051 Phase 1a) is a separate, **mandatory never-gated** counter: a structured `should_post` verdict that fails to parse falls closed to silence, and this counter makes that parser break alertable rather than buried in the suppression totals. It is **additive, not a re-route** — under reasoning a parse failure fires *both* this counter *and* `channel.messages.gated{reason=parse_failure}`, so the two must **not** be summed as if disjoint. `mode ∈ {bid, plan}` (an `off` value marks a misuse of the public parser). Any sustained non-zero here is a prompt/parser drift to investigate, independent of how much the feature is dampening.

The RFC 0034 persona conversation window emits a `conversation_window.*` family (module-owned in `agents/observability/_metrics_conversation_window.py`; like the RFC 0051 `deliberation.*` / `reflexion.*` blocks it is deliberately *not* under the `agent.persona.*` umbrella — `metrics.py` is at the file-size cap, so the instruments live in a helper module). `conversation_window.cache_access{result=hit|miss}` is the per-turn fetch-cache look-up; hit rate is `hit / (hit + miss)` over *consulted* look-ups (a turn whose event carries no `message_id` consults neither, so the denominator stays honest). `conversation_window.cache_evictions` is the bounded-LRU eviction count — a sustained rate means the cache bound (`_WINDOW_CACHE` capacity, default 256) is undersized for the live channel count and thrashing. `conversation_window.fetch_duration` (ms histogram) is the cost of one real history fetch the cache exists to avoid; it is recorded on a **successful** fetch only, so a failed fetch's latency (a timeout is the worst case) never skews the steady-state cost the `max_turns` / `max_tokens` retune reads. `conversation_window.fallback{reason=fetch_failed|fetch_none}` is the §F silent-degradation signal — the window fell back to current-event-only despite an enabled config on a channel turn; a sustained rate is a history-endpoint outage masquerading as the feature working, so alert on it, not on a low hit rate.
