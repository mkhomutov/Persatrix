...
| 2026-06-01 | Claude (Opus 4.8) | macOS + Docker | V5 | Bridge ✅ / arc carried | v0.3.5 release-prep PR 1 ([v0.3.5-execution-report.md](v0.3.5-execution-report.md)). V5 **bridge** green live ([`test_session_continuity.py`](../../tests/integration/test_session_continuity.py): single-session arc reads back through every tier; arc-2 default recall excludes arc-1), and session write-tag + recall isolation verified live on persisted rows (see [MT-SESSION-003](MT-SESSION-003.md)). The full 5-leg, 45-min **qualitative** arc with RFC 0020 idle windows + LLM-reasoning judgment was **not** run in this environment (carried by the bridge gate); recorded ⚠️ Accepted-with-known-gap pending a full Go-+-Anthropic-+-idle-window run. |
