...
| 2026-06-10 | Claude (Fable 5) | macOS (Darwin 24.6.0) | ✅ **Step 6 only — Pass (check 1); check 2 zero replies, all Tier B** | Live on Anthropic, tip `6271243` (both floor-capable PRs [#601](https://github.com/mkhomutov/Persatrix/pull/601)/[#602](https://github.com/mkhomutov/Persatrix/pull/602) in). Four probes whose only structured mention was the `respond: never` human (`alex`): the orchestrator logged `channels: mentions name no floor-capable member` for **all four** message ids, and **zero `directed_elsewhere` suppressions occurred anywhere** (agent logs + `channel_messages_gated_total` — only `when_mentioned` (ember-owl) and `low_salience{below_threshold}` series exist). Both `participant`s ran a live salience bid on every probe (one provider call each — proof Tier A admitted them; a directedness drop is zero-call). The defect signature is absent → the discriminating check **passes**. Check 2's positive arm never landed: every bid declined — `Tier B salience bid suppressed turn (reason=below_threshold, score=0.2)` from **both** members, bar 0.8 (0.9 on the two probes naming `@alex` in free text — the NL other-named penalty) — across 3 re-probes including maximally direct, no-names-in-text questions. Per the step's own note this is a Tier B outcome, not a directedness failure. **Side findings for v0.3.8 release-prep:** (1) the demo `participant`s score a constant 0.2 at temp 0 and never clear the unset-threshold bar — on this stack the room answers *only* `@`-mentions; Step 3 of this MT and [MT-CHANNEL-RELEVANCE-002](MT-CHANNEL-RELEVANCE-002.md) will hit the same wall. (2) `channel_messages_gated_total` carries no per-agent label, so the two participants' `low_salience` series collide at the otel-collector (last-write, not sum) — the counter undercounts multi-agent suppression. |
