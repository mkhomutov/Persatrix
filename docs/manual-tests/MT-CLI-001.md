# Manual Test MT-CLI-001: `persatrix run` End-to-End Against a Running Orchestrator
...
**Purpose**: Verify that the `persatrix` CLI can submit a workflow run to a live orchestrator and print
the result, exercising the full Rust CLI → REST → orchestrator round-trip.

**Scope**: `persatrix run`, `persatrix status`, and `persatrix validate` subcommands against a locally running
orchestrator.
...
- Binary was renamed from `orch` to `persatrix` in PR #125. Historical test results below used the old name.
