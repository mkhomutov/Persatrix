...
> **A restart still leaves the rate-limiter bucket full.** The RFC 0009 bucket is not flushed when the orchestrator restarts, so the first turn after one can draw `429` for up to ~60 s. Read that as "wait", not as "the restart did not take" — restarting again only burns another turn.
>
> *(The registry warning that stood here through v0.3.14 is gone: since v0.3.15 the personas re-register themselves when the orchestrator comes back — [ISSUE-0125](../issues/ISSUE-0125-agents-never-reregister-after-orchestrator-restart.md). The restart steps below are safe as written, with no persona restart needed. If a leg ever does go quiet after a restart, `GET $PERSATRIX_SERVER/api/v1/agents` is still the check — and an orchestrator holding **zero** registered agents now says so at ERROR in its own log.)*
...
# wait for /healthz before the next turn — the personas re-register themselves
persatrix login          # as alice
# sanity check: GET /api/v1/agents with the login token lists every persona healthy
...
Let this interaction close too, before moving on — and note this **changed in v0.3.15**. RFC 0020 §C holds open interactions **in memory only**, and the personas used to be restarted at the top of Leg 3, which discarded any that were still open. They re-register themselves now ([ISSUE-0125](../issues/ISSUE-0125-agents-never-reregister-after-orchestrator-restart.md)) and are no longer restarted, so an interaction left open here **survives into Bob's leg** and can close under *his* principal — the aggregate-close residual ([ISSUE-0082](../issues/ISSUE-0082-orchestrator-per-request-session-principal-emission.md) R-1), not a new defect. Closing it here keeps that variable out of Leg 3.
...
# restart the orchestrator; wait for /healthz before the next turn
...
Re-bootstrap as alice (`--participant alice-person`) — the same `rm` → `bootstrap` → orchestrator restart dance as Leg 3 — **then apply the fresh-room rule**: delete Alice's DM channel so the re-ask cannot be served off the transcript, and let the next chat re-create it empty.
...
Set `auth.mode: disabled`, restart the orchestrator, and chat with no credential.
...
2. **Shared-room semantics — and the bound on them.** Two authenticated people in one *group channel* get per-speaker persona memory **for each turn's own write**: neither turn recalls the other's disclosures. That is the promise applied inside a room, not a regression; room continuity is unaffected (transcript and verbatim history are not principal-scoped). **Widened 2026-09-03 (v0.3.15): the carve-out is retired.** Through v0.3.14 this note had to say the promise did *not* extend to the close-derived aggregate or to a persona's relayed reply — [ISSUE-0123](../issues/ISSUE-0123-per-speaker-interaction-scope.md) (R-1) and [ISSUE-0124](../issues/ISSUE-0124-orchestrator-hop-drops-tenant-on-agent-cascade.md) (R-2). Both closed in v0.3.15 and were verified live ([v0.3.15 execution report](v0.3.15-execution-report.md)): the derived write binds the record's own frozen principal, and a relayed turn carries the causal tenant across the orchestrator hop. **The promise now holds for the close-derived aggregate as well as for each turn's own write**, so the release notes may state it at that wider scope.

    Two bounds still apply, neither a residual. Derived group memory now fragments **per speaker per tenant** ([ISSUE-0131](../issues/ISSUE-0131-derived-memory-has-no-speaker-attribution.md)) — a coherence trade, stated rather than discovered. And this MT still cannot *observe* either surface (see Out of Scope); [MT-MEMORY-GROUP-TENANT-001](MT-MEMORY-GROUP-TENANT-001.md) remains the instrument.
3. **Agent-origin turns stay `local` — only where no human caused them.** The fleet holds no accounts and publishes anonymously (RFC 0039 §Non-Goals), so a persona emits no principal of its own. **Narrowed 2026-09-03 (v0.3.15)**: [ISSUE-0124](../issues/ISSUE-0124-orchestrator-hop-drops-tenant-on-agent-cascade.md) (R-2) now attributes a *relayed* turn to its causal principal server-side. Expect `local` only from **autonomous / tick-origin** publishes; under `enabled`, a relayed row reading `local` is the R-2 regression signature.
4. **Provenance.** `PERSATRIX_MEMORY_PROVENANCE=1` is a weaker instrument here than the storage read: since v0.3.16 ([ISSUE-0122](../issues/ISSUE-0122-relationship-tier-emits-no-provenance.md)) the `relationship` tier records its admission, so Leg 3's identity read emits a `tier_admitted` line — one carrying no `principal_id`, which says the identity reached the prompt, never whose row it came from. Read `principal_id` off the tables.
