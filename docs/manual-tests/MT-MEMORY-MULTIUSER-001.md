...
2. **Shared-room semantics.** Two authenticated people in one *group channel* also get per-speaker persona memory — neither turn recalls the other's disclosures. That is the promise applied inside a room, not a regression; room continuity is unaffected (transcript and verbatim history are not principal-scoped). The release notes state it.
