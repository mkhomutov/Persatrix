...
1. **Wrong participant type (the ISSUE-0119 class).** The persona read an agent-typed row and found nothing — or, from the second turn in that room on, found the mistyped row its own close path had written. Check the delivered event's `sender_participant_type` (must be `user`) and Leg 1b's row type. A **release-blocking regression** of the [#799](https://github.com/mkhomutov/Persatrix/pull/799) fix, not a quality miss.
2. **Reasoning miss.** The identity line reached the prompt in `relationship_context` and the model ignored it. A quality finding.

> **Provenance sees this leg since v0.3.16** ([ISSUE-0122](../issues/ISSUE-0122-relationship-tier-emits-no-provenance.md), PR B1). With `PERSATRIX_MEMORY_PROVENANCE=1` the identity read emits one `persatrix.memory.tier_admitted` line with `tier=relationship` and `item_id=<participant_type>:<participant_id>` — on this leg, `user:alex` — and the tokens the tier was charged. Read it before the two modes above: **`item_id=user:alex` present** → the identity line reached the prompt, so a fail is mode 2 (reasoning miss); **`item_id=agent:alex`** → mode 1, the wrong participant type — the read matched an *agent*-typed row, which on a `make reset` stack is only possible from the second turn in that room (the close path first has to write an interaction onto the mistyped row), and the line means that row *did* reach the prompt, so check the reply for a mode-2 miss as well; **no `relationship` line at all** → the read matched nothing: a recall miss, **or mode 1 on a fresh channel's first ask** (an agent-typed lookup finds no row, so the tier renders nothing and records nothing) — triage it with Leg 1b's `relationships` query and the delivered event's `sender_participant_type` exactly as before. The record carries ids and tokens only, never the identity text, so the log stays safe to ship. Until v0.3.16 the tier charged the budget without recording an admission, and zero admissions was the *expected* reading here — the v0.3.13 run read that silence as a miss (corrected 2026-08-04); a run against a pre-v0.3.16 build must still read it that way.
...
| 2026-09-25 | Claude (Opus 5.5) | macOS 26.6.2 (`arm64`) | Anthropic `claude-sonnet-4-6` | ✅ Pass — **Legs 1b and 2b only** | A check after [#995](https://github.com/mkhomutov/Persatrix/pull/995) (`054f0de2`) rewrote the memory snippet's identity lines. Leg 1b ("Hi, I'm Sam and I lead the platform team. Please remember my name.") wrote a `user`-typed relationship row carrying `{"name": "Sam", "role": "Platform team lead"}` and no contact note. Leg 2b, in a fresh group channel holding only the sender and ember-owl: "Yes, Sam — you're the platform team lead. Got you on file." The trigger asked directly ("do you remember my name?"), a weaker ask than this leg's. The same two legs on `make demo-ollama` (llama3.2) could not run: on Docker's CPU every completion outlasted the persona's 300 s event timeout. Legs 1–4 not run. |
