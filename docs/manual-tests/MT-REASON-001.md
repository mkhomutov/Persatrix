...
4. Operator access to the **agent container logs** with the agent running at `--log-level DEBUG` (`docker compose logs -f agent` or the configured log sink), where the verbatim `reason_note` egresses as one `agent.deliberation.reason_note` record per suppressed turn (§E; since v0.3.16). The count-only `agent.deliberated` audit line (INFO) and the `deliberation.*` metrics are observable at the default level.
