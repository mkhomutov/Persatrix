...
4. Operator access to the **agent container logs** with the persona agents running at `--log-level DEBUG`. The stock compose files pass no `--log-level` (the agent default is `INFO`) and `make demo-*` loads explicit `-f` files, so an override file is not picked up: append `"--log-level", "DEBUG"` to the `command:` list of each persona service in `docker-compose.yaml` (`agent-ember-owl`, `agent-iron-fox`, `agent-nova-sparrow`) before precondition 1, and read `docker compose logs -f agent-iron-fox` (or the configured log sink). The orchestrator's own log store never shows these lines — a channel turn carries no execution id, so the log shipper's copy is dropped on arrival. That is where the verbatim `reason_note` egresses as one `agent.deliberation.reason_note` record per suppressed turn (§E; since v0.3.16). The count-only `agent.deliberated` audit line (INFO) and the `deliberation.*` metrics are observable at the default level.
...
- **Observe the reason** in the operator-debug **agent log**: that persona's turn ends in `DO_NOTHING`; the `agent.deliberated` audit line records the decision + a deliberation `reason_code` (e.g. `only_agreeing` / `already_answered` / `nothing_to_add`) **count-only**, and — at `DEBUG` — a separate `agent.deliberation.reason_note` record carries the one-clause verbatim `reason_note`. The audit never carries the note; at the `INFO` default the note appears nowhere and the reason is read at `reason_code` granularity.
...
| 2 | Semantic silence | a would-be-pile-on `participant` posts nothing; the agent log carries the `reason_code` (audit, `INFO`) and, at `DEBUG`, the `reason_note`; `deliberation.suppressed{reason_code,mode}` fires |
