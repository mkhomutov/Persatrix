...
| [MT-PROVIDER-WATSONX-001](MT-PROVIDER-WATSONX-001.md) | IBM watsonx.ai (native `ibm-watsonx-ai`) — cloud provider smoke; required `project_id`/`space_id` fail-closed | Active |
...
| v0.3.13 | [v0.3.13-execution-report.md](v0.3.13-execution-report.md) | ✅ Complete — the whole MT-MEMORY-CROSSROOM-001 v1.1 arc passed live on Anthropic, zero Fail; Leg 4's tool round **evidenced** on the first probe, legs 1b/2b run for the first time (in-room 2b re-run clean on an empty-transcript channel after Leg 2's own reply confounded it); three findings fixed in-PR — the `max_cascade_depth` CLI surface, CI never running `cargo test`, and the Leg 2b run rule (row back-filled at v0.3.15 Phase 4) |
| v0.3.14 | [v0.3.14-execution-report.md](v0.3.14-execution-report.md) | ✅ Complete — the whole MT-MEMORY-MULTIUSER-001 arc live on a real provider with two accounts, **both emitted principals read off storage** (the release gate's own criterion); two defects found and fixed before the tag rather than deferred — `persatrix chat` could not authenticate under `enabled`, and a restart re-derived replayed history into the shared tenant. Cost not measured (lost to teardown; the instruction was wrong — corrected at v0.3.15) (row back-filled at v0.3.15 Phase 4) |
| v0.3.15 | [v0.3.15-execution-report.md](v0.3.15-execution-report.md) | ✅ Complete — all **ten** legs of the extended MT-MEMORY-GROUP-TENANT-001 arc live on Anthropic for **$0.1343** across 29 leases, zero Fail: R-2 carried the tenant on 5/5 dispatches incl. a persona-sender publish, every close-derived record single-speaker **and** single-principal, replay `B = C` byte-identical. Two legs accepted-with-known-gap (F-4 quorum never formed, F-5 absence bar vacuous); **ten defects found in the MT itself** — it had never been run against a compose deployment — plus a near-miss where a preflight gate matched a comment and a green arc read as R-2 failing. MT-MEMORY-MULTIUSER-001 not re-run (accepted-with-known-gap) |
| v0.3.16 | [v0.3.16-execution-report.md](v0.3.16-execution-report.md) | ✅ Complete — all five legs of MT-PERSONA-CONFIDENTIALITY-001 live on Anthropic, Leg 5 under both auth modes, for **$0.4259** across 43 reconciled leases, zero Fail: Alice's two DM-taught facts read *withhold-disjoint* in front of Bob with fetch-failed 0 in both modes; the §F recall side door was found and fixed in the PR ([ISSUE-0158](../issues/ISSUE-0158-recall-filter-audience-blind.md)); Leg 4 accepted-with-known-gap ([ISSUE-0159](../issues/ISSUE-0159-episodic-score-inverts-bm25.md)) |
