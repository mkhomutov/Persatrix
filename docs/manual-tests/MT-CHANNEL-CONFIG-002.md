...
| 2026-06-16 | Maksim Khomutov | `015149a` (Anthropic overlay, `ENABLE_UI=1 up --build`; Chrome-driven) | **PASS (core steps 1–5; edge cases deferred to vitest)** | First live web exercise. **Step 1** ✅ panel renders for the group channel with every knob + provenance badge; idle timeout **600 / Inherited default**; interaction budget renders the effective **0 / Inherited default** (post-amendment + #660 mock fix — resolves the value, no longer empty/`null`). **Step 2** ✅ untick Inherit + set 60 + Save → single-key PATCH landed (`PATCH …/config` 200, `PutChannelConfig config_revision:1`); the idle row reads **60 / Overridden on this channel**. Observed (expected post-ISSUE-0103): the first edit seeds **all** knobs from resolved governance, so every row flips to "Overridden" and the chair **survives** (no detachment — ISSUE-0103 fix confirmed live, unlike the pre-fix behavior the MT's ⚠ note describes). **Step 3** ✅ G4 — `./bin/persatrix channel config get planning` reads back **60 / `channel` / revision 1**, identical to the browser; the browser wrote the one store the CLI reads. **Step 4** ✅ drove activity then a >60 s gap; idle-rotation decision fired at `window:60, rotated:true` (gap 78.2 s) — the running channel honors the browser edit with no restart. **Step 5** ✅ re-tick Inherit + Save → idle row back to **600 / Inherited default**; CLI reads **600 / `default` / revision 2** (`config_revision:2` — revert is a new higher revision, never a decrement). **Edge cases 1–3 not re-run live** — each is pinned deterministically by the named vitest/Go suites (`ChannelTimeline.settings.test.js` toggle-off no-mount + `channel_config_handlers_test.go` 403; `ChannelSettings.test.js` 409-reload; the Phase 1 `validateEscalationChair` 400), so the live arc focused on the G4 cross-surface acceptance the automated half cannot cover. |
