...
| 4 | R-1 close binding (MUST exclude/tag the fanned closing turn from speaker projection + fact extraction — RFC 0020 §G stated exception), asymmetry cleanup. ~~Reserve re-size~~ pulled forward into PR 3 at review (2026-08-28): `1 + N×max(1, N−1)` in `synthesis_reserve.go` | `feature/v0315-issue0123-scope-key` (re-size) / `feature/v0315-issue0123-close-path` | ⬜ Not started | — | — |
