...
**Status**: ✅ Accepted (decisions locked 2026-06-03; PR 1 of 2 implemented — see [Implementation](#implementation))
**Date**: 2026-06-03
...
- **No change to the channel-creation amendment's *surface*.** This composes with the [channel-creation amendment](0048-amendment-channel-creation.md): the same Channels panel gains DM ergonomics (here) and channel creation (there). The create form's **direct mode** (its "lighter, create-form-local cousin" of this amendment's persona entry point) is **kept**, with one mechanical adjustment forced by §C: its landing no longer rides the removed `nav.targetChannel`; instead the form hands the persona back and the panel **opens DM mode** for it (the same consolidated surface the persona picker uses) — because a `dm:` channel is filtered out of the group-channel picker. Its direct persona `<select>` is relabelled "Direct-message persona" so it stays individually addressable while the top persona picker is also visible. (The two DM entry points are by-design cousins; whether the create-form one is later retired as redundant is a follow-up, not this PR.)
...
1. **PR — Consolidate conversations onto the Channels panel (§B–§D). ✅ Done.** Client: added the persona entry point + DM mode to [`ChannelTimeline.svelte`](../../web/src/panels/ChannelTimeline.svelte) (persona picker, persona header, synchronous `sendChat` in DM mode, scope selector, abortable turn) — DMs **render as channels** (`ChannelMessage` rows + poll, per the rendering decision in §B), with the chat composer extracted to `DmComposer.svelte`; merged the onboarding empty states; rehomed the sticky DM selection (`selection.dmAgent`); deleted `nav.svelte.js` and the §F deep-link (including from `Chat.svelte`/`PersonaHeader.svelte`, which keep working sans the now-redundant link until PR 2). Reused `PersonaPicker`/`PersonaHeader`/`ScopeSelector` and the existing `ChannelMessage` timeline. `config/ui.yaml` is **unchanged** (the `chat` panel still serves until PR 2). The create form's direct-mode landing was rerouted to DM mode (see §"does NOT change").
2. **PR — Remove the Chat panel (§A).** Delete `Chat.svelte`, `ChatMessage.svelte`, and the `chat` entries from `KNOWN_PANELS` ([`bootstrap.js`](../../web/src/lib/bootstrap.js)), `config/ui.yaml`, and any `config/environments/*` override; drop the `#/chat` route from the shell; retire the now-orphaned `selection.chatAgent`. Exercised by the existing Svelte component/bootstrap tests (assert the panel set no longer includes `chat`; the consolidated panel covers the DM flow E2E per the [RFC 0048 test strategy](0048-operator-tester-web-console.md#test-strategy)).
