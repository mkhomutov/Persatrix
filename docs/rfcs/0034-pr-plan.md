...
**Master plan**: [v0.3.1-plan.md Phase 1 (RFC 0034 PR plan PR — row 1b)](../v0.3.1-plan.md#phase-1--author-the-two-rfc-pr-plans)
...
- **[OQ #1](0034-persona-conversational-working-memory.md#open-questions) — per-session vs. per-channel transcript window: resolution 1a (per-channel, no session filter).** The window filters on `event.channel_id` only; rows are admitted regardless of `chat_session_id` or `persatrix_session_id`. Policy anchor: [RFC 0031 PR plan §OQ #1 resolution 1a](0031-pr-plan.md#open-question-resolutions-locked-at-plan-authoring-time) frames Phase 1 session columns as **operational hygiene**, not a **prompt-content privacy boundary**. The Conversation Window follows the policy. Forward-compatible: if RFC 0031 Phase 2 ever re-frames the boundary as privacy-bearing, the window picks up the same session filter additively. **Phase 1 unit-test obligation**: assert two events on the same channel under different `persatrix_session_id` values share one window.
...
| File | Change |
|------|--------|
| `agents/channel_history_fetcher.py` | **New**. Module exposes (a) a `ChannelHistoryFetcher` `typing.Protocol` with one method `async fetch(channel_id: str, *, limit: int) -> list[dict[str, Any]]`, and (b) a default production implementation `HttpChannelHistoryFetcher` backed by `aiohttp` with the same 10s timeout the catch-up path uses today. The implementation is the verbatim body of the existing [`_fetch_channel_history`](../../agents/channel_catchup.py#L352) helper, lifted unchanged. |
...
- Network timeout (simulated via a slow stub) raises a typed exception; the caller can catch and degrade.
- HTTP 404 / 5xx returns `[]` and logs a WARN — same behaviour as the lifted helper today.
- Catch-up regression: `test_channel_catchup.py` suite still passes after switching to the injected fake.
...
- [ ] `pytest tests/integration/persona/test_conversational_continuity.py -q` passes.
...
- [ ] If any finding touched the call site, the [§Tests](#tests-2) integration test from PR 3 re-runs green.
...
| Risk | Mitigation |
|------|------------|
| The PR 1 fetcher refactor accidentally changes catch-up behaviour (e.g. error semantics on a flaky history endpoint). | PR 1 lifts the body verbatim; `test_channel_catchup.py` runs unchanged after fixture migration. The Protocol introduction is type-only. |
