...
- **No telemetry instruments shipped in Phase 1.** The `conversation_window.*` metrics surface lands in Phase 3 per [RFC §Phase 3](0034-persona-conversational-working-memory.md#phase-3-instrumentation-and-tuning); shipping inert counters now would invite premature dashboard work.
...
- [x] [v0.3.1-plan Master Progress Overview](../v0.3.1-plan.md#master-progress-overview) row 3b → 🔀 PR open (closeout).
...
**Implemented in v0.3.10** ([0034-phase3-pr-plan.md](0034-phase3-pr-plan.md)). Cache-access (hit/miss), eviction, fetch latency, and fallback-to-current-event counts are exposed as OTEL metrics under `conversation_window.*` (module-owned in [`agents/observability/_metrics_conversation_window.py`](../../agents/observability/_metrics_conversation_window.py)). The `max_turns` / `max_tokens` retune from a telemetry sample is the one outstanding follow-up — gated on collecting the data the new metrics produce. Tunables documented in [`docs/guides/persona-agents.md`](../guides/persona-agents.md). The [RFC §F "Known gap" cache framing](0034-persona-conversational-working-memory.md#f-caching-and-fetch-policy) (a)→(b) re-spec remains optional, deferred until telemetry justifies it.

**Conversation-window fetch cache has no eviction.** *Resolved in v0.3.10 (Phase 3).* Phase 1's in-process `_WINDOW_CACHE` never deleted an entry — a channel seen once kept its `(message_id, raw_rows)` tuple for the life of the process, so the dict grew with the number of *distinct* channels a long-running orchestrator ever served (not the number concurrently active), each entry holding up to `max_turns + 1` raw row dicts. Harmless for the DM dogfood; unbounded over a long-lived process across many channels. Phase 3 replaced the dict with a bounded LRU (`_WindowCache`) extracted into [`agents/persona_runtime/_conversation_window_cache.py`](../../agents/persona_runtime/_conversation_window_cache.py), and the `conversation_window.cache_evictions` counter now makes an undersized bound observable. Surfaced in PR 2 review; the in-module comment documented the gap until it was fixed here.
