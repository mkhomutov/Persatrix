...
- **No `enabled=false` path in the integration test.** The test exercises the live path; the disabled-path semantics are PR 2's unit-test territory.
...
The PR 3 review's single blocking finding — the reconstructed `messages` array could open with an `assistant` turn (a hard Anthropic 400) — was fixed in PR 3 itself via the `_drop_leading_assistant_turns` guard, not deferred. The non-blocking findings below are deferred to this PR:

- **`conversation_window` config is resolved on the unwired short-circuit path.** `_ConversationWindowMixin._build_seed_messages` ([`conversation_seed.py`](../../agents/persona_runtime/conversation_seed.py)) resolves and caches `_conversation_window_config` *before* the `_history_fetcher is None` check. On the unwired path — task-only agents and partial-init test paths — the resolved config is then discarded: `resolve_conversation_window_config` runs for nothing. It cannot crash (the resolver never raises), so this is harmless, but a reader expects the `None`-fetcher short-circuit to skip *all* conversation-window work. **Fix**: move the lazy resolve below the `_history_fetcher is None` check so the short-circuit is total. Pure reordering — no behaviour change; the existing unwired-fetcher test still pins the seed shape.
- **`resolve_conversation_window_config` type-checks but does not range-check the integer counts.** The resolver rejects non-`int` (and `bool`) values for `max_turns` / `max_tokens` and degrades them to the per-key default, but accepts any other `int` — including `0` and negatives. [`schemas/agent.schema.json`](../../schemas/agent.schema.json) enforces `minimum: 1`, so configs gated through `make validate` are safe; dict-built and test configs bypass that, and a `max_turns: 0` (or negative) silently yields an empty replayed window — `_apply_admission` drops every turn — instead of degrading to the default. The resolver docstring's "degrades a bad value to its per-key default" oversells this: an out-of-range `int` is *accepted*, not degraded. **Fix**: treat `max_turns < 1` / `max_tokens < 1` as malformed and degrade to the per-key default, consistent with the wrong-type path; update the docstring to match. The JSON schema stays the production range gate — this only makes the resolver self-consistent for the configs that bypass it.
- **Mixin base-class list formatting in `_LLMPersonaAgent`.** [`agents/persona_runtime/__init__.py`](../../agents/persona_runtime/__init__.py) places `_ActionLoopMixin, _ConversationWindowMixin,` on one shared line while every other base in the class statement is one-per-line. Cosmetic only — the MRO is unaffected. **Fix**: split `_ConversationWindowMixin` onto its own line for consistency.
...
- [ ] RFC 0034 status = `⚠️ Partially Implemented`.
- [ ] [ROADMAP RFC Master Index](../../ROADMAP.md#rfc-master-index) updated.
- [ ] [v0.3.1-plan Master Progress Overview](../v0.3.1-plan.md#master-progress-overview) row 3b → ✅.
- [ ] [ISSUE-0052](../issues/ISSUE-0052-persona-conversational-working-memory-gap.md) status updated to "Closed by RFC 0034 Phase 1 (v0.3.1)".
...
| 5 | Phase 1 closeout | `feature/v031-rfc0034p1-close` | ⬜ Not started | — | — |
