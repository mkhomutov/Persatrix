...
3. **Token-usage attribution under veto.** If `before_model` vetoes, the model was not called and there is no token usage — but if a callback itself called the model (e.g., a moderation LLM), where is that usage recorded? Lean: a separate `role="callback"` discriminator or a dedicated `CallbackModelOutput` event — reusing `ModelOutput(role="assistant")` for in-callback model calls would mislead subscribers (channel publish would treat it as the assistant's turn output). Resolve before Phase 1.
...
6. **`CallbackContext.state` typing during the 0041 → 0042 gap.** Phase 1 of this RFC ships before [RFC 0042](0042-state-namespacing-by-scope.md) Phase 1, but §C references `ScopedState` (a 0042 type) on the callback context. Lean: Phase 1 ships `CallbackContext.state` typed against the existing per-store accessors (`MemoryStore`, channel store, session namespace, working memory) bundled as a transitional `LegacyState` Protocol; the rename to `ScopedState` is part of the same Phase 2 sweep that re-types `StateDelta.scope`. Subscribers must not depend on `state` having the `ScopedState` shape until then.
