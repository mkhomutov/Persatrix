...
- [x] Catch-up fetch integration test: agent restarts mid-conversation, fetches recent messages, replays through ingest, **no** outbound responses to replayed messages — pinned by `tests/unit/python/test_replay_mode_action_loop.py` (replay-mode short-circuit), `tests/unit/python/test_channel_catchup.py` (REST round-trip + membership filter + oldest-first ordering + best-effort error paths), and `tests/unit/python/test_server_catchup_wiring.py` (startup wiring + persona-only filter + shared-session contract)
