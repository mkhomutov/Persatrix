...
|----------|-------------|
| `PRAGMA user_version` baseline | ✅ Landed in PR 2's SF-4 migration — `applySchema` now stamps `channelStoreSchemaVersion` (`internal/channels/sqlite_schema.go:125`) and `TestSQLiteStore_SchemaVersion_Stamped` pins the contract. |
