...
6. **Operator-controlled, conservative default.** A per-channel `reasoning` knob fits the [RFC 0050](0050-extensible-channel-configuration.md) config surface. It is **inert until a channel is Tier-B-governed**; once governed it defaults to the *minimal active rung* — `bid` (silence verdict only, never the full plan-threaded `plan`) — and `off` stays a one-flip kill switch ([OQ 2](#open-questions), [§G](#g-configuration--an-rfc-0050-knob)).
