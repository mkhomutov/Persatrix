...
- **Tier B** — `run_salience_gate` ([`salience_gate.py`](../../agents/persona_runtime/salience_gate.py)), the action-loop seam that runs the *pure* bid `evaluate_salience` ([`salience_bid.py`](../../agents/salience_bid.py)): a leased `fast`-model bid (≈64 output tokens *today* — the structured verdict needs a larger, `mode`-scaled cap, [§C](#c-the-deliberation-verdict); temp 0.0, bias-to-silence) that runs *only* on the ambiguous open-floor admit. Drop → `DO_NOTHING`.
...
7. **Retain `threshold` as a hard floor under reasoning?** [§C](#c-the-deliberation-verdict) supersedes the numeric score gate, which makes the per-member `threshold` ([RFC 0050](0050-extensible-channel-configuration.md)) inert under `mode: bid`/`plan`. A conservative variant keeps `threshold` as a *floor* and layers `should_post=false` as an additional veto on top (stay silent if `score < bar` **or** `should_post=false`): it preserves operator threshold-tuning and is a near-drop-in generalization of the existing `speak: no` veto, but it does **not** fix the *under-admit* half of [Motivation §1](#motivation) — a below-floor turn that would add real substance still cannot speak. Default: supersede (fixes both over- and under-admission, at the cost of orphaning the knob); revisit if operators report losing useful `threshold` control. Decide at the Phase 1 PR, since it changes the Phase 1 parser contract.
