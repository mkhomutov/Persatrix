...
- **Room-wide closes fan.** A structural close, an end-vote quorum, the RFC 0052 bounded/cost close, and the close-notification turn are *room* events; each closes **every** record open in the scope (`close_scope`), and the close-notification message lands as the final turn of each. Idle remains per-record — a speaker who went quiet idles out on their own timer. Consequence: `agent.interactions.closed` and its per-reason subtotals fire once per **record**, so a room close increments by N. **Stated exception (PR #846 review):** that fanned closing turn is the one deliberate breach of single-speaker construction — it carries the closing sender's message into every sibling record, discriminable in the turn payload (`sender` ≠ the record's `speaker_id`). The residuals PR 4 close-path binding MUST exclude or tag it when projecting `speaker_id` and extracting facts, or a fact from the closer's words would be attributed to the record's own speaker — the misattribution the key exists to prevent.
...
The coherence trade is deliberate and stated: a persona's close-derived memory of a group discussion fragments per speaker per tenant (N partial views, not one narrative); room continuity is unaffected because the transcript and RFC 0036 verbatim history are scoped by neither axis. Three further consequences of the re-key, stated after the PR #846 re-review rather than discovered later: **(a)** a message from a sender with no open record — a session-end terminator included — opens that sender's record like any other turn, so a recordless closer's terminator becomes its own small single-turn fragment rather than being discarded or absorbed into another speaker's record; **(b)** a late straggler of a retired wire conversation likewise opens a fresh record stamped with the **retired** id and is closed by the next current-wire event as its own late fragment of *that* conversation — honest attribution at the price of one extra episode per straggler; **(c)** a room-wide close stamps every sibling with **one** `closed_at` instant, so RFC 0026 fact assertions from sibling records can tie on `asserted_at` — the storage layer's deterministic later-arrival-wins tie rule (previously unreachable in production) now decides those, until the PR 4 speaker binding disambiguates them properly.
...
**Amendment 2026-08-28 (the §G re-key's unit, PR #846 review):** since a room now holds one record per `(principal, speaker)`, a room-wide close fans over N records — but the reflect counter (and the DM relationship bump, `record_closed_interaction`) stays **per close event**, not per record: every fan designates one `conversation_lead` record — the first whose Phase-1 persist actually scheduled Phase 2 — to carry both, so the §H threshold keeps tracking conversational arcs rather than inflating by room size. The idle sweep leads per **scope** per sweep (same-scope records idling out together are one conversation going quiet), the stale-rotation splits lead per retired wire conversation, and a max-turns close riding the same event as a session end shares that event's one lead. Stated residual: the lead's own Phase-2 finalize failing after scheduling forfeits the event's tick — the same failure surface as its summary.
