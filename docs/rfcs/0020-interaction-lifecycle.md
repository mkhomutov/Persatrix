...
- **Room-wide closes fan.** A structural close, an end-vote quorum, the RFC 0052 bounded/cost close, and the close-notification turn are *room* events; each closes the scope's records rather than one speaker's. **What each admits differs**, and the difference is load-bearing: every fan skips a record positively stamped with a *different* wire interaction id — a successor conversation opened by a rotation the trigger predates, which must not be buried as "ended"; the end-vote close additionally skips a record that was not open when its vote was parked, since its quorum never judged that conversation; and the close notification applies the wire conjunct too. **Every fan leaves replay-opened records to the catch-up sweep** — a replayed span is closed by that pass's own `REASON_CATCHUP_COMPLETE`, never under a live room cause, and the exclusion lives in `InteractionTracker.admitted_records` — the eligibility seam every fan reads, whether through `close_scope` or (for the close notification, which needs the admitted set before it closes anything) directly — rather than in `close_record`, the method the sweep itself closes them with. **Only the close notification also ingests**: it lands the closing message as the final turn of each record it closes, while the other three close their records without a terminator turn. Idle remains per-record — a speaker who went quiet idles out on their own timer. Consequence: `agent.interactions.closed` and its per-reason subtotals fire once per **record**, so a room close increments by N. The same multiplier reaches spend, not just counters — the close path schedules one summarise-and-extract LLM call per closed record, and the §Security `max_turns` cap is per record, so both the call count and the buffered-turn ceiling scale with the number of distinct speakers rather than with the room. **Stated exception (PR #846 review):** that fanned closing turn is the one deliberate breach of single-speaker construction — it carries the closing sender's message into every sibling record, stamped in the turn payload as `room_close` (`agents/memory/interaction_types.py`), so consumers discriminate it on a recorded fact rather than re-deriving `sender` ≠ the record's `speaker_id`. The `participants` read surface already excludes it. The `speaker_id` projection discharges that as **exclude**, at one chokepoint: `close_entries.own_turn_items` is how every close-pipeline consumer reads a record's turns — the summarise+extract input, the single-turn fast path, and the turn context `close_path.persist_closed_interaction` persists — so a room-close turn whose `sender` is not the record's speaker reaches none of them, and neither a derived fact nor the stored row's FTS-indexed context can carry the closer's turn under the record's own speaker. The persisted `turn_count` counts those same post-exclusion turns, so the row never claims a turn it does not hold. It stays on the closer's OWN record, where the turn is native and the content is attributable.
