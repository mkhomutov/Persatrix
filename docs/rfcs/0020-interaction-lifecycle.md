...
- **Room-wide closes fan.** A structural close, an end-vote quorum, the RFC 0052 bounded/cost close, and the close-notification turn are *room* events; each closes the scope's records rather than one speaker's. **What each admits differs**, and the difference is load-bearing: the structural (session-end) close takes every *live* record in the scope (`close_scope`); the end-vote and bounded/cost closes skip a record positively stamped with a *different* wire interaction id — a successor conversation opened by a rotation the trigger predates, which must not be buried as "ended"; the end-vote close additionally skips a record that was not open when its vote was parked, since its quorum never judged that conversation; and the close notification applies the wire conjunct too. **Every fan leaves replay-opened records to the catch-up sweep** — a replayed span is closed by that pass's own `REASON_CATCHUP_COMPLETE`, never under a live room cause, and the exclusion lives in `close_scope` rather than in `close_record` (the method the sweep itself closes them with). **Only the close notification also ingests**: it lands the closing message as the final turn of each record it closes, while the other three close their records without a terminator turn. Idle remains per-record — a speaker who went quiet idles out on their own timer. Consequence: `agent.interactions.closed` and its per-reason subtotals fire once per **record**, so a room close increments by N. The same multiplier reaches spend, not just counters — the close path schedules one summarise-and-extract LLM call per closed record, and the §Security `max_turns` cap is per record, so both the call count and the buffered-turn ceiling scale with the number of distinct speakers rather than with the room. **Stated exception (PR #846 review):** that fanned closing turn is the one deliberate breach of single-speaker construction — it carries the closing sender's message into every sibling record, discriminable in the turn payload (`sender` ≠ the record's `speaker_id`). The residuals PR 4 close-path binding MUST exclude or tag it when projecting `speaker_id` and extracting facts, or a fact from the closer's words would be attributed to the record's own speaker — the misattribution the key exists to prevent.
