...
**Chat is leased as a channel message** ([ISSUE-0155](../issues/ISSUE-0155-rest-chat-leased-as-channel-message.md)). PR 4 tagged `CAUSE_CHAT` on the gRPC `SendChatMessage` servicer, but REST chat has gone through a DM channel since v0.3.0 ([chat-as-DM amendment](0011-amendment-chat-as-dm.md)): each chat turn reaches the persona through `ReceiveChannelMessage` and leases under PR 6's `CAUSE_CHANNEL_MESSAGE`, because the `chat_session_id` key that selects `CAUSE_CHAT` stops at the orchestrator. So PR 6, not PR 4, is what put REST chat under a lease and closed the v0.2.3 bypass, and nothing the orchestrator drives produces `CAUSE_CHAT`. The label is kept rather than restored: the wallet reads the cause only in its log lines, per-cause budget policies stay out of scope (above), and [RFC 0032](0032-channel-interaction-layer.md) treats the chat session as a concept to retire along with the chat façade.

**Persona calls share one workflow total** ([ISSUE-0157](../issues/ISSUE-0157-persona-calls-share-one-workflow-budget.md)). §B charges every lease to all three scopes, and §C sends an empty `workflow_id` for chat and TICK. The wallet keeps a per-workflow total under the empty string like any other id, so every persona call — compose turns, bids, critiques, close summaries, and any workflow step a persona serves — counts against one shared total, and the per-workflow limit caps them all together.
