...
Estimating `estimated_input_tokens` pre-call requires tokenising the prompt. Today neither side does this: [`internal/cost/`](../../internal/cost/) computes cost from already-counted tokens (provider-reported, post-hoc) and the Python agent has no `tiktoken`-style dependency. Two viable answers, both deferred to [Open Question §5](#open-questions) as a **Phase 0 blocker** (the choice changes the proto contract and the Python client surface, so it must close before Phase 1 lands):

1. **Shared tokeniser.** Vendor a Python BPE/`tiktoken` wrapper in `agents/`, exposed via a small in-process call from `LLMClient`; the Go side either re-implements the same vocab or accepts the agent's tokenisation as authoritative for both the lease and the post-hoc record.
2. **Pessimistic length-based estimator.** `estimated_input_tokens ≈ ceil(len(prompt_chars) / k)` for a per-model `k` (e.g. `k=3.5` for Claude). Systematically over-grants leases; over-accounting is already the safe-side bias per [Failure Modes](#f-failure-modes), so this trades acquire-time precision for zero new code paths. Acceptable as a v0.3.x stopgap if (1) is judged too large.
...
When RFC 0011 channels deliver a message and the recipient agent generates an LLM-backed reply, the lease is acquired with cause = `CHANNEL_MESSAGE`. This phase ships alongside [RFC 0011 PR 4](0011-pr-plan.md)'s response gate so the gate runs *before* lease acquisition (no lease held during gate evaluation).
...
5. **Tokeniser parity between Go and Python for the input estimate.** Both sides must agree on the input-token count, or settlement deltas will be systematically biased. Confirmed during [Section E rewrite](#e-python-client-integration): neither side tokenises today — `internal/cost/` only computes cost from already-counted tokens, and the Python agent has no `tiktoken` dependency. **Promoted to Phase 0 blocker.** The choice between a shared tokeniser and a length-based pessimistic estimator must close before Phase 1 lands; both proto and Python client surface depend on it.
