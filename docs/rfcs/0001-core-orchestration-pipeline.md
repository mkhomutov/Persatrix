...
>
> **Implementation warning:** Do NOT add `RunRetrying RunStatus = 5` in this RFC's implementation. The value 5 is reserved for alignment with `proto/task.proto` `TaskStatus.RETRYING`, but adding it without Scheduler retry support would be dead code that misleads consumers into believing retry transitions are supported. It will be added by RFC 0003.
...
- `CreateRun` generates a UUID if `run.ID` is empty. If a run with the given ID already exists, `CreateRun` returns an error (parallel to the Registry's `ErrAgentAlreadyRegistered` behavior — which the registry itself dropped on 2026-08-23; `state.ErrRunAlreadyExists` is unaffected). A corresponding `ErrRunAlreadyExists` sentinel error is defined for this case.
- `GetRun` returns a deep copy of the `WorkflowRun` to prevent callers from mutating internal state. (Same rationale as the Registry's `List` snapshot — without this, concurrent callers like the Scheduler and REST API could corrupt the store.) Deep copy is implemented via manual field copy; both the `Steps` map and the `Inputs` map must be reconstructed via manual copy — a simple struct copy shares the underlying map references for both, creating a subtle concurrency hazard where a caller mutating the returned map would corrupt the store's internal state. This applies to all `map` fields on `WorkflowRun`, not just `Steps`.
...
- `DeleteRun` removes a run from the store by ID. Returns `ErrRunNotFound` if the ID does not exist. `DeleteRun` accepts any run regardless of current status in v0.1 — restricting deletion of in-progress runs (e.g., requiring cancellation first) is deferred to RFC 0003 when the Scheduler owns the execution lifecycle. This method is included to ensure the `Store` interface is complete for v0.2 SQLite migration (which will need deletion for retention/cleanup policies) without requiring a breaking interface change. In v0.1, it also enables test cleanup. **Known risk for RFC 0002:** since DeleteRun has no status restriction, a running workflow's state could be deleted mid-execution via the REST API. RFC 0002's delete endpoint should refuse deletion of `Running`-status runs at the API layer even though the store permits it.
- **Concurrent DeleteRun + UpdateStepState interaction**: If a run is deleted while another goroutine is updating its steps, `UpdateStepState` will return `ErrRunNotFound` after acquiring the lock and finding the run absent. The `sync.RWMutex` serialization handles this naturally — no additional coordination is needed. This is documented for implementer awareness since it is the most likely real-world race (REST API delete concurrent with Scheduler step updates).
...
- `Register` rejects duplicate IDs (returns `ErrAgentAlreadyRegistered`). **Amended 2026-08-23 ([ISSUE-0125](../issues/ISSUE-0125-agents-never-reregister-after-orchestrator-restart.md)): `Register` is now an upsert and `ErrAgentAlreadyRegistered` no longer exists.** The two-step `Unregister`-then-`Register` dance below is exactly the friction anticipated here — "an atomic `RegisterOrUpdate` method may be needed when agents reconnect" — and ISSUE-0125 found it blocking fleet recovery after an orchestrator restart, so the upsert landed on `Register` itself rather than as a second method. The existing `Registry` interface takes `AgentInfo` by value (not pointer) — this is intentional as a defensive copy on register. The `InMemoryRegistry` stores a pointer to an internal copy of the received value (`&AgentInfo{...}` populated from the value fields), matching the deep-copy rationale applied to `GetRun`/`ListRuns` in Phase 1. Implementers should not change the interface to accept a pointer. Re-registration after agent restart requires calling `Unregister` first, then `Register` with the new address. This two-step sequence is non-atomic — a concurrent caller could observe the agent as absent between `Unregister` and `Register`. For v0.1 in-memory usage this is acceptable, but an atomic `RegisterOrUpdate` method may be needed when agents reconnect over gRPC (RFC 0003). An `Update` method may be added later if idempotent re-registration becomes a common pattern.
...
- Unmarshal into a `WorkflowFile` wrapper type first, since workflow YAML files (e.g. `feature-builder.yaml`) have a top-level `schema_version:` field alongside a `workflow:` object:
  ...
  > **YAML float coercion note:** An unquoted `schema_version: 0.1` in YAML would be parsed as float `0.1`, not string `"0.1"`. However, since the `WorkflowFile.SchemaVersion` field is typed as `string`, `gopkg.in/yaml.v3` coerces the float value to the string `"0.1"` during unmarshalling — so this works correctly despite the YAML type mismatch. The existing fixture correctly quotes the value (`"0.1"`). This is documented here because unquoted numeric schema versions are a common YAML gotcha and implementers should be aware that the coercion is handled by the library, not by explicit conversion.
...
- Validate workflow ID format: reuse the agent ID pattern `^[a-z0-9][a-z0-9-]*[a-z0-9]$`. Workflow IDs appear in state store keys, REST API URLs (RFC 0002), and log entries — malformed IDs could cause routing or injection issues downstream. The existing fixture (`feature-builder`) matches this pattern.
- **Unknown YAML keys**: Unknown keys (e.g., `approval_required`, `approval_timeout`, or typos like `depeends_on`) are silently ignored by `yaml.v3`'s default unmarshalling. This is intentional for forward compatibility — fields deferred to later RFCs (such as approval gates) already exist in `workflow.schema.json` and must not cause parse failures. JSON Schema validation via `make validate` is the primary defense against typos and malformed keys. The planner does NOT use `yaml.Decoder.KnownFields(true)` strict mode. This design choice means a typo in a step-level key (e.g., `depeends_on` instead of `depends_on`) would be silently discarded, potentially causing the step to run too early. CI-enforced schema validation is the mitigation.
- Validate `output_key` format (if present) using the step ID regex pattern: `^[a-z0-9]([a-z0-9_-]*[a-z0-9])?$`. While `output_key` is not consumed by `ResolveInputs` (which looks up by step ID), the Scheduler (RFC 0003) will use it for storing execution results. Malformed values discovered at that point would be harder to diagnose. Optionally validate uniqueness of `output_key` across steps to prevent silent overwrites when the Scheduler stores outputs.
...
- **Defensive node-count check**: `Plan()` MUST verify after Kahn's algorithm completes that `len(emitted) == len(workflow.Steps)` and return an error if not. This catches cycles that slip past a missing `ValidateDAG` call, turning Kahn's implicit detection from silent data loss into an explicit error.
...
- Provide a `ResolveInputs(step Step, outputs map[string]string, vars map[string]string, logger *zap.Logger) (string, error)` package-level function that the Scheduler/Executor will call at runtime to substitute actual values.
...
- Use `regexp` for pattern matching — no Jinja2 engine in Go. Support only the two patterns above for v0.1. Recommended combined regex: `\{\{\s*(steps\.([a-z0-9](?:[a-z0-9_-]*[a-z0-9])?)\.output|([a-z_][a-z0-9_]*))\s*\}\}`. Group 2 captures the step ID for output references; group 3 captures plain variable names. The step ID sub-pattern must be consistent with Phase 3a's step ID format validation regex. Specifying the pattern explicitly prevents implementation inconsistency when different regex choices produce subtly different behavior with nested braces or edge-case inputs. Implementation MUST define the step ID pattern as a single `const` (e.g., `const stepIDPattern = `[a-z0-9]([a-z0-9_-]*[a-z0-9])?``) and reference it from both the Parse validation regex and the `ResolveInputs` template capture group. This prevents the two regexes from diverging over time.
- **Variable name charset**: The variable name pattern `[a-z_][a-z0-9_]*` intentionally restricts variables to lowercase letters, digits, and underscores. This is a deliberate design choice — forms like `{{ UserRequest }}` or `{{ USER_REQUEST }}` are not supported. The existing fixture uses `{{ user_request }}`, and the `WorkflowRun.Inputs` map keys follow the same convention. This restriction simplifies parsing and ensures consistency with the project's lowercase-identifier conventions (agent IDs, step IDs).
...
- **Interface placement**: `ResolveInputs` is an exported package-level function in `internal/planner/`, not a method on `*YAMLPlanner` or the `Planner` interface. It is pure computation (regex substitution + map lookups) that does not depend on any `YAMLPlanner` state — making it a method would force the Scheduler/Executor (RFC 0003) to depend on the concrete `*YAMLPlanner` type rather than the `Planner` interface, defeating the interface abstraction. The logger is passed as a parameter for the suspicious-pattern warnings described above. This avoids premature interface expansion and unnecessary concrete-type coupling.
...
> **Note:** The `logger` parameter MUST be non-nil — passing `nil` will cause a nil pointer panic on suspicious-pattern warnings. Test callers should use `zap.NewNop()` as the standard no-op logger. This is the established testing convention across the project (both `NewInMemoryStore` and `NewInMemoryRegistry` follow the same pattern).
...
Since the components are not consumed by any caller in v0.1, the initialized variables would trigger Go's `declared and not used` compiler error. Assign them to a `components` struct or package-level variables that are logged at startup (e.g., `logger.Info("components initialized", zap.String("store", "InMemoryStore"), ...)`), ensuring the compiler accepts the code and the initialization is observable in logs.
...
| `registry` | ~~`ErrAgentAlreadyRegistered`~~ | ~~`Register`~~ | **Removed 2026-08-23** — `Register` is an upsert ([ISSUE-0125](../issues/ISSUE-0125-agents-never-reregister-after-orchestrator-restart.md)) |
...
- **State store tests**: CRUD operations, concurrent read/write with goroutines, status transitions, `UpdateRunStatus` on nonexistent run ID (returns error), `DeleteRun` on existing and nonexistent IDs, `ListRuns` deep copy verification (modifying a returned run must not affect store state), `UpdateStepState` adding a new step ID not pre-populated in the run's `Steps` map (should succeed per Phase 1 design — verifies the Scheduler can initialize step state on first execution), concurrent `UpdateStepState` on the same run with different step IDs (simulates the most common real-world concurrency pattern: parallel stage execution where the Scheduler updates multiple steps simultaneously), `CreateRun` with empty `Steps` map vs. `nil` `Steps` map vs. pre-populated `Steps` (verifies `UpdateStepState` works correctly regardless of initial `Steps` state — `CreateRun` should initialize an empty map if `Steps` is nil to prevent nil-map panics on subsequent writes).
