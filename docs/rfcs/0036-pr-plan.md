...
- **Epoch binding** — `RecallParams.EpochID` defaults to [`DefaultEpochID`](../../internal/channels/sqlite.go#L42) (`"live"`). PR 3 threads the caller's epoch exactly as the publish handler does, in two hops: [`resolveEpochOverride`](../../internal/server/channel_epoch_override.go#L49) returns a `context.Context` carrying the override (via [`WithEpochOverride`](../../internal/channels/epoch_override.go#L39)) — **not** an epoch string — and [`EpochOverrideFromContext`](../../internal/channels/epoch_override.go#L48) then reads it back to populate `RecallParams.EpochID`, so recall and publish agree on the epoch axis. `messages.epoch_id` exists from migration v6.
