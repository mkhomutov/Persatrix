...
Three host files sit **at or one line under** the [`file_size.py --strict`](../../.github/copilot-instructions.md) 500-line cap, so new code is routed around them:
...
| [`internal/channels/channels.go`](../../internal/channels/channels.go) | `RecallParams` struct + `RecallMessages` on the `ChannelStore` interface (the struct keeps the interface line short under the cap). |
...
- **Epoch binding** — `RecallParams.EpochID` defaults to [`DefaultEpochID`](../../internal/channels/sqlite.go#L42) (`"live"`); PR 3 resolves the caller's epoch into it via the existing [`resolveEpochOverride`](../../internal/server/channel_epoch_override.go#L49) / [`EpochOverrideFromContext`](../../internal/channels/epoch_override.go#L48) path (the same mechanism the publish handler uses), so recall and publish agree on the epoch axis. `messages.epoch_id` exists from migration v6.
...
|---|-------|-------|--------|--------|-----------|--------|
| 1 | 1 | Migration v10 — `messages_fts` + triggers + backfill | `feature/v039-rfc0036-fts-migration` | ⬜ Not started | — | — |
