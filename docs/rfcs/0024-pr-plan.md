...
**Prerequisites**:
- [RFC 0023](0023-llm-call-leasing.md) (LLM Call Leasing) — shipped in v0.3.2; PR 1 lights `wake.kind` up as a lease-request attribute on the existing `LeaseRequest` surface.
- [RFC 0011](0011-channels-bridges.md) (Channels & Bridges) — shipped in v0.3.0; PR 5 reshapes the channel-message dispatch path it introduced.
...
**Recommended merge order**: **PR 1 → PR 2 → PR 3a → PR 3b → PR 4 → PR 5 → PR 6 → PR 7**. The order tracks the RFC's phase boundaries: PR 1 (Phase 1) is the structural foundation — every later PR builds on `EventLoop` and `SyncDispatchHandle`. PRs 3a/3b (Phase 3) are reviewable in isolation only after PR 1's wake taxonomy exists. PR 4 (Phase 4) requires PR 1's `event_loop.enqueue` API. PR 5 (Phase 4 channel dispatch) is the closing implementation PR and carries the cost-regression CI gate as its acceptance, not a follow-up.
...
- **`wake.kind` lights up as a lease-request attribute.** PR 1 plumbs the wake variant (`inbound`, `scheduled`, `salience`, `dropped`) through to the existing [RFC 0023](0023-llm-call-leasing.md) `LeaseRequest.wake_kind` field that v0.3.2 PR 5 reserved for this hand-off (today every leased call passes `"tick"`). The legacy timer synthesises `wake.kind = "scheduled"` with `timer_id = "legacy_tick"`, so the v0.3.2 cost-attribution dashboards keep working unchanged; new wake variants light up as their phases land.
- **Backpressure is discard, not block** per [RFC §Decided §1](0024-event-driven-scheduling.md#decided-backpressure). PR 1's tests pin the `agent.wake.dropped` increment shape; an integration test follows in PR 5 once the channel-message origin is the dominant producer.
...
- [ ] [RFC 0023](0023-llm-call-leasing.md) `wake.kind` lease attribute lights up: legacy adapter emits `wake.kind = "scheduled"` with `timer_id = "legacy_tick"`; `test_action_loop_tick_lease.py` is amended to assert the new attribute is propagated to the lease request.
...
- **No new metrics here.** The wake counters (`agent.wake.salience`, `agent.wake.dropped`) land in PR 3b alongside the producer. Adding them in 3a would pin a counter on writes that never produce wakes, which the cost-regression CI gate (PR 5) would then have to special-case.
