...
- **Go** — an import-graph deny rule (e.g. [`depguard`](https://github.com/OpenPeeDeeB/depguard) or a `go list`-based check in CI) forbidding the MIT-candidate Go packages from importing non-extractable `internal/*`. Go deny rules match whole import paths (per package, not per file), so seed the rule only on packages that are *already* leaf: `internal/wallet` and the wallet proto package `internal/generated/walletpb` — **not** the shared `internal/generated`, which also holds the BUSL-only `logpb`/`taskpb`.
...
**`internal/cost` joins the Go gate only after it is split.** It is a single Go package today, and one of its files reaches up: `context_package_metrics.go` imports `internal/executor/packaging` to surface RFC 0008 packaging metrics through the cost endpoint. Because the deny rule matches whole packages, `internal/cost` cannot be added to the gate while keeping it green until its embeddable engine is separated from that orchestrator-coupled metrics path — a seam-cut deferred to [RFC 0023](0023-llm-call-leasing.md)'s extraction RFC and carried in its dependency-direction proof ([§H](#h-the-candidate-set-and-the-per-extraction-rfc-requirement)). So at acceptance the gate is seeded on the already-leaf packages above, and `internal/cost` is enforced once the split lands. Seeding it this way moves no code now — it scopes the contract to what is actually leaf on `main`.

**Update ([RFC 0046](0046-budget-lease-extraction.md)).** The flagship extraction resolved its MIT artifact to a single-language **Python** library, keeping the Go `internal/cost`/`internal/wallet` BUSL as the reference server ([RFC 0046 §D](0046-budget-lease-extraction.md#d-what-stays-busl-and-the-proto-as-the-shared-contract)). The `internal/cost` split is therefore **not** a prerequisite for the budget extraction, and its MIT↛BUSL boundary is enforced by the Python `import-linter` contract rather than the Go deny rule. The Go deny-rule seeding above is retained as forward-looking hygiene for any future MIT Go code.
...
|----------------|-------------------|------|-----|----------------|
| **budget-lease** (flagship) | A single-language **Python** library — pure-Python in-process budget engine (default) + remote-gRPC backend + ADK/LiteLLM adapters — plus the published `proto/wallet.proto` contract. The Go `internal/cost` + `internal/wallet` **stay BUSL** as the reference server; Persatrix agents dogfood the library in remote mode. (Composition resolved in [RFC 0046](0046-budget-lease-extraction.md) §A/§D, refining the polyglot framing this row first assumed.) | MIT | Universal, uncrowded, differentiated cost *gate* ([RFC 0023](0023-llm-call-leasing.md)). Idle-loop ([RFC 0024](0024-event-driven-scheduling.md)) kept internal — decided in [RFC 0046 §H](0046-budget-lease-extraction.md#h-event-loop-placement-resolution). | [RFC 0046](0046-budget-lease-extraction.md) |
...
2. **Enforcement landed.** The dependency-direction CI check ([§B](#b-the-dependency-direction-invariant)) merges green, seeded on the packages that are *already* leaf on the current tree, and the reserved-seams note is recorded. (`internal/cost` joins the gate later, once the flagship extraction splits its embeddable engine from the orchestrator-coupled metrics path — [§B](#b-the-dependency-direction-invariant).) *Gate for any extraction:* nothing moves until this gate exists and passes.
