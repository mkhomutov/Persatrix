...
- **`RegisterAgent` is sketched as *unary*, which is a transport swap and nothing more — see [ISSUE-0146](../issues/ISSUE-0146-registration-not-a-property-of-a-live-connection.md).** One call, once, from the same agent startup path, over a typed transport instead of REST. That leaves the registry's remaining defect exactly where it is: it is in-memory, an orchestrator restart empties it, and re-registration is something the *agent* does when its channel drops — the orchestrator never observes on its own whether a registered agent is still there, and nothing drives `Registry.UpdateStatus`. Making registration a property of a **live connection** — connection-scoped or streaming, so a broken stream is both the re-register trigger and the liveness signal — would close it here, and is the amendment ISSUE-0146 asks this section for (§Open Questions 6). The shared precondition that used to sit here — `InMemoryRegistry.Register` was not an upsert, so a re-register was a no-op against a populated registry — was **removed 2026-08-23** with ISSUE-0125's shape (4); `Register` upserts, and the agent re-registers on its own when the orchestrator channel reconnects. That is the interim, not the destination: the connection-scoped shape this section is asked to describe is still open, and since ISSUE-0125 resolved on the interim (2026-09-03) it is tracked as ISSUE-0146.
...
6. **Should `RegisterAgent` be connection-scoped or streaming rather than unary?** §C sketches it unary, which migrates the transport but keeps registration a one-shot at agent startup — leaving the liveness half of [ISSUE-0125](../issues/ISSUE-0125-agents-never-reregister-after-orchestrator-restart.md) (resolved 2026-09-03 on the interim reconnect watcher, shape (4)) open through Phase 2 — tracked as [ISSUE-0146](../issues/ISSUE-0146-registration-not-a-property-of-a-live-connection.md). A stream makes the connection itself the registration, so a break is both the re-register trigger and the liveness signal — one mechanism for "who is here" and "is it still here", which also gives the undriven `Registry.UpdateStatus` something to drive. Cost: it is the one non-unary RPC on the service, and it forfeits the "unary RPCs inherit the middleware for free" property in §C. Resolve before the proto lands in Phase 2 — the shape is non-additive once it ships. `Register` has been an upsert since ISSUE-0125 shape (4) landed (2026-08-23); the remaining question is the stream.
