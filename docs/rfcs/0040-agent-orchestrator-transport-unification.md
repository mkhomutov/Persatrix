...
- **`RegisterAgent` is sketched as *unary*, which is a transport swap and nothing more — see [ISSUE-0125](../issues/ISSUE-0125-agents-never-reregister-after-orchestrator-restart.md).** One call, once, from the same agent startup path, over a typed transport instead of REST. That leaves the registry's standing defect exactly where it is: it is in-memory, an orchestrator restart empties it, and nothing re-registers, so the fleet stays silently unreachable until every agent process is restarted by hand. Making registration a property of a **live connection** — connection-scoped or streaming, so a broken stream is both the re-register trigger and the liveness signal — would close it here, and is the amendment ISSUE-0125 asks this section for (§Open Questions 6). The shared precondition that used to sit here — `InMemoryRegistry.Register` was not an upsert, so a re-register was a no-op against a populated registry — was **removed 2026-08-23** with ISSUE-0125's shape (4); `Register` upserts, and the agent re-registers on its own when the orchestrator channel reconnects. That is the interim, not the destination: the connection-scoped shape this section is asked to describe is still open.
...
6. **Should `RegisterAgent` be connection-scoped or streaming rather than unary?** §C sketches it unary, which migrates the transport but keeps registration a one-shot at agent startup — leaving [ISSUE-0125](../issues/ISSUE-0125-agents-never-reregister-after-orchestrator-restart.md) (an orchestrator restart empties the in-memory registry and nothing re-registers) open through Phase 2. A stream makes the connection itself the registration, so a break is both the re-register trigger and the liveness signal — one mechanism for "who is here" and "is it still here", which also gives the undriven `Registry.UpdateStatus` something to drive. Cost: it is the one non-unary RPC on the service, and it forfeits the "unary RPCs inherit the middleware for free" property in §C. Resolve before the proto lands in Phase 2 — the shape is non-additive once it ships. Independently of the answer, make `Register` an upsert; today it `409`s.
