...
- **[OQ #5](0023-llm-call-leasing.md#open-questions) — tokeniser parity.** **Resolved: promote `tiktoken` to a hard runtime dependency.** It moves out of the `accurate-tokens` optional extra into the runtime `dependencies` list in `agents/pyproject.toml`; `LLMClient` reuses the existing `cl100k_base` `_count_tokens` helper for `estimated_input_tokens`, and `LeaseRequest.estimated_input_tokens` carries that tiktoken count.
