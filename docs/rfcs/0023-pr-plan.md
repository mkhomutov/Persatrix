...
_Deferred to this PR from the second PR 2 review (a third item from that review — the `int32` narrowing of `ttl_seconds` — was pulled forward and closed within PR 2; see the fourth PR 2 review paragraph above): (1) **addressed in this PR** — the `finalize` log messages are no longer assembled by string concatenation; each of the four (`"wallet: lease finalized"`, `"wallet: finalize rejected — unknown lease"`, `"wallet: finalize is a no-op — lease already settled"`, `"wallet: finalize reconcile miss — provisional already cleared"`) is now a constant, with the `settle` / `release` discriminator riding on a `zap.String("op", op)` field — log aggregators can group on the message text again. New `wallet_finalize_log_test.go` pins the constant-message + op-field shape on all four branches (success, unknown-lease, already-settled no-op, reconcile-miss); (2) **left as-is by design** — the wallet's lease-lifecycle defaults are duplicated between the JSON-schema `default:` values and the Go constants in `internal/wallet/config.go`. They agree today and `make validate` plus `LoadConfig` agree on the bounds at both ends; no mechanism is warranted to pin the defaults themselves, only to be aware of the drift risk if either side is edited in isolation. One related code-clarity nit needs no separate tracking — in `cmd/orchestrator/main.go` `walletSvc` is constructed whenever the cost config loaded but only registered and given a running reaper inside the nested `grpcServer != nil` → `logBuf != nil` block, so a log-buffer init failure builds a wallet that is never served; the load-bearing half of this (wallet enforcement silently disabled when the log buffer fails) is already tracked as a PR 3 implementation detail, and the remainder is a one-line comment best folded into that PR 3 change._
...
| 7 | — | Review follow-ups | `feature/v032-rfc0023-followups` | 🔄 In progress | — | — |
