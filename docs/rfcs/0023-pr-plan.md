...
- **The scheduler's post-hoc `recordStepUsage` double-counts workflow-task spend once the wallet records it.** `internal/scheduler/budget.go`'s `recordStepUsage` writes a completed workflow step's actual token usage to the shared `TokenCounter` via `RecordUsage`. This PR brackets the workflow-task LLM call in `wallet.lease(...)`, and the wallet already writes that same spend to that counter — `RecordProvisional` at acquire, `Reconcile` to the provider actuals at settle. Left as-is, both paths count the workflow-task spend, inflating every budget scope ~2×. PR 3 retires the `RecordUsage` call from `recordStepUsage`; the wallet's `Reconcile` becomes the single recording authority for leased calls. The sibling `costReporter.RecordStepCost` call is independent — it feeds the per-step `/cost` endpoint, not the budget counter — and stays. Surfaced by the PR 2 review as a forward-looking note.
...
_Deferred to this PR from the second PR 2 review: (1) the `int32` narrowing of `ttl_seconds` in the lease grant — re-examined from the round-one note, the conversion is exact (TTL is whole seconds) but not non-overflowing, since `schemas/optimization.schema.json` sets `minimum: 1` but no `maximum`, so an absurd operator value above `2^31−1` wraps the advertised `TtlSeconds`; close it with a `maximum` bound on the three `wallet` keys (`ttl_seconds`, `reaper_interval_seconds`, `max_active_leases`), which also catches other fat-finger configs at `make validate`; (2) the `finalize` log messages are assembled by string concatenation (`"wallet: " + op + " rejected …"`) — move the `settle` / `release` discriminator into a `zap.String("op", op)` field so the messages are constants log aggregators can group on; (3) the wallet's lease-lifecycle defaults are duplicated between the JSON-schema `default:` values and the Go constants in `internal/wallet/config.go` — they agree today but nothing keeps them in sync (drift risk noted; no mechanism warranted). One related code-clarity nit needs no separate tracking — in `cmd/orchestrator/main.go` `walletSvc` is constructed whenever the cost config loaded but only registered and given a running reaper inside the nested `grpcServer != nil` → `logBuf != nil` block, so a log-buffer init failure builds a wallet that is never served; the load-bearing half of this (wallet enforcement silently disabled when the log buffer fails) is already tracked as a PR 3 implementation detail, and the remainder is a one-line comment best folded into that PR 3 change._
