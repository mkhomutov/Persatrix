...
Retraction policy: **symmetric latest-asserted-wins, per-session**. Within a single `(agent_id, subject, predicate, session_id)` key, only one row stays live and the row with the greatest `asserted_at` wins. On every write:

* Existing live rows with `asserted_at <= new.asserted_at` in the **same session or the `legacy` carve-out** are marked superseded by the new row.
* If a strictly-newer live row already exists *in the same session*, the new row is itself marked superseded by it — an out-of-order older write does not leave two live rows.
...
The `session_id` predicate is the **RFC 0031 Phase 2 PR 5 §F amendment** (2026-05-29) — supersede was originally keyed on `(agent_id, subject, predicate)` only, which left F-3 only partially closed on the facts surface: a write in `run-b` could retroactively mark a `run-a` row as `superseded_by`, removing it from `run-a`'s view (the read-side §D filter would then never surface either row in `run-a`). Per-session supersede aligns the write predicate with [RFC 0031 §D](0031-per-session-namespacing-channels.md#d-recall-semantics)'s read predicate so each named session keeps its own truth about `(subject, predicate)`. The `legacy` carve-out is folded in **asymmetrically**, matching §D's `IN (active, legacy)` read filter: the older-row sweep spans `(session_id, legacy)` so an active-session write **absorbs an older `legacy` predecessor** (the upgrade hot-path — a pre-RFC fact reasserted under a pinned session would otherwise leave both the legacy row and the reassertion live and contradictory in the active session's recall); but the newer-dominator check stays exact, so a `legacy` write never supersedes a named session's row ("not vice versa"). Because `superseded_by` is a single global pointer, an active session absorbing a `legacy` row also drops it from other sessions' carve-out view — the accepted latest-wins consequence on the shared pre-RFC baseline; isolation between two *named* sessions is unaffected. Tracked as [ISSUE-0079](../issues/ISSUE-0079-cross-session-supersede-not-scoped.md); closed by PR 5.
...
> **Amendment (v0.3.14 PR 3 — [ISSUE-0081](../issues/ISSUE-0081-session-id-process-global-not-task-local.md) erasure residual).** The traversal is **tenant-scoped**: both DELETEs carry `AND principal_id = ?` resolved from the active principal, so a caller erases exactly the rows it could read — and the returned counts are scoped with them, since a foreign row in the `records_deleted` tally discloses that another tenant holds facts about the subject. The `session_id` and `epoch_id` axes are deliberately **not** scoped: erasure must reach every row the principal ever wrote, including earlier rooms and earlier runs. Because the principal predicate has no `"*"` sentinel by design ([RFC 0031 §C/§D](0031-per-session-namespacing-channels.md)), "erase this subject across all tenants" is unspellable — an operator-initiated `SubjectErasure` has no principal of its own, so RFC 0013 must decide whether the right to be forgotten spans tenants before it implements: [ISSUE-0127](../issues/ISSUE-0127-cross-principal-erasure-verb.md).
