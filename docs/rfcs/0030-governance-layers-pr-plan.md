...
| `proto/*` (channel event) | Add `string interaction_id = 12` to `ChannelMessageEvent` (next free field after `cascade_depth=11`), typed scalar — same rationale as cascade_depth ([§M](0030-multi-agent-conversation-governance.md#m-wire-and-config-surfaces)). Regenerate Go + Python stubs. |
...
| File | Change |
|------|--------|
| `proto/wallet.proto` | Extend `AcquireLeaseRequest` with `string interaction_id = 8` and `int64 interaction_budget_tokens = 9` ([§E](0030-multi-agent-conversation-governance.md#e-layer-1--per-conversation-cost-ceiling)); add `INTERACTION_BUDGET_EXHAUSTED` to the `LeaseDenied` reason enum. Regenerate stubs. |
