...
> **Producer follow-on (2026-06-10).** This plan ships the layers *ahead of the `interaction_id` producer* — every "inert until the producer" note below is intentional. The producer (the orchestrator-side resolver that mints/rotates the id per RFC 0020 §G scope, plus the agent-side `END_INTERACTION_VOTE` implementation and lease threading) is planned separately in the [interaction-id producer PR plan](0030-interaction-id-producer-pr-plan.md), which also discharges the two hardening constraints PRs 3/4 left as preconditions.
