...
> **Second-pass deep review (PR 4a, in-PR fixes).** A follow-up review hardened PR 4a itself, no behavioural changes (still dark): (a) corrected two inaccurate `synthesis_reserve.go` doc-comments — the half-cap clamp's rationale understated its own risk (a realistic full-roster/modest-cap config triggers it too, not only a tiny/degenerate cap, and when it bites the CLOSE path is the one left under-funded, not the discussion), and the chair-synthesis-turn sizing wrongly cited the Layer-0 depth cap as bounding a single call's *token cost* (Layer-0 bounds recursion *depth*, not per-call size); (b) added `TestSynthesisReserveTokens_ClampCanUnderfundClose`, pinning (not fixing) that the clamp can under-fund the close under a realistic config — see the new residual item below; (c) added a Go↔Python cross-language drift pin (`tests/unit/python/test_cross_language_synthesis_reserve_drift.py`) for `DefaultSynthesisCallReserveTokens` against the RFC 0020 summary's own bounds, following the established drift-pin pattern — nothing previously caught a Python-side bump silently under-sizing the Go reserve; (d) added the missing store-canonical (revision > 0) **chair**-drift lock test (`TestChannelConfig_AutonomousChairDriftAtRevisionLocksThenRecovers`) — the chair half of the item below that only the convener side pinned — which surfaced a genuine asymmetry: unlike the convener, disarming ALONE does not recover a drifted chair (`validateEscalationChair` has no `effectiveEnabled()` short-circuit — the chair is a general RFC 0050 knob, not autonomous-scoped), so recovery needs `escalation_chair_id` cleared/re-pointed in the same edit; (e) added the missing observer-chair baseline-drop test (`TestChannelConfig_AutonomousFirstEditDropsObserverChair`), the observer branch the non-member-chair test did not cover; (f) fixed the RFC 0052 §B canonical example YAML, which had drifted invalid under this PR's own mandatory-chair gate (no `escalation_chair_id`) and separately mis-nested `interaction_budget_tokens` under `autonomous` (both are top-level sibling knobs, not autonomous sub-knobs). The residual item below is the one not fixed in this pass.

> **Third-pass deep review (PR 4a, in-PR follow-ups).** A further review applied five findings — still dark, one observability-only log line, the rest doc/tracking: (a) promoted the **chair-synthesis-turn under-sizing** to a first-class KNOWN GAP #2 in `synthesis_reserve.go` and the residual item below — distinct from and *additive to* the clamp gap, it can deny the chair turn itself even when the clamp never bites, and is the more likely §D failure of the two; (b) flagged in `EvictInteraction`'s doc that PR 4a ships **no settle barrier** for its own async, cross-process close-summary leases, with the tracked item below; (c) added the residual item to confirm the `1 + N` reserve model against the **distributed** per-agent close (the shared-`interaction_id` assumption); (d) added the residual item for the **chairless-armed store-canonical upgrade lockout** across the #714 → #715 dev line; (e) made the first-edit baseline's silent **disarm** operator-visible — `autonomousBaseline` now `Warn`s when it drops an armed block, since disarming a channel is a bigger blast radius than dropping a bare chair / reasoning knob. No accounting or validation behaviour changed.
...
- **Resolved (PR 4a).** ~~[PR 4] Require a chair (or a designated synthesizer) on an armed channel.~~ PR 1 validated `convener != escalation_chair_id` but did **not** require a chair to exist — an armed channel with no `escalation_chair_id` validated, making the distinctness rule vacuous in that case. RFC [§D](0052-autonomous-agent-channels.md#d-termination-and-synthesis--always-produce-an-artifact) "always produce an artifact" is the **chair's** synthesis-on-close job. PR 4a adds the `validate` gate (`ErrAutonomousChairRequired`, load path + REST apply path) that an `autonomous.enabled` channel declares the role that authors the mandatory synthesis turn — convener/chair distinctness is no longer vacuous.
...
- **[PR 4b] Confirm the `1 + N` reserve model against the *distributed* per-agent close.** The soft-budget → trigger-close → draw-reserve accounting assumes a **single** close moment where `1 + N` calls fire against **one** shared `interactionTokens[interaction_id]` entry. But close is distributed: each agent's runtime detects close (end-vote / idle) and summarizes in its own process, at its own time. The reserve only governs those N summaries if they all draw a lease under the **same governance `interaction_id`** in the wallet — an assumption PR 4a relies on but does not establish. PR 4b must confirm the shared-interaction-id wiring (else the per-interaction reserve / soft-budget governs nothing at close) and reconcile the coordinated-close accounting with the independent per-agent close timing.
- **[PR 4b / RFC 0050 hardening] A chairless-armed channel made store-canonical *before* the mandatory-chair gate is locked out of config edits.** PR 3 lets an operator arm a channel with a cap + convener but **no chair**; persisted at revision > 0 it is store-canonical, and after PR 4a its stored blob (armed, no chair) makes `validateAutonomous` reject `ErrAutonomousChairRequired`, so **every** subsequent config edit `400`s until the operator disarms — the same shape as the revision>0 *membership-drift* lockout the tests pin, but reached by a **validation-rule tightening over already-stored state**, not by drift. Reachable only across the #714 → #715 dev line (RFC 0052 is unreleased) and recoverable by disarming in the same PATCH (`autonomous.enabled=false` short-circuits `validateAutonomous`), so it is not tested here; PR 4b/5 should add the migration note to the release checklist or one-time-normalize such stored blobs when the gate lands.
...
| 4b-i | 1d — deterministic bounded-close trigger (max_rounds + soft-budget) + router→wallet spend read + artifact-bearing close teardown | `feature/v0311-rfc0052-bounded-close` | 🔀 PR open |
| 4b-ii | 1d — goal-directed chair synthesis turn (close-on-reply ordering) + OQ #6 close-summary metering | `feature/v0311-rfc0052-synthesis-turn` | ⬜ |
