...
- [ ] Standing leg asserts the wallet `interactionTokens` map is bounded *within a process* by the aggregate bound (≈`max_convenings` entries); per-convening eviction is **not** yet wired (tracked residual — `EvictInteraction` needs its settle barrier), so the map is a bounded leak, not flat.
