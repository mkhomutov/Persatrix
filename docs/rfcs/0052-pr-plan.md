...
> **Second-pass deep review (PR 1, in-PR fixes).** Hardened two gaps *inside* PR 1: (a) **group-only arming** — the apply path rejects an `autonomous.enabled` DM/thread (`ErrAutonomousNotGroup`), which validation previously accepted despite being un-convenable-by-construction; and (b) the **first-edit baseline drop** now keys on "armed AND un-convenable", so an armed rung with no convener (not only a drifted one) is dropped rather than freezing a block that would 400 an unrelated edit.
...
> **Phase-1 close-out deep review (post-PR 5, 2026-07-07).** Two §D artifact-safety fixes shipped with regression tests — convene now **pre-flights the escalation chair** (drift → `ErrAutonomousChairUnavailable`/400) and the **floor-path bounded close re-checks the terminating verdict *after* the floor-queue park** so a parked round no longer dispatches into a discussion it just closed — plus a doc-truth pass (bundled example ships disarmed; reserve/eviction/cost-close comments match shipped state). See the CHANGELOG entry. **Newly tracked residuals, later PRs:** reserve-**preservation** (a discussion-cause soft-cap in `AcquireLease`; the reserve is only a router-side trigger point today); post-close disarm dropping the no-reopen latch; the panic-in-teardown liveness wedge; Layer-1 cost-close metering; the operator-directive wrap at the close-summarizer seam; the idle-rotation double `interaction_closed` count.
...
- **Resolved (PR 4a).** ~~[PR 4] Require a chair (or a designated synthesizer) on an armed channel.~~ PR 1's `convener != escalation_chair_id` rule was vacuous with no chair; PR 4a adds the mandatory-chair `validate` gate (`ErrAutonomousChairRequired`, load + REST apply) so an armed channel declares the role that authors the §D synthesis turn.
...
- **Resolved (PR 4b-ii, PR #718 review).** ~~Redelivery ingest-skip gated on `bounded`~~ (`close_notification.py` co-gates `redelivery and bounded`); ~~duplicated operator-directive framing / member-lookup loops / the `synthesisTurnClosedOnTimout` typo~~ (shared `operator_directive.py`; a `memberByID` helper single-sources the by-id lookup; const renamed `…Timeout`).
...
| 7b-i | 3 — convening counter + aggregate `max_convenings` enforcement (429) | `feature/v0311-rfc0052-convening-counter` | 🔀 PR open |
| 7b-ii+ | 3 — timer-wiring seam + `standing_budget_tokens` + CLI/web readout + MT | `feature/v0311-rfc0052-standing` | ⬜ |
