...
- **[PR 7b-ii+ / future §E hardening] `max_convenings` is per-process, so a restart refills the aggregate budget.** *(PR 7b-i deep-review residual; see the CHANGELOG.)* The count ([`convening_counter.go`](../../internal/channels/convening_counter.go)) is in-memory and resets on restart, so it bounds the recurring total per-process — not *across the standing window* §E's safety framing targets ([§E](0052-autonomous-agent-channels.md#e-standing-and-scheduled-discussions); MT-AUTONOMOUS-003). A long-lived standing channel outlives many restarts, each refilling the budget. **Latent today** (convene manual-only), **load-bearing once the timer fires unattended**. A durable count needs persistence, which RFC 0052 rules out ("no new store migration") — a follow-up, like the in-memory `standing_budget_tokens` ceiling.
