...
> **Second-pass deep review (PR 4a, in-PR fixes).** Hardened PR 4a itself, no behavioural change (still dark): (a) corrected two inaccurate `synthesis_reserve.go` doc-comments — the half-cap clamp's rationale understated its risk (a realistic full-roster/modest-cap config triggers it, and it under-funds the CLOSE path, not the discussion), and the chair-turn sizing wrongly cited the Layer-0 depth cap as bounding a single call's token *cost* (it bounds recursion *depth*); (b) added `TestSynthesisReserveTokens_ClampCanUnderfundClose`, pinning (not fixing) the clamp under-funding the close under a realistic config; (c) added a Go↔Python drift pin (`test_cross_language_synthesis_reserve_drift.py`) for `DefaultSynthesisCallReserveTokens`; (d) added the store-canonical (revision > 0) chair-drift lock test (`TestChannelConfig_AutonomousChairDriftAtRevisionLocksThenRecovers`), surfacing that — unlike the convener — disarming alone does not recover a drifted chair (`validateEscalationChair` has no `effectiveEnabled()` short-circuit), so recovery needs `escalation_chair_id` cleared/re-pointed in the same edit; (e) added the observer-chair baseline-drop test (`TestChannelConfig_AutonomousFirstEditDropsObserverChair`); (f) fixed the RFC 0052 §B example YAML (invalid under the mandatory-chair gate, and mis-nesting `interaction_budget_tokens` under `autonomous`). The residual item below is the one not fixed in this pass.

> **Third-pass deep review (PR 4a, in-PR follow-ups).** Applied five findings — one observability log line, the rest doc/tracking: (a) promoted the **chair-synthesis-turn under-sizing** to a first-class KNOWN GAP #2 in `synthesis_reserve.go` and the residual below (distinct from and additive to the clamp gap; the more likely §D failure — it can deny the chair turn even when the clamp never bites); (b) flagged in `EvictInteraction`'s doc that PR 4a ships **no settle barrier** for its async cross-process close-summary leases; (c) added the residual confirming the `1 + N` reserve model against the **distributed** per-agent close (shared-`interaction_id`); (d) added the residual for the **chairless-armed store-canonical upgrade lockout** (#714 → #715); (e) made the first-edit baseline's silent **disarm** operator-visible (`autonomousBaseline` now `Warn`s when it drops an armed block). No accounting or validation behaviour changed.
...
- **[PR 5 / defense-in-depth] Gate the redelivery ingest-skip on `bounded`** *(PR #718 review)*. `close_notification.py` skips the closing-turn ingest on `close_notification_redelivery is True` independently of the `bounded` flag, so a spoofed `redelivery=True` on a sole-delivery close drops the turn. Unreachable from the Go orchestrator (producer-hardening, not a live bug); one-line fix `redelivery and bounded`.
- **Resolved (PR 4b-ii, PR #718 review).** ~~[cleanup] Duplicated operator-directive framing + hand-rolled member-lookup loops + the `synthesisTurnClosedOnTimout` typo.~~ A shared `operator_directive.py` now backs `convener.py`/`synthesis_turn.py`; a `memberByID` helper (`router_autonomous.go`) single-sources the by-id lookup in chair-escalation (×2), config-apply, convene-classify, and synthesis-close; const renamed `…Timeout`.
