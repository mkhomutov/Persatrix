...
- [x] RFC 0033 status = `⚠️ Partially Implemented (Phases 1–2)` (frontmatter `status: partially_implemented` + header + regenerated [INDEX.md](INDEX.md) via `make rfcs`).
- [x] [ROADMAP RFC Master Index](../../ROADMAP.md#rfc-master-index) updated; merged-PR history backfilled (#421–#437, includes RFC 0033 PRs 1–7).
- [x] [v0.3.4-plan Master Progress Overview](../v0.3.4-plan.md#master-progress-overview) row 2 → ✅ Merged (and the stale row 1 PR-plan row corrected to ✅ Merged [#430](https://github.com/mkhomutov/Persatrix/pull/430)).
- [x] Doc-only change (5 doc files; no code / config / schema touched), so `make test` / `make lint` are unaffected and not re-run (mirrors the PR 6 disposition). Verified: `make validate` ✓, `make rfcs-check` ✓, `make issues-check` ✓, and the doc-integrity suite (`doc_links.py` 5293 links / 287 files, `doc_status_markers.py`, `doc_audit.py` 0 violations, `file_size.py --strict`) all clean.
- [x] [Progress Overview](#progress-overview) row 7 filled.
...
- **Phase 3 — ✅ shipped in v0.3.5**: removed the raw-ID fall-through in `resolve()`; deleted `_infer_provider` and the `provider_inference` config block; bumped `schema_version` to `"0.3"`; raw vendor IDs in `agents.yaml` are now rejected with a clear `SystemExit` at resolve ([RFC §Phase 3](0033-model-alias-layer.md#phase-3--pass-through-removal--_infer_provider-retirement--v035)). The gate (`persatrix.llm.alias.raw_id_usage{agent_id}` reading zero across dogfood — observed traffic, not a calendar date) was satisfied, and the maintainer opted Phase 3 in as a conditional co-resident of v0.3.5 ([v0.3.5 plan §Conditional co-resident](../v0.3.5-plan.md#conditional-co-resident--rfc-0033-phase-3)).

Phase 3 did not get its own multi-PR plan file: it landed directly as two slices under v0.3.5 — deliverable 1 ([#481](https://github.com/mkhomutov/Persatrix/pull/481)) and the deliverable 2–3 follow-up (this PR). See the RFC's [§Implemented in v0.3.5](0033-model-alias-layer.md#implemented-in-v035-phase-3) for the full record.
...
| 7 | — | Phases-1–2 closeout | `feature/v034-rfc0033-close` | ✅ Merged | [#437](https://github.com/mkhomutov/Persatrix/pull/437) | 2026-05-26 |
