...
| Other `create_provider` call sites | Update the remaining caller(s) to the tuple shape ([RFC §D](0033-model-alias-layer.md#d-factory-integration) names ~3 sites: persona runtime, sub-agent path, summarization path). The summarization path consumes `context_management.summarization.model` — it resolves the same way once PR 3 points that field at the `summarizer` alias; this PR keeps it working on the raw value via the pass-through. |
