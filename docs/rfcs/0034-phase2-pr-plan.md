...
| `docs/manual-tests/MT-PERSONA-CONVERSATION-002.md` | **New.** (`-002` is the next free id after the shipped `MT-PERSONA-CONVERSATION-001.md`.) Group-channel continuity MT: three personas + a user on one group channel; a turn that only resolves if the persona saw a *named* peer's prior contribution. Records the realism outcome the master plan's combined MT execution re-runs live. |
