...
**Status**: 🚧 Implementing — Tier A (addressing-aware eligibility) + the `respond_policy → disposition` reframe **shipped in v0.3.7** (the directed-elsewhere filter in [`agents/response_gate.py`](../../agents/response_gate.py) + Go candidate-set parity + the `participant`/`addressed`/`observer` vocabulary with back-compat; acceptance [MT-CHANNEL-RELEVANCE-001](../manual-tests/MT-CHANNEL-RELEVANCE-001.md)). **Tier B shipped in v0.3.8** — the cheap leased `fast`-model salience bid ([`agents/salience_bid.py`](../../agents/salience_bid.py)), the now-live per-disposition `threshold` (bias-to-silence default), the **`chair`** low-threshold facilitator, natural-language addressing as a bias signal, and the `salience_max_channel_members` cap; acceptance [MT-CHANNEL-RELEVANCE-002](../manual-tests/MT-CHANNEL-RELEVANCE-002.md). The `chair` ships as a **facilitator only** — its moderator half (Layer 5: reading the transcript to wrap up / terminate) remains **v0.4.0**, present but inert ([`agents/chair_moderation.py`](../../agents/chair_moderation.py) — a typed attach point no runtime path calls). The two version-split scoping questions are resolved in the [v0.3.7 plan §Open-question status](../v0.3.7-plan.md#open-question-status); the remaining open questions below are version-routed (v0.4.0), not blocking.
...
- [Scope — v0.3.x vs v0.4.0](#scope--v037--v038--v040)
...
> **Conformance note (floor-capable-directedness amendment, 2026-06-10).** The Tier A row above defines directedness as a mention of a *different* agent, but the v0.3.7 implementation tests only "`mentions` non-empty" — so a mention of a party that cannot take the floor (the human operator, who joins as `respond: never` by documented convention; an `observer`; a non-member) suppresses every `participant` and silences the room. The [floor-capable-directedness amendment](0030-amendment-floor-capable-directedness.md) restores the definition above: suppression requires a **floor-capable** addressee (a member, other than the sender, whose normalized policy ≠ `never`), resolved orchestrator-side and carried on the wire.
