...
> **Conformance note (floor-capable-directedness amendment, 2026-06-10).** The Tier A row above defines directedness as a mention of a *different agent*, but the v0.3.7 implementation tests only "`mentions` non-empty" — so a mention of a party that cannot take the floor (the human operator, who joins as `respond: never` by documented convention; an `observer`; a non-member) suppresses every `participant` and silences the room. The [floor-capable-directedness amendment](0030-amendment-floor-capable-directedness.md) restores the definition above: suppression requires a **floor-capable** addressee (a member whose normalized policy ≠ `never`), resolved orchestrator-side and carried on the wire.
