...
The convention is deliberately shallow: for four of the six scopes it does not migrate storage, change schemas, or alter access patterns — it adds a uniform vocabulary that removes a class of design decisions. **Two exceptions**: the `channel:` and `session:` scopes each need a small additive sidecar table (`channel_state`, `session_state`; see [§E](#e-mapping-to-existing-stores) and [OQ #1](#open-questions)), so the "no schema changes" promise is qualified — it holds for `persona:` / `interaction:` / `temp:` / `app:`, and `channel:` / `session:` require two pure-additive migrations with no data movement.
...
- [RFC 0023](0023-llm-call-leasing.md) wallet spend is keyed by fixed `global` / `per_workflow` / `per_agent` budget scopes (the `per_agent` scope is persona-granular), plus an [RFC 0030](0030-multi-agent-conversation-governance.md) per-interaction budget ceiling — server-side in `internal/wallet/`, not by an ad-hoc per-session composite.
...
4. **Minimal storage change.** Existing tables stay where they are for `persona:` / `interaction:` / `temp:` / `app:`; the prefix is a *naming* and *routing* layer over the current stores. The two exceptions are the `channel:` and `session:` scopes, which each require one additive sidecar table (`channel_state`, `session_state`) — pure additions with no data movement.
...
- **Storage-layout redesign.** [RFC 0029](0029-personal-society-storage-split.md) owns the storage layout; this RFC layers naming on top (with the two additive sidecar tables — `channel_state`, `session_state` — noted in Goal 4).
...
| `session:` | One session = room continuity `(agent, channel)` ([RFC 0031](0031-per-session-namespacing-channels.md) / [memory-scope-axes.md](../memory-scope-axes.md)) | Persistent in the `session_state` table; **accumulates** across the session, archived not cleared | `session:S-abc:active_personas` |
...
| `session:` | `session_state` table (new additive sidecar in `channels.db`, symmetric with `channel_state`) | Explicit delete only — session state accumulates and is archived, not cleared at "session end" ([memory-scope-axes.md](../memory-scope-axes.md): a room-continuity session resets only when a new channel is used); archived-session rows persist (no deletion path, unlike `channel:`). Run/test isolation is the separate `epoch` axis. |
...
| `session:` | RFC 0031 provides a `session_id` *tagging/filter* dimension plus a `sessions` registry table (`internal/channels/session_registry.go`, `session_binding.go`), **not** a general key-value store | New additive `session_state(session_id, key, value)` sidecar table in `channels.db` — the second pure-additive migration, symmetric with `channel_state` (same opaque-KV shape). It is **FK-less** (the synthetic `legacy` session has no `sessions` row) and orchestrator-side, keyed by the resolved `session_id`; not "the existing namespace." No data moved. |
...
No data is moved for any scope. Two scopes add a new pure-additive sidecar table — `channel:` (`channel_state`) and `session:` (`session_state`); both are `CREATE TABLE`-only, with no existing row touched. Where "existing store" is a typed facade (`persona:`), a new sidecar table (`channel:` / `session:`), or an in-memory tier (`interaction:` / `temp:`) rather than a ready-made KV surface, the key-value adapter is new code — the wrapper is still the seam, but §E is honest that only `app:` is a pure read-through rename; the other five scopes each need a thin new KV surface (two of them backed by the additive tables above).
...
| Go orchestrator | `internal/channels/` (channel-state + session-state stores — new `channel_state` and `session_state` sidecar tables via new sibling migration files + store accessors; **not** `internal/server/channel*.go`, which is the REST handler surface / Phase-2 publish call site); RFC 0031 session namespacing lives in `internal/channels/session_registry.go` + `session_binding.go` and `internal/server/session_handlers.go` | Two additive sidecar tables (`channel_state`, `session_state`) + accessors. In Phase 1 the Go side stores opaque blobs (no scope-prefix parsing). |
...
| Storage | `internal/channels/sqlite_migrations.go` (+`sqlite_schema.go`, + new sibling migration files) | **Two** additive migrations: `channel_state` (v10→v11) and `session_state` (v11→v12) tables. All other scopes: no schema change. |
...
- **Manual tests**: [MT-MEMORY-005](../manual-tests/MT-MEMORY-005-dementia-test.md) re-runs unchanged (byte-identical no-regression guard — it validates recall quality, which this RFC does not change). Optionally add an `MT-STATE-00x` acceptance overlay for the security-sensitive path (two-persona `persona:` isolation; `temp:` vanishing at `Control(turn_completed)`; `session:` accumulation) — a candidate deliverable for the [PR plan](0042-pr-plan.md).

## Open Questions

1. ~~`channel:` storage layout — KV column or sidecar table?~~
   **Resolved**: sidecar `channel_state(channel_id, key, value)` table — the next `channelStoreSchemaVersion` after v10, a pure-additive migration with no data movement (precedent: v6/v8/v9). This is **one of two** carve-outs from the "no schema changes" guarantee — the symmetric `session_state` table for the `session:` scope (see [§E](#e-mapping-to-existing-stores)) is the other; Summary / Goal 4 / Non-Goals / Files Touched reflect both.
...
- **True dependency distance.** Only **Phase 1b** (`StateDelta` emission, [§D](#d-the-scopedstate-api)) is gated on RFC 0041 — and RFC 0041 is currently only **Proposed**, so "lands" is two status transitions away (Accepted → a landed Phase 1). **Phase 1a** (the enum, API, wrappers, `channel_state` + `session_state` tables, secret-lint, and tests) carries **no** RFC 0041 dependency and can be scheduled independently.
...
1. Author ratifies the design decisions applied in this revision (sidecar `channel_state` **and** the symmetric sidecar `session_state`; `session:` = room-continuity; `epoch`/`principal` as envelope, not scopes; wallet not `session:`-scoped; `agents/scoped_state.py` naming).
2. Resolve OQ #6 (envelope wiring) and settle the OQ #5 mechanism (dispatcher middleware vs. per-subscriber).
3. ✅ **Authored in this revision** — the Phase-1 PR plan is drafted at [`0042-pr-plan.md`](0042-pr-plan.md), mirroring [`0041-pr-plan.md`](0041-pr-plan.md): six test-first slices — `Scope` enum + parser (PR 1) → `ScopedState` Protocol + `temp:` backend (PR 2) → `channel_state` + `session_state` migrations (PR 3) → per-scope wrappers + routing tests (PR 4) → secret-lint (PR 5) → (Phase 1b) `StateDelta` emission (PR 6); Phase-2 call-site migration and Phase-3 RFC 0037 overlay verification are scoped out of that plan. At Accepted, schedule it into the v0.4.0 plan.
