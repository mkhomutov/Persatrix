...
> **Correction (2026-09-22), recorded not amended.** "Provably cannot reach a prompt context" held only for the operator route. Since RFC 0049 PR 4 ([#784](https://github.com/mkhomutov/Persatrix/pull/784)) the persona's default facts recall passes `"*"`, and every widened row passes the RFC 0037 §D gate before the prompt ([RFC §Security Considerations](0031-per-session-namespacing-channels.md#security-considerations)). The same claim appears in Prerequisites item 1 ("library/test-only") and in PR 5's ISSUE-0086 scope row ("provably unreachable from a prompt context").
...
**Resolved (maintainer, 2026-05-31):** the **epoch** axis — the actual home of test-run isolation now — ships as its own v0.3.5 phase, **Phase 3b**, *with* an operator surface (a `--epoch` override + `PERSATRIX_EPOCH` documentation), and the `make reset` operator-guide framing is redirected at epoch rather than `session new --activate`. Phase 3 (this plan) deliberately does not cover epoch; the dedicated [`0031-epoch-pr-plan.md`](0031-epoch-pr-plan.md) sequences it (leaf → migration → filter+wiring → rail → operator surface → closeout), and the [v0.3.5 plan §Phase 3b](../v0.3.5-plan.md#phase-3b--rfc-0031-epoch-axis-issue-0085) gates the release on it. The remaining facts-by-subject follow-up ([ISSUE-0084](../issues/ISSUE-0084-fact-scope-by-subject-not-uniform-session.md)) stays deferred beyond v0.3.5; full reframing record in [Memory Scope Axes §Consequences](../memory-scope-axes.md#consequences-for-the-current-code-and-rfc-0031).
