...
- **Phase 1**: unit tests pinning the warning fires (config load with each deprecated spelling; REST paths), and — equally — that `addressed`/`observer`/`always` **and the omitted-`respond` defaults** (string-shorthand member, empty REST field) do **not** warn. No drift-pin changes.
...
3. Phase 2 timing: gated on Phase 1 warn telemetry showing the spellings are gone from real configs, with v0.4.0 as the earliest candidate — never the same release as Phase 1. Known blind spot: log-only warns (OQ 2) under-count REST-originated declarations — an operator driving the API from automation never reads orchestrator logs — so "gone from real configs" should weigh the YAML evidence over REST silence, or OQ 2's `Deprecation` header becomes the measurement fix rather than a courtesy.
