...
4. **PR D — Tester identity override (§E).** Client-only: "Acting as (local testing)" override defaulting to the `/ui/context` principal, annotating each turn, **inert/hidden when `authenticated: true`**. No backend change. **Gated on Open Question #5 sign-off.**
...
4. **§D timeline order** — conversational (oldest-top, chosen) vs. log-tail (newest-top, as shipped). The RFC frames the panel as "watch personas interact" (conversational); chosen the reversal, **with the pinned-scroll guard** so autoscroll never steals a reading operator's scroll position. Confirm the reversal + guard are wanted over the current log-tail order.
5. **§E tester identity override** — should the console expose a local "Acting as" override at all? **Chosen: yes** — local-mode only, defaults to the `/ui/context` principal, annotated as testing, **inert once `authenticated: true`**. This is the one decision that bends [RFC 0048 §F rule 1](0048-operator-tester-web-console.md#f-auth--multi-tenancy-forward-compatibility); it needs explicit sign-off before PR D because it documents a carve-out, not just an implementation. The alternative (keep identity frozen) keeps §F rule 1 literal but leaves Goal 4's persistence demo undemonstrable from the browser.
6. **§F cross-panel link** — acceptable to deep-link the chat DM into the timeline (chosen yes), or keep the panels siloed for Slice 1? Chosen the link (it is the cheapest way to make the combined persistence story visible); confirm.
