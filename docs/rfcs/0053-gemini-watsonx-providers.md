...
3. **Priced by default** — both ship demo alias configs with explicit per-token pricing, so the missing-price guard (RFC 0033 PR 4) and the derived Go cost table (RFC 0033 §F) keep the RFC 0023 budget gate live. No silent $0.
...
Gemini and watsonx.ai are **non-local** providers, so the [RFC 0033 missing-price guard](0033-model-alias-layer.md#f-pricing-keyed-by-alias) fails closed on any unpriced alias (an unpriced non-local model would read $0 in the derived Go cost table and silently disable the RFC 0023 budget gate). Every demo alias entry therefore carries explicit `input_per_1m_tokens` / `output_per_1m_tokens` set to the vendor's current published price. The numbers are a config value the operator keeps current, not a code constant — the RFC fixes the *requirement*, not a frozen rate. The autonomous-channel mandatory cost cap (RFC 0052 Goal #4) sits on top of this, so a four-vendor unattended brainstorm is double-bounded (priced lease + per-interaction cap).

### E. Demo + compose parity

`make demo-gemini` / `make demo-watsonx` mount `config/demo/gemini/optimization.yaml` / `config/demo/watsonx/optimization.yaml` (priced aliases pointing the society at the new provider), exactly like `demo-openai`. Matching `docker-compose.gemini.yaml` / `docker-compose.watsonx.yaml` pass the provider env (`GEMINI_API_KEY`; `WATSONX_API_KEY` + `WATSONX_PROJECT_ID` + `WATSONX_URL`). The **four-vendor** autonomous brainstorm demo is a separate, RFC 0052-owned target that references all four cloud alias configs at once.

## Security Considerations

- **New credential surfaces.** Two more API-key env vars (`GEMINI_API_KEY`, `WATSONX_API_KEY`) and watsonx's `project_id`/`url`. Keys flow only through the existing env → provider path and must be covered by the RFC 0009 secret redactor's patterns (add Google/IBM key shapes to the redactor allow-list so they never reach logs).
...
The provider-side enablement for the cross-vendor brainstorm: a society blueprint can pin distinct personas to `anthropic` / `openai` / `gemini` / `watsonx` aliases in one channel. The autonomous demo target + MT that *exercise* this live in [RFC 0052 Phase 4](0052-autonomous-agent-channels.md#phase-4-flagship-demo).
