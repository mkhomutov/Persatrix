...
> **Live RFC table:** [INDEX.md](INDEX.md) — auto-generated by `make rfcs`
> from per-RFC YAML front-matter. Read it first to see what exists, what
> ships in which version, and what is in flight.
...
Every RFC carries two parallel pieces of metadata:

1. **YAML front-matter** at the top of the file (machine-readable, the
   source of truth for [INDEX.md](INDEX.md)).
2. A **bold-markdown header** below it (human-readable; this is what
   GitHub renders inline). Keep the two in sync.

### YAML front-matter (machine-readable)

`scripts/rfcs.py` parses this block to regenerate [INDEX.md](INDEX.md).

```yaml
---
id: RFC-NNNN
title: Short title
summary: One-line description for grep / future INDEX summary column.
type: feature           # feature | architecture | protocol | process
status: proposed        # see lifecycle table; lower-case + underscores
author: Maksim Khomutov
created: 2026-04-08     # YYYY-MM-DD (validated)
target: v0.2
depends_on:             # zero or more, documentary only
  - RFC-0001
superseded_by: RFC-NNNN # omit if none
---
```

Allowed `status` values (lower-cased; the table marker is rendered from
[the lifecycle table](#rfc-lifecycle)): `draft`, `proposed`, `accepted`,
`implementing`, `implemented`, `partially_implemented`, `rejected`,
`deferred`, `stable`, `superseded`.

### Bold-markdown header (human-readable)
...
- [ ] YAML front-matter block at top of file is present and includes all required fields (`id`, `title`, `summary`, `type`, `status`, `author`, `created`, `target`).
- [ ] Bold-markdown header below the YAML matches the front-matter (Type / Status / Author / Date / Target).
- [ ] All required sections for the declared type are present.
- [ ] Table of Contents matches actual headings.
- [ ] Status is set to `proposed` / `📋 Proposed` (or `draft` / `🔨 Draft` if incomplete).
...
- [ ] `make rfcs` has been run and [INDEX.md](INDEX.md) regenerated.
...
- [ ] `make rfcs` re-run after any front-matter change.
