...
- **Phase 2 (CI gating) is out of v0.3.11 scope.** Phase 1 ships a runner whose report a human reads; a failed eval does not block merge until Phase 2 wires `.github/workflows/eval.yml`.
  - *2026-09-06 — recommendation for v0.3.16: **take it, not cut.*** Phase 2 is slotted cuttable in the 2026-08-19 amendment. The replay is $0 and deterministic, and every release now spends a paid live arc on legs the previous release already proved; recording a golden at release-prep PR 1 and replaying it in CI turns that spend into a standing regression gate. Not pulled forward into v0.3.15 (version-train gate) — recorded here so the v0.3.16 plan opens with the call made. A `Docs hygiene`-style job running `make eval-replay` on the five seeds is the whole Phase 2 PR; the per-recipe overlay note below is the one prerequisite for replaying a *live*-recorded golden.
  - *2026-09-12 — **landed** as v0.3.16 [PR C2](../v0.3.16-pr-plan.md#pr-c2--featurev0316-rfc0044-eval-gate)*: a step in the required `Python` job rather than a new job (the C1 shape — a new job is advisory until the owner's branch-protection change). The runner gained `--tier`, the Makefile `TIER=`, and the six seeds moved to `stable`; the promotion bar is in the [guide](../evaluators-guide.md#promoting-a-recipe-to-stable).
