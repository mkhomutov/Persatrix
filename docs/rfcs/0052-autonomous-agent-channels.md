...
#### Amendment 2026-08-31 — the reserve scales with close RECORDS, not the roster (v0.3.15, ISSUE-0123 / ISSUE-0131)

The sizing rule above stands; what it counts changed. §D(2) sized the reserve for `1 + N` — one summary per participating persona — on the premise that a close produces one RFC 0020 record per persona. The v0.3.15 [RFC 0020 §G amendment](0020-interaction-lifecycle.md#amendment-2026-08-26--the-record-key-gains-a-principal-and-a-speaker-dimension-v0315-issue-0123--issue-0131) re-keyed the interaction record to `(principal_id, speaker_id, scope)`, so one persona now holds one record per `(principal, speaker)` pair it observed and the room-wide fan closes — and meters — all of them.

The reserve is therefore **record-scaled `1 + R`** ([`wallet.CloseRecordUpperBound`](../../internal/wallet/synthesis_reserve.go): `personas × principals × speakers`, bounded from the room). The soft threshold reads the same `R`, so the "threshold basis must never be smaller than the reserve basis" requirement holds by construction rather than by the member-count-exceeds-roster accident it used to rely on.

The speaker axis is **not** bounded by the roster, and that is a review finding rather than a detail. The record key's speaker half is the ingested event's `sender_id`, and the two forced-turn directives this RFC defines — [§B](#b-self-convening--starting-without-a-human-turn)'s convene turn and [§D](#d-termination-and-synthesis--always-produce-an-artifact)'s synthesis turn — ride *synthetic* senders (`orchestrator:convene`, `orchestrator:synthesis`) that are deliberately not valid participant ids and hold no seat. Each opens a close record of its own on the persona it was directed at, which the room-wide fan then closes and meters. So the bound's speaker term is `channelSize + 2` (`wallet.ControlSenderSpeakers`, pinned against the enumerated senders by `TestOrchestratorDispatchSenders_MatchTheReserveSpeakerAllowance`). Without it a three-seat room — one authenticated person plus two personas, which sits *exactly* at the partition maximum and therefore has zero slack — is bounded below its true close-record count, and the shortfall is the silent kind.

Two consequences are stated rather than fixed. `R` grows with the **cube** of the room (the partition argument divides the naive product by about four — a constant factor, not a degree), so the half-cap clamp — which protects the discussion and does not scale with `R` — now bites in ordinary configs, the bundled four-seat `autonomous-multivendor` blueprint at its own shipped 200k cap included; the effect is bounded (a clamped reserve is half the cap, so the discussion gets half its budget and the close fires earlier, never later) but it leaves the close under-funded, which degrades silently into the janitor's placeholder. So the re-size ships the clamp's signal with it — `channel.conversation.synthesis_reserve_clamped{channel_type, trigger}` per close, plus a Warn line once per channel per configuration — and defers the constants to [ISSUE-0138](../issues/ISSUE-0138-close-reserve-multiplier-calibration.md) in the [ISSUE-0109](../issues/ISSUE-0109-rfc0052-autonomous-defaults-calibration.md) idiom. The §D(2) alternative — scoping the metering and the reserve to a single designated close-summarizer — is now one of that issue's live options, at the cost of the per-record attribution v0.3.15 exists to provide.
