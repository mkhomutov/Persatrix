...
| [`cli/src/commands/chat.rs`](../../cli/src/commands/chat.rs) | Rename `chat_resp.session_id` accesses (lines 170–171, 91). The local `session_id: String` variable (line 57) stays unprefixed for the same reason as the Python servicer. |
| [`cli/src/main.rs`](../../cli/src/main.rs) | Rename `session_id` field on the request-builder code path (line 124). |
...
- [ ] Grep audit: every `session_id` in code post-rename either belongs to RFC 0031's operator-namespace (introduced in PR 2 / PR 3) or is a function-local variable inside a chat-handler scope.
...
| `internal/server/main.go` (boot site) | `os.Getenv("PERSATRIX_SESSION_ID")` at boot; if empty, log `INFO "PERSATRIX_SESSION_ID unset, defaulting to 'legacy'"` and use `"legacy"`. |
...
| `internal/observability/metrics.go` | New counter `sessions.writes` with `session_id` attribute. Low-cardinality only — Phase 1 inserts the env-var value verbatim, so cardinality is bounded by operator-controlled session count. |
