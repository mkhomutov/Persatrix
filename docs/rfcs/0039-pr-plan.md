...
Review follow-up filed: [ISSUE-0117](../issues/ISSUE-0117-agent-ingress-close-knob.md) — an `auth.agent_ingress: open|closed` knob so a fleet-less deployment can close the carve-out without waiting for RFC 0009 Phase 4 agent tokens (RFC 0009 track).
...
**PR = #794.** As-implemented notes: **(1)** the enabled-mode posture landed as a NEW dedicated operator guide, [`docs/guides/auth.md`](../guides/auth.md) — `web-console.md` sat at 2998/3000 of the documentation word cap, with no room for the amendment's browser posture; the console guide's §Security flips compactly and points into the new guide (section renamed to *"Security — exposure beyond localhost"*; the four docs referencing the old anchor were re-pointed). **(2)** `MT-AUTH-001` is **authored** here with live execution slotted at v0.3.12 release-prep (the MT-AUTONOMOUS-00x / MT-MEMORY-CROSSROOM-001 status-line precedent — the master plan's [Acceptance](../v0.3.12-plan.md#acceptance-for-v0312) owns live runs), not run inside this PR; the arc was smoke-verified informally during PR 4/5 development. **(3)** The three #793 review items are carried: the **HTTPS-or-login-loops** `__Host-`/`Secure` cookie footgun (auth guide [§HTTPS](../guides/auth.md#https-is-required-beyond-localhost) + a console troubleshooting row + MT edge case 1); the **RFC 0037 interaction** — anonymous channel reads, including classified channels, stay open under `enabled` + non-loopback via the agent-ingress carve-out (auth guide [§what stays open](../guides/auth.md#what-stays-open-under-enabled--the-agent-ingress) + SECURITY.md + MT Leg 2, with the [ISSUE-0117](../issues/ISSUE-0117-agent-ingress-close-knob.md) close knob cross-referenced); **operators also 403** on cross-user chat-history reads (auth guide §claim + MT Leg 3). Also: the [sessions guide](../guides/sessions.md) account ≠ (login) session ≠ participant ≠ (memory) session disambiguation; README + SECURITY.md posture updates; the stale MT-CONSOLE-001 exposure row refreshed; the v0.3.12-plan progress rows 1–2 de-staled to their merged closeouts. No CHANGELOG entry (docs-only closeout; #785/#789 precedent — the PR 3/4/5 entries carry the story).
