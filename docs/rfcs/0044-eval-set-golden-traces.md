...
2. **How are LLM-replay responses recorded?** Probably as a structured cassette per-eval: `{request_hash: response_payload}`. Hashing the request needs a stable canonicalization that ignores volatile fields (timestamps, idempotency keys).
