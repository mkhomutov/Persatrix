...
> **2026-09-12 — landed** as v0.3.16 [PR C2](../v0.3.16-pr-plan.md#pr-c2--featurev0316-rfc0044-eval-gate): a step in the required `Python (lint + test)` job runs `make eval-replay TIER=stable` — no separate `eval.yml`, because a new job is advisory until the owner changes branch protection while a step in a required job gates from merge. The runner gained `--tier`; an empty tier exits non-zero, since a gate over no recipes is vacuous rather than green. The six committed seeds moved to `stable` (§F's initial classification). The `experimental` post-merge run and the nightly drift run are not built: an `experimental` recipe is reported by the developer's sweep and gates nothing.
