...
> **2026-09-12 — landed** as v0.3.16 [PR C2](../v0.3.16-pr-plan.md#pr-c2--featurev0316-rfc0044-eval-gate): a step in the required `Python (lint + test)` job runs `make eval-replay TIER=stable` — no separate `eval.yml`, because a new job is advisory until the owner changes branch protection while a step in a required job gates from merge. The runner gained `--tier`; any run that replays nothing exits non-zero (an empty tier, a `--target` that matches nothing), since a run over no recipes is vacuous rather than green, and a stable recipe that fails to load or has no golden is a failed recipe *in the report* rather than a traceback. The six committed seeds moved to `stable` (§F's initial classification). Still unmet from §F: the `experimental` post-merge run and the nightly drift run are not built — no CI run replays an `experimental` recipe, so it is reported only by a developer's local sweep and gates nothing — and the structured artifact (bullet 4) is not attached to the CI run: the step passes no `REPORT=`, so the per-assertion JSON exists only when a developer asks for it locally.
...
| CI | `.github/workflows/ci.yml` — a step in the `python` job, not the separate `eval.yml` first estimated | CI gate (Phase 2, landed v0.3.16 PR C2) |
...
4. **What is the `stable` tier admission criterion?** ✅ Resolved (Phase 2, v0.3.16 PR C2). Initial reading: re-verified across two independent runs and marked stable by the maintainer. The bar as written in the [author guide](../evaluators-guide.md#promoting-a-recipe-to-stable): an offline-recorded golden, replayed green on `main` after the recording commit, and the `tier` flip landing in its own reviewed PR.
5. **Eval scenarios involving wall-clock time.** ✅ Resolved (PR 3). The `elapsed` field is simulated time, not real time. The seam exists ([RFC 0021](0021-persona-temporal-awareness.md) §Phase 1): the persona reads "now" only through an injectable `Clock` (`agents/clock.py`), so the runner constructs the persona agent around a `FrozenClock` and calls `clock.advance(parse_elapsed(elapsed))` before each interaction — the persona's temporal awareness sees the delta deterministically, and the frozen anchor is also what keeps a time-anchored prompt stable enough to replay byte-for-byte.
...
The blocking open questions are resolved in the PR plan: **OQ #1** (goldens) → sidecar `<id>.golden.yaml`; **OQ #2** (replay cassette) → the replay-client PR (**landed** — canonicalization spelled out above); **OQ #5** (`elapsed`) → the runner PR (**landed** — the `FrozenClock` seam wired above). PR 1 landed the deterministic core — the eval-set format (`schemas/eval_set.schema.json`), the [§B](#b-assertion-vocabulary) assertion engine, and `load_eval_set` / `evaluate` — built test-first, with no dependency on the unlanded RFC 0041 taxonomy. PR 2 landed the replay client ([`evaluators/replay_llm_client.py`](../../evaluators/replay_llm_client.py)): the recorded-response `LLMProvider` + its record half, the mock-as-LLM that makes replay CI-safe. PR 3 landed the runner ([`evaluators/runner.py`](../../evaluators/runner.py) + [`persona_driver.py`](../../evaluators/persona_driver.py) + [`report.py`](../../evaluators/report.py)): recipe → real-persona-runtime drive → `evaluate` → structured artifact, the `make eval-replay` / `eval-record` / `eval-drift` targets, and the [author guide](../evaluators-guide.md); record→replay symmetry is proven end-to-end **through** the runtime, not just at the provider. PR 4a landed the **first pre-0041 seed** — `EVAL-MEMORY-001`, the dementia test ([MT-MEMORY-005](../manual-tests/MT-MEMORY-005-dementia-test.md)) as a five-interaction recall recipe with an offline-recorded golden that replays green against the pre-0041 surface (assertions restricted to `final_transcript` / `terminal_state`); it also fixed the driver to force an isolated `:memory:` DB so the golden is portable and no eval touches production memory. The event-asserting seeds (`EVAL-ERROR-001`/`002`) and the remaining recipes follow in PR 4b (gated on RFC 0041 typed events). **Phase 2 landed** as v0.3.16 PR C2 — `make eval-replay TIER=stable` inside the required Python job, the six seeds promoted to `stable`, the admission bar (OQ #4) written in the guide.
