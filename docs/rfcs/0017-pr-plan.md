...
- `_MEMORY_BUDGET_TOKENS = 1500` is the committed Phase 1 default ([OQ1](0017-persona-memory-injection-budget.md#open-questions)). Acceptance does not gate retuning — any retune is a one-line constant change. *(Since v0.3.16 PR K1 an operator retunes it with `memory_budget.tokens` in `config/optimization.yaml`; the constant is the default when the key is absent.)*
