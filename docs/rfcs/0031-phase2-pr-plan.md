...
- **`"*"` is a footgun** ([RFC §Security Considerations](0031-per-session-namespacing-channels.md#security-considerations)): a debug path that defaults to all-sessions and is wired into a prompt context re-introduces F-3. PR 4's `test_session_recall_default_path.py` pins that the persona-runtime context path never reaches `"*"`. The `"*"` sentinel is CLI/debug-only (Phase 3 surfaces it via `persatrix memory recall --all-sessions`).
  > **Correction (2026-09-22), recorded not amended.** Since RFC 0049 PR 4 ([#784](https://github.com/mkhomutov/Persatrix/pull/784)) the persona's default facts recall passes `"*"`, and every widened row passes the RFC 0037 §D gate before the prompt, so the pin is now "no ungated widening" ([RFC §Security Considerations](0031-per-session-namespacing-channels.md#security-considerations)). The `--all-sessions` verb was never built ([ISSUE-0086](../issues/ISSUE-0086-operator-all-sessions-recall-verb.md)). The same old pin appears in PR 4's two test rows and Tests bullet above and in the Risks table below. The live pins are [`test_cross_session_read_sites.py`](../../tests/unit/python/test_cross_session_read_sites.py) and [`test_prompt_path_sessions.py`](../../tests/integration/test_prompt_path_sessions.py).
...
- **PR 451 deep-review carry-forwards** (Facade read-path extension + call-site threading):
  - **M1 (resolved in PR 451 follow-up commit) — Default-session-resolution lives in two places.** `MemoryStore.retrieve_relevant` originally resolved `sessions=None` to `[self._session_id]` (facade snapshot) while `channel_history.recall_channel_episodes` passed `sessions=None` straight to the tier. Resolved by dropping the facade-side pre-resolve so the tier's `_resolve_session_list` is the single source of truth; `retrieve_relevant` now mirrors `channel_history` pass-through semantics. Pinned by `test_session_recall_default_path.py::TestFacadeRetrieveRelevantSessionForwarding::test_default_passes_none_through_to_tier`.
  - **M1-pool (resolved in PR 451 follow-up commit) — `pool.read(sessions=None)` footgun.** After the ISSUE-0078 M2 fix moved the cross-session policy to the data layer, an explicit `sessions=None` still propagated to `EpisodicMemory.recall` and narrowed to `[_active_session_id, "legacy"]` — the same silent-narrowing hazard ISSUE-0078 was filed against, gated behind explicit-`None` rather than default-omit. Resolved by normalising `None` → `"*"` at `SharedMemoryPool.read` entry so the data-layer policy applies uniformly. Pinned by `TestPoolReadTierDefaultIsCrossSession::test_explicit_none_is_cross_session` (tier) and `TestFacadeReadFromPoolSessionForwarding::test_explicit_none_is_cross_session` (facade).
  - **L1 (resolved in PR 451 follow-up commit) — Source-level pin scope.** The `PERSONA_RUNTIME_RECALL_MODULES` list covered the persona-runtime modules but not `agents/base.py` (the task-agent `retrieve_relevant` call). Renamed to `PROMPT_CONTEXT_RECALL_MODULES` (back-compat alias kept) and extended to include `agents/base.py`.
  - **L5 — Runtime "never sees star" pin is synthetic.** `test_session_recall_default_path.py::TestPersonaRuntimeCallSitesDoNotPassAllSentinel::test_episodic_recall_default_path_never_sees_star` calls `EpisodicMemory.recall` directly with kwargs that *resemble* what `_inject_memory_context` passes, rather than invoking `_inject_memory_context` itself. A future edit to the prompt-assembly pipeline that wires `sessions="*"` into the mixin won't break this test if it bypasses the spied recall. Fold into PR 5: with the dementia-test bridge already wiring the full persona pipeline, add a spy on `EpisodicMemory.recall` and drive a real `_inject_memory_context` call so the assertion is against the actual call site, not a stand-in. The source-level scan above remains as the cheap defence. **Resolved by [#974](https://github.com/mkhomutov/Persatrix/pull/974).** PR 5 ([#452](https://github.com/mkhomutov/Persatrix/pull/452)) added `TestInjectMemoryContextDefaultPath`, but it replayed the recall calls instead of driving the mixin. The test now lives in [`test_prompt_path_sessions.py`](../../tests/integration/test_prompt_path_sessions.py) and drives a real turn under every pairing of the two `cross_room` knobs, with a spy on every tier read that takes a `sessions` width and a row from the other session in each tier, and the synthetic spy is gone.
