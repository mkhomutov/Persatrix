...
- When `create.enabled && create.available`, the channel picker row ([`ChannelTimeline.svelte`](../../web/src/panels/ChannelTimeline.svelte), beside the existing **Refresh** control) gains a **"New channel"** affordance that opens a small form:
  ...
  - **Members** (at least one required — the endpoint rejects an empty `members` array with `400`) — a multi-select populated from `GET /api/v1/agents`, **filtered to persona agents** (`isChattable`, i.e. `type != "task"`): a task agent runs workflow steps and never holds a conversation, so it is not a discussion participant ([`agents.js`](../../web/src/lib/agents.js); the panel already loads the list for sender decoration). Each persona carries a per-member **respond policy** select (`when_mentioned` (default) / `always` / `never`, matching [RFC 0011 §D](0011-channels-bridges.md) `RespondPolicy`).
- On submit, `POST /api/v1/channels` with `{ name, description?, members: [{ id, respond }] }`. On `201`, **reuse `loadChannels()`** to refresh the picker and select the newly-created channel (`group:<name>`) via a one-shot "select this channel" hand-off, landing the operator directly in the channel they just made. (Originally this rode a `nav.targetChannel` intent; the [chat-panel-retirement amendment §C](0048-amendment-chat-panel-retirement.md) retired `nav.svelte.js` in favour of an in-panel `pendingSelectId` that `loadChannels` consumes — same behaviour, no cross-panel module.)
- **Direct-message mode (a 1:1 DM with a persona).** The form carries a **channel-type** choice — **Group** (above) or **Direct**. A DM is not creatable through `POST /api/v1/channels` (group-only by construction); a `dm:` channel is born when you *chat* a persona (`GetOrCreateDM` on the first message — [RFC 0011 chat-as-DM](0011-amendment-chat-as-dm.md)). So **direct mode** picks **one persona** + an **opening message** and sends it via the existing chat façade (`POST /api/v1/agents/{id}/chat`, `sendChat`). `GetOrCreateDM` creates the channel and adds both participants (so no separate member/identity handling is needed). After [chat-panel-retirement §C](0048-amendment-chat-panel-retirement.md), the form hands the **persona** back and the panel **opens DM mode** for it (resolving the `dm:` channel via the chat-history endpoint) — a `dm:` channel is filtered out of the group-channel picker, so it lands as a conversation, not a channel-picker row. This is the lighter, create-form-local cousin of the [chat-panel-retirement amendment](0048-amendment-chat-panel-retirement.md)'s §B persona entry point; it adds no new backend write path (the chat façade already existed).
...
- **The acting user is added to the new channel as a member** (`respond: "never"`), because the store rejects a publish from a non-member ([`ErrNotMember`](../../internal/channels/channels.go) — *"sender is not a member of the channel"*); without this the operator could create a channel they cannot post into. The member id is the `/ui/context`-derived principal the panel already carries (the publish sender), so this adds **no new identity surface** — it is the *same* single identity source (§F rule 1), now also seeded into the membership it creates. `respond: "never"` keeps the human a participant (can post, sees history) without ever being dispatched a turn like an agent.
- In local mode the acting principal is the `/ui/context`-derived `local` (or the [slice1-ux §E "Acting as"](0048-amendment-slice1-ux.md) override, which already exists for the publish box on this panel). The create call carries no separate *creator* field — `handleCreateChannel` does not take one — so when RFC 0039 plumbs a real principal, that principal is both the creator and the seeded human member, and membership is scoped to what it can see ([RFC 0048 §F rule 2](0048-operator-tester-web-console.md#f-auth--multi-tenancy-forward-compatibility)) — a purely additive change to the same endpoint.
...
- **It does not add a DM/thread *create endpoint*.** The `POST /api/v1/channels` write path stays `group:`-only (`handleCreateChannel`, unchanged). The form's **direct mode** (§B) opens a DM through the **existing** chat façade (`GetOrCreateDM`, implicit-on-first-message — [RFC 0011](0011-channels-bridges.md)), not a new endpoint; threads remain reply-anchored and out of scope. The only backend delta for direct mode is the additive `channel_id` field on the chat response.
...
1. **The §D carve-out — ship a structural write before RFC 0039?** **Resolved 2026-06-03: yes** (signed off), behind a default-off toggle, justified because the endpoint is already exposed unauthenticated (zero new reachability) and creation is load-bearing for Slice 1's "watch personas interact" thesis; reconciled by making `create.available` capability-gated once auth lands (mirrors the slice1-ux §E sign-off). *Alternative (not taken):* hold channel creation for Slice 5 and accept the CLI/`config/channels.yaml` detour until RFC 0039 — honours the Non-Goal literally at the cost of the console's own hero scenario.
