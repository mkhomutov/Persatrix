...
| `config/demo/gemini/optimization.yaml` | **Priced** demo aliases (`quality` / `fast` / `summarizer` → `gemini-3.5-flash`) with explicit `input_per_1m_tokens` / `output_per_1m_tokens` — the missing-price guard fails closed on an unpriced non-local alias ([RFC §D](0053-gemini-watsonx-providers.md#d-pricing--the-missing-price-guard)). (Originally `gemini-2.5-pro` / `-flash`; Google retired both for new API users on 2026-07-12, repointed to `gemini-3.5-flash`.) |
