...
**Resolution: the summary is already generated at close — v0.3.8 surfaces it, with one gap to close.** Per [RFC 0020 §C](0020-interaction-lifecycle.md#c-interaction-lifecycle-states) and [§D](0020-interaction-lifecycle.md#d-storage-model), a closed multi-turn interaction (`turn_count > 1`) already persists an LLM-generated summary (`context_management.summarization.model`) to the `episodes` table on close, transitioning the row to `closed`/`summarized`; single-turn interactions degenerate to the per-event summary text; a summary timeout writes the `"[interaction summary unavailable]"` sentinel via the janitor. The summarising close path is `_persist_closed_interaction` ([agents/persona_runtime/episode_routing.py](../../agents/persona_runtime/episode_routing.py), body extracted to [`close_path.py`](../../agents/persona_runtime/close_path.py)) — it runs the LLM summary + episodic write; the in-memory boundary/trigger detection (idle/structural/cap → a closed `Interaction`) is owned by `InteractionTracker` ([agents/memory/interactions.py](../../agents/memory/interactions.py)), which performs no persistence or summarisation itself. So **generation is not in scope** — surfacing is.
...
| File | Change |
|------|--------|
| [`agents/persona_runtime/episode_routing.py`](../../agents/persona_runtime/episode_routing.py) (+ [`close_path.py`](../../agents/persona_runtime/close_path.py)) | Verify the **summarising** close path: the Layer 4 end-vote structural close already routes through `_persist_closed_interaction` (the LLM summary + episodic write live here, not in `interactions.py`). **Fix the gap**: a Layer 1 cost-ceiling termination must *close + summarise* the interaction (route it through the same `_persist_closed_interaction` path with a `cost` close reason), not merely stop fanout — otherwise a cost-bounded brainstorm stops with no readable result (SS2). |
| [`agents/memory/interactions.py`](../../agents/memory/interactions.py) | The trigger/boundary side: add a `cost` `CloseReason` (so `InteractionTracker` can emit a cost-closed `Interaction` for the persona runtime to persist) + the matching `agent.interactions.closed.by_cost` counter alongside the existing `by_idle_gap`/`by_structural` reasons. No summarisation here — `interactions.py` is the in-memory tracker. |
...
- **PR 4 merges** → CHANGELOG `[0.3.8]` Upgrade Note seeded; RFC 0020 note records the summary surface landing in v0.3.8. ✅ Merged ([#586](https://github.com/mkhomutov/Persatrix/pull/586) — `MT-INTERACTION-SUMMARY-001` + channels/web-console docs + RFC 0020 status note + CHANGELOG Upgrade Note).
...
- [`agents/persona_runtime/episode_routing.py`](../../agents/persona_runtime/episode_routing.py) + [`close_path.py`](../../agents/persona_runtime/close_path.py) (the summarising close path), [`agents/memory/interactions.py`](../../agents/memory/interactions.py) (the in-memory tracker / `CloseReason`), `web/src/panels/ConversationFeed.svelte`, [`cmd/orchestrator`](../../cmd/orchestrator) — the code this plan touches.
