...
**The trade the release note states.** The agent's default level is INFO, so nothing changes until an operator runs an agent with `--log-level DEBUG` — and at that level the record is an agent-log line like any other: redacted by the RFC 0018 §F hook and, when the log shipper is on, **shipped to the orchestrator's log store off-host**. §E's "off in prod by default" is the level default, not a separate switch. Operators who ship agent logs off-host should know the debug level now carries the persona's private silence reasons.
