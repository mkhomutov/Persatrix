...
> **Amendment — [ISSUE-0165](../issues/ISSUE-0165-relationship-hidden-outside-its-first-session.md), 2026-09-22.** The relationship **row** is outside the modes above. Its key has no session, and the §C amendment calls it cross-session shared, with the per-session views drawn from `interactions`; the §A amendment has relationships follow a peer across channels. Phase 2 PR 3 ([#450](https://github.com/mkhomutov/Persatrix/pull/450)) still filtered the row on its first-seen `session_id`, and once [#459](https://github.com/mkhomutov/Persatrix/pull/459) gave every channel its own session, a peer read as a stranger in every session but the first. Two cases hit it: a peer seeded from `relationships:` config under a start-up `PERSATRIX_SESSION_ID`, in every channel, and a peer first met in one session, in every later one, with that session's own interactions hidden too. Now `get_trust` reads the row with no session predicate and takes no `sessions`, as `get_identity` already did. `get_relationship_summary` and `get_all_relationships` read the row the same way and apply the modes above to the interaction history only (count, recent interactions, first and last seen: ISSUE-0080's Policy C). The row's `session_id` remains a first-seen record, and principal and epoch stay strict equality with no carve-out. The tier stays outside the RFC 0037 §D gate: nothing said in a channel reaches trust or the trust note in production, because trust comes only from config seeds and `update_trust` has no production caller.
...
- **Cross-session recall as a footgun.** `sessions="*"` returns every row. A debug verb that defaults to all-sessions and is wired into a prompt context risks reintroducing F-3 against the very fix this RFC ships. Mitigation: the `"*"` sentinel is gated to CLI/debug paths only and is not in the default persona-runtime context path. Pinned by an explicit unit test that asserts `_active_session_id` is consulted at every recall site in `agents/persona_runtime/`. *Updated 2026-09-18: RFC 0049 PR 4 ([#784](https://github.com/mkhomutov/Persatrix/pull/784)) changed this pin to "no ungated widening". The default facts recall passes `"*"` and the episodic recall ranks across sessions, and every widened row passes the RFC 0037 §D gate before the budget ([fact-scope amendment](0031-amendment-fact-scope-by-consolidation-level.md)); the other persona recall tiers stay session-scoped. [`test_cross_session_read_sites.py`](../../tests/unit/python/test_cross_session_read_sites.py) lists every cross-session read and what guards it.*
