...
> **Amendment — [the relationship row reads the same from every session](0031-amendment-relationship-row-cross-session.md), 2026-09-22, ratified by [#980](https://github.com/mkhomutov/Persatrix/pull/980).** That row's key has no session, so it is outside the modes above: `get_trust` takes no `sessions` and the row reads carry no session predicate, while the interaction history keeps one.
