...
| The rate limiter | Off, with `SECURITY_RATE_LIMIT_ENABLED=false`. The advisers send no ID of their own, so all four share one anonymous bucket of 60 requests a minute ([ISSUE-0111](../issues/ISSUE-0111-anonymous-wallet-rpcs-share-rate-limit-bucket.md)), and a refused publish drops an adviser's reply with no line in the call log; on the mock provider it dropped the chair's memo. A meeting whose orchestrator has not logged the limiter off is refused before the operator speaks. The harness's own requests carry the ID `exp001-harness` | 5a |
| The meeting's channel | Declared in the deployment's `channels.yaml`, shaped like the shipped `roundtable`: the arm's members and governance from `panel.yaml`, the operator as an observer, armed, with the topic "Advice for" and the organisation, an empty agenda, the panel's goal and convener, round limit 8, a 2 000 000-token budget, depth cap 5 and classification `internal`. It is named `advice-` and the meeting's place in its series, the same in every arm | 5a |
| A deployment per meeting | Arms B and C hold each meeting in a directory of its own, refused unless empty, which holds the deployment, the call log and the meeting's record. The processes run this checkout's orchestrator binary and `python -m agents.server`, on loopback, and inherit no `PERSATRIX_` or `SECURITY_` setting from the harness's shell. The meeting's clock anchor is the moment its deployment starts. The advisers stop first, each draining its memory summaries, then the orchestrator; a process still running 90 seconds after its stop signal is killed | 5a |
| The close | The first close the orchestrator logs for the channel after the operator's message, by end vote or by bounded close (round limit, depth cap or cost), or 600 seconds with no message, the orchestrator's idle window, which no channel here changes. The memo request keeps the orchestrator's own stamp of that close in the transcript. Nothing else ends the wait: disarming while the chair's closing synthesis is pending would abandon the close | 5a |
| The memo turn | After every meeting but the briefing, the harness disarms the channel at the configuration revision it has just read, makes the three other advisers observers, then posts the panel's plan or recall instruction as the operator, mentioning only the chair. The memo turn's time is the harness's real time just before that post. The memo is the chair's first message after the request | 5a |
| Failures a meeting shows | A discussion still open 60 minutes after the operator's message never closed; that is recorded, and the memo turn is held anyway, which ends it. A chair that has not answered 35 minutes after the request wrote no memo; that allows three of the provider library's 10-minute tries. Any other message after the request, 10 seconds after the memo, is recorded as the memo turn going on. None of these is retried: retries come with PR 5b. A lease the wallet refused fails the meeting once it is stopped and recorded (check 6) | 5a |
