...
> **Scope note (2026-09-06; release shape updated 2026-09-17).** This document describes the **RFC-level** cycle: how one RFC goes from authoring to closure. It is still accurate for that. The **release-level** cycle — sequencing amendment → version plan (scope locks and release checklist inside) → implementation PRs, the last of which runs the live arc → tag PR → tag, with the follow-up as the next plan's first section (until v0.3.16, release-prep PRs 0–4 sat between the implementation PRs and the tag, and the follow-up was a PR of its own) — is described in [docs/methodology/release-cycle.md](methodology/release-cycle.md); this document nests inside its Phase 1. The review discipline the phases below rely on is in [methodology/review-process.md](methodology/review-process.md), and the process vocabulary in the [process glossary](methodology/process-glossary.md).
