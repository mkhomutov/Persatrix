...
| [workflow-execution.md](workflow-execution.md) | Three sequences: (1) end-to-end workflow run (CLI → REST → planner check → pending run in `internal/state`, which the scheduler polls every second → planner → executor → task agent → wallet lease → LLM, with budget checks and cost accounting); (2) chat-message path, a DM since v0.3.0 (CLI or console → `POST /chat` → channel router → `ReceiveChannelMessage` gRPC → PersonaAgent → memory → wallet lease → LLM → reply posted back to the DM → HTTP reply); (3) autonomous brainstorm, v0.3.11 (convene → governed rounds → bounded close → chair synthesis → per-persona summaries) |
