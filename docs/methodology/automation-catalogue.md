...
| `scripts/checks/markdown.py` | Shared: what a Markdown page shows once rendered — its lines outside code fences and HTML comments (a fence closes only on a bare line of its own character at least as long as the opener, as in CommonMark, so a four-backtick block may show a three-backtick line), its ATX headings, and its table cells (closing `\|` optional) — used by the doc-links, plan-status and ROADMAP-status checks, so they read a page the way GitHub renders it | library |
