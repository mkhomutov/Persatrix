...
| `make demo-offline` / `demo-autonomous` / `demo-ollama` / `demo-anthropic` / `demo-openai` / `demo-gemini` / `demo-watsonx` | The demo society on one provider via a per-provider alias overlay; the first two spend $0 | `demo-autonomous` is the offline smoke in the last implementation PR and the tag PR |
...
| `make notices` (`scripts/generate_third_party_notices.py`) | Regenerate `THIRD_PARTY_NOTICES.md` from the three dependency graphs, with go-licenses and cargo-license at the Makefile pins (`GO_LICENSES_VERSION`, `CARGO_LICENSE_VERSION`): each is installed when missing, and any other version is refused (`make go-licenses-install` / `cargo-license-install`) | `notices-check` | Make-only; the tag PR |
...
| `make bump-version VERSION=X.Y.Z [DRY_RUN=--dry-run]` (`scripts/bump_version.py`) | Bump the five version strings ([guide](../guides/version-bump.md)) | — | The tag PR |
...
| `make release-doc KIND= VERSION= CODENAME= [PREVIOUS=] [FORCE=1]` (`scripts/release/open_doc.py`) | Open a version plan or an execution report from its template with version, codename, previous version and date filled and the guidance blockquotes removed; never overwrites. The other release documents fold into the plan (ruling (e)), so they have no kind | — | Phase 0; the last implementation PR |
| `make release-sweep [RUN=1] [REPORT=path] [ONLY=…] [SKIP=…] [OPTIONAL=1] [TIMEOUT=s]` (`scripts/release/sweep.py`) | The pre-tag gates as one command, and the only copy of their list; dry-run prints the plan, `RUN=1` runs them and prints the execution-report results table; `OPTIONAL=1` adds the Docker smoke. The venv interpreter reaches `make` through `MAKEFLAGS` (the Makefile's `PYTHON :=` ignores a plain env var) | — | The tag PR; the last implementation PR's structural-gates table |
...
|-----------------|------|---------------|
| `make lint` | `lint-go` (golangci-lint) + `lint-python` (ruff + mypy on `agents/`, `tests/`, `evaluators/`; `imports-check`) + `lint-rust` (clippy `-D warnings`, on the release `cli/rust-toolchain.toml` pins) | Locally; CI runs `make lint-go` itself (the Makefile pin + `.golangci.yml`, ISSUE-0142) and the Python/Rust tools directly |
...
| `make check-licenses` (`-go`/`-python`/`-rust`) | go-licenses at the Makefile pin (the Go leg refuses any other version), `scripts/checks/python_licenses.py`, cargo-deny against `scripts/checks/allowed_licenses.txt` / `deny.toml` | CI (`Third-party license check`) |
...
| `scripts/checks/plan_status.py` (`make plan-status-check`) | A 🔀 / ⬜ progress row whose linked PRs have all merged is stale; a 🔀 row that links no PR — a PR's own row, written before its number existed — is stale once the squash-merge that wrote it is on `main` (`git blame --first-parent`); released versions' plans are skipped, 🔄 rows are not judged. It also fails any `docs/vX.Y.Z-*.md` file but the plan and a test-findings PR plan, for a patch release after v0.3.16 (ruling (e)) | CI (`Docs hygiene`) + pre-commit |
| `scripts/checks/amendment_evidence.py` (`make amendment-evidence-check`) | Every `## Amendment YYYY-MM-DD` in a tracked `docs/**/v*-sequencing*.md`, dated 2026-09-12 or later, has an External evidence section whose five rows (installs, outside issues/PRs, demos, conversations, experiment results) each appear once and are not blank, a placeholder or "TBD", bare or wrapped in code or brackets — "none" counts; the rows, and the template's other `##` sections that stay inside an amendment, are pinned to the amendment template's by a test. A heading from that date that names an amendment in another form (level, case, word order, extra words, markup, date dashes, a setext heading) fails rather than being skipped, unless it is a sub-heading of an amendment at least as recent; so does a code fence or HTML comment that never closes, and those messages print even when no amendment could be read. Earlier amendments stay verbatim and are not judged; finding none from that date fails. It knows the sequencing record's layout and the rule's date, so the manifest lists it as Persatrix-specific, and a unit test pins its CI step and make target instead | CI (`Docs hygiene`) + pre-commit |
...
the doc checks and FILEMAP freshness in `Docs hygiene`, the RFC index, the
ROADMAP status rows and file sizes in `Validate configs` / `File size check`.
The hook is the fast local copy, not the only copy.
...
| `dependabot.yml` (config, not a workflow) | monthly | One grouped version-update PR per ecosystem — Go, pip (`agents/`), Cargo (`cli/`), the Rust toolchain (`cli/rust-toolchain.toml`, one release per PR), npm (`web/`), GitHub Actions — titled `chore(deps): …`; at most two open per ecosystem. Committed 2026-09-06; before that only security updates ran. The pip block manages the ranges in `agents/pyproject.toml` only; CI's pins move through `python-constraints-refresh.yml` |
...
CHANGELOG.md` in the tag PR, then hand curation into one bullet per
story.
