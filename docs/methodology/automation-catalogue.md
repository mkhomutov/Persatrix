...
| `scripts/checks/amendment_evidence.py` (`make amendment-evidence-check`) | Every `## Amendment YYYY-MM-DD` in a `docs/` file with "sequencing" in its path, dated 2026-09-12 or later, has an External evidence section whose five rows (installs, outside issues/PRs, demos, conversations, experiment results) each appear once and are not blank, a placeholder or "TBD" — "none" counts; the rows are pinned to the amendment template's by a test. A heading from that date that names an amendment in another form fails rather than being skipped, and so does a code fence or HTML comment that never closes. Earlier amendments stay verbatim and are not judged; finding none from that date fails. It knows the sequencing record's layout and the rule's date, so the manifest lists it as Persatrix-specific | CI (`Docs hygiene`) + pre-commit |
