...
4. **Fact scope follows subject**: person-subject facts are person-scoped (cross-room); topic-subject facts stay room-scoped. (Confirmed — **⚠️ superseded 2026-06-06 by [RFC 0049 §D](rfcs/0049-memory-consolidation-gradient.md#d-reconciliation-with-memory-scope-axesmd-and-the-one-decision-reopened)**: scope follows a fact's *consolidation level*, not its subject. A consolidated (L2) fact is cross-room regardless of subject — topic facts included — and *leakage* is enforced at egress by the RFC 0037 classification gate rather than by a recall wall. Decisions 1–3, 5–6 stand unchanged. See also the re-rooted [ISSUE-0084](issues/ISSUE-0084-fact-scope-by-subject-not-uniform-session.md).)
...
- **[ISSUE-0083](issues/ISSUE-0083-session-binding-sender-axis-fragments-multiparty-rooms.md) — drop the sender axis. ✅ Resolved 2026-05-30.** `internal/channels/session_binding.go` / `internal/channels/grpc_dispatcher.go`: the `(agent, channel, user/sender)` triple became `(agent, channel)`; `ErrEmptySessionAxis` lost the user axis and the no-sender edge case stopped being special. Channel-store schema bumped to v5 (`migrateV4ToV5`) rebuilding `session_bindings` onto the pair PK, existing triples collapsing earliest-wins. The `grpc_dispatcher_session_test.go` "two senders → distinct sessions" assertion was inverted to "two senders → one room session"; a distinct-channels test pins surviving per-room isolation. Landed before Phase 3 PR 4 (the load-bearing prerequisite for the rest).
...
- **[ISSUE-0085](issues/ISSUE-0085-epoch-axis-run-isolation.md) — add the epoch axis. ✅ Resolved in v0.3.5** (RFC 0031 [Phase 3b](v0.3.5-plan.md#phase-3b--rfc-0031-epoch-axis-issue-0085), [epoch PR plan](rfcs/0031-epoch-pr-plan.md) PRs 1–6). Migration v12 added `epoch_id` across the persona-memory tiers + channel-store v6, with `epoch_id` in the `relationships` PK; `agents/memory/_epoch_filter.py` enforces strict equality (no carve-out, no `"*"`); `agents/epoch_id.py` + the `persatrix-epoch` gRPC rail carry it from the orchestrator boot env (`PERSATRIX_EPOCH`, default `live`) / `--epoch` override into a per-request `epoch_scope`. The structural F-3 fix is gated by `tests/integration/test_epoch_run_isolation.py`. `make reset` reframed at epoch as the everyday run-isolation tool.
