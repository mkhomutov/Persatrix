...
class TestFutureMigration:
    async def test_hypothetical_v18_migration_applied(self):
        """Patch MIGRATIONS with a hypothetical v18 entry, verify v1–v18 applied.

        Forward-compat probe — always one past the highest real
        migration.  Bumped v17 → v18 when migration v17 (ISSUE-0120: fold a
        human's split agent-typed relationship row onto the user-typed one)
        landed; the rename + table-name bump preserves the "one-past-the-tail
        collision" contract that previously caught ``UNIQUE constraint
        failed: schema_version.version`` regressions.
        """
        ...
        v18 = (
            ...
            "CREATE TABLE IF NOT EXISTS _test_v18 (id TEXT PRIMARY KEY);",
        )
        original = list(MIGRATIONS)
        try:
            MIGRATIONS.append(v18)
            mem = EpisodicMemory(agent_id="test-agent", db_path=":memory:")
            await mem.initialize()
            db = mem._ensure_db()

            # All eighteen versions should be recorded
            ...
            # v18 table should exist
            async with db.execute(
                "SELECT name FROM sqlite_master WHERE type='table' AND name='_test_v18'"
            ) as cursor:
