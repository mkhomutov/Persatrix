...
class TestFutureMigration:
    async def test_hypothetical_v8_migration_applied(self):
        """Patch MIGRATIONS with a hypothetical v8 entry, verify v1–v8 applied.

        Original test asserted v1–v5; v5 is now occupied by RFC 0020's
        episodes-interaction migration, v6 by RFC 0008 PR 5's
        procedural-tier columns, and v7 by RFC 0031 Phase 1's
        session_id columns, so the forward-compat probe is bumped to
        v8.  Behaviour under test (forward-compat for new migration
        tail entries) is unchanged.
        """
        ...
        v8 = (
            ...
            "CREATE TABLE IF NOT EXISTS _test_v8 (id TEXT PRIMARY KEY);",
        )
        original = list(MIGRATIONS)
        try:
            MIGRATIONS.append(v8)
            mem = EpisodicMemory(agent_id="test-agent", db_path=":memory:")
            await mem.initialize()
            db = mem._ensure_db()

            # All eight versions should be recorded
            ...
            # v8 table should exist
            async with db.execute(
                "SELECT name FROM sqlite_master WHERE type='table' AND name='_test_v8'"
            ) as cursor:
