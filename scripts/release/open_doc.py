...
Kinds: plan, scope-locks, release-prep-plan, release-checklist,
execution-report. The previous version is read from ``CHANGELOG.md``'s newest
dated heading unless ``--previous`` is given.
"""
...
def _previous_version(repo_root: Path, version: str) -> str:
    released = sorted(
        released_versions(repo_root), key=lambda v: tuple(int(p) for p in v.split("."))
    )
    target = tuple(int(p) for p in version.split("."))
    older = [v for v in released if tuple(int(p) for p in v.split(".")) < target]
    return older[-1] if older else "0.0.0"
