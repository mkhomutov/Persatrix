...
func (s *Server) writeChannelError(w http.ResponseWriter, err error) {
	...
	case errors.Is(err, channels.ErrAutonomousConveningBoundReached), // RFC 0052 §E PR 7b — aggregate convening COUNT ceiling
		errors.Is(err, channels.ErrAutonomousStandingBudgetExhausted): // RFC 0052 §E PR 7b — aggregate standing SPEND ceiling
		writeError(w, "TOO_MANY_REQUESTS", err.Error(), http.StatusTooManyRequests)
