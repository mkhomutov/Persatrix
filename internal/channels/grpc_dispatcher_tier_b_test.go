...
// TestChannelMessageToProto_PopulatesTierBFields pins the wiring:
...
func TestChannelMessageToProto_PopulatesTierBFields(t *testing.T) {
	...
	ev := d.channelMessageToProto(
		...
		DispatchEnvelope{
			Recipient:              Member{ParticipantID: "b", RespondPolicy: RespondAlways, TierBActive: true, Threshold: &thr},
			ChannelSize:            4,
			TierBMaxChannelMembers: 20,
		})
	assert.True(t, ev.TierBActive, "a salience-gated recipient carries tier_b_active=true")
	...
	assert.Equal(t, int32(20), ev.TierBMaxChannelMembers)
	...
	assert.False(t, ev.TierBActive, "a legacy always recipient is not salience-gated")
