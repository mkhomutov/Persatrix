...
func TestLiftDisplayNameMentions_MultiWordLongestMatch(t *testing.T) {
	...
	assert.Equal(t, []string{"iron-fox"},
		liftDisplayNameMentions("@Iron Fox, take this one.", roster, "sender"),
		"the longest matching name wins — @Iron Fox is iron-fox, not iron")
	assert.Equal(t, []string{"iron"},
		liftDisplayNameMentions("@Iron, you first.", roster, "sender"),
...
func TestLiftDisplayNameMentions_CaseInsensitiveName(t *testing.T) {
	assert.Equal(t, []string{"ember-owl"},
		liftDisplayNameMentions("hey @ember owl can you take this", liftRoster(), "x"),
		"display-name matching folds case")
	assert.Equal(t, []string{"ember-owl"},
		liftDisplayNameMentions("@EMBER OWL ping", liftRoster(), "x"),
...
func TestLiftDisplayNameMentions_IdMatchAndPrecedence(t *testing.T) {
	assert.Equal(t, []string{"iron-fox"},
		liftDisplayNameMentions("@iron-fox you're up", liftRoster(), "x"),
...
func TestLiftDisplayNameMentions_AmbiguousLiftsNobody(t *testing.T) {
	...
	assert.Equal(t, []string{"iron-fox"},
		liftDisplayNameMentions("@River Heron and @Iron Fox — split the review.", roster, "x"),
		"the ambiguous display name lifts nobody; the unambiguous token still lifts")
	assert.Nil(t,
		liftDisplayNameMentions("@River Heron, you take it.", roster, "x"),
...
func TestLiftDisplayNameMentions_BoundaryAnchoring(t *testing.T) {
	assert.Nil(t,
		liftDisplayNameMentions("mail me at local@iron-fox.example", liftRoster(), "x"),
		"an email's `@` is not a mention boundary")
	assert.Nil(t,
		liftDisplayNameMentions("foo@iron-fox", liftRoster(), "x"),
		"a mid-word `@` is not a mention boundary")
	assert.Nil(t,
		liftDisplayNameMentions("(@iron-fox)", liftRoster(), "x"),
		"a non-whitespace lead (the TOKEN_RE `(^|\\s)@` posture) is not a boundary")
	assert.Equal(t, []string{"iron-fox"},
		liftDisplayNameMentions("@iron-fox at start", liftRoster(), "x"),
...
func TestLiftDisplayNameMentions_TrailingBoundary(t *testing.T) {
	assert.Nil(t,
		liftDisplayNameMentions("@Iron Foxes are clever", liftRoster(), "x"),
		"a name is not matched into the middle of a longer word")
	assert.Equal(t, []string{"iron-fox"},
		liftDisplayNameMentions("@Iron Fox! over to you", liftRoster(), "x"),
...
func TestLiftDisplayNameMentions_ExcludesSender(t *testing.T) {
	assert.Equal(t, []string{"ember-owl"},
		liftDisplayNameMentions("@Iron Fox @Ember Owl — your turn", liftRoster(), "iron-fox"),
		"the sender's own display name is not lifted; the co-addressee is")
	assert.Nil(t,
		liftDisplayNameMentions("@iron-fox thinking out loud", liftRoster(), "iron-fox"),
...
func TestLiftDisplayNameMentions_EveryoneSentinelNotLifted(t *testing.T) {
	assert.Nil(t,
		liftDisplayNameMentions("@everyone please weigh in", liftRoster(), "x"),
...
func TestLiftDisplayNameMentions_EmptyShapes(t *testing.T) {
	assert.Nil(t, liftDisplayNameMentions("", liftRoster(), "x"),
		"empty content lifts nothing")
	assert.Nil(t, liftDisplayNameMentions("@Iron Fox", nil, "x"),
		"an empty roster lifts nothing")
	assert.Nil(t, liftDisplayNameMentions("just a plain sentence, no addressing", liftRoster(), "x"),
...
func TestLiftDisplayNameMentions_RegistrylessMemberStillIdMatches(t *testing.T) {
	...
	assert.Equal(t, []string{"alex"},
		liftDisplayNameMentions("@alex what's the call", roster, "x"),
