...
//
// The reserve/release split makes the count reflect convenings that ACTUALLY
// happened: [ChannelRouter.reserveConvening] takes a slot atomically before the
// dispatch, and a dispatch MISS returns it via [ChannelRouter.releaseConvening],
// so a flapping convener endpoint never silently exhausts the aggregate budget
// without a single discussion opening. Doing the check-and-increment under one
...
// dispatch misses, so the persisted count reflects convenings that actually
// opened. Check and increment run under one lock so two concurrent convenes at
// `max-1` cannot both slip through (the count's half of the idle convene race).
...
// ConveningCount reports how many times channelID has been convened this process
// lifetime — the value the §E aggregate bound is measured against and the web
// convening-count readout (a later PR 7b slice) renders. Zero for a channel never
// convened.
