...
type Channel struct {
	...
	// Classification is the RFC 0037 §A level (v0.3.12 PR 2). Empty on input
	// normalizes to `internal` at the store boundary ([NormalizeForStamp],
	// the SessionID precedent); always a known lattice level on output.
...
// participantIDPattern / channelNamePattern / ValidateParticipantID /
// CanonicalDMID live in identifiers.go (moved with channelTypeFromID when
// RFC 0037 PR 2's classification field pushed this file past the 500-line
// review cap).
