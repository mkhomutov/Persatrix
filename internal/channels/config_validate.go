...
func (c *Config) Validate() error {
	...
	// The item-8 dark-window ceiling that sat here (PR 1–3) was deleted when
	// the §D gate armed (RFC 0037 PR 4) — any §A level is now declarable.
	...
	for i, ch := range c.Channels {
		...
		// The item-8 dark-window ceiling that sat here (PR 1–3) was deleted
		// when the §D gate armed (RFC 0037 PR 4).
