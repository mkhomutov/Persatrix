...
// TestToConfigOverrides_CapturesNonDefaultReasoning is the F1 regression: a
// YAML-declared NON-off reasoning rung MUST be captured in the snapshot the
// reconcile persists, or the boot round-trip (adopt → ResolveFromStore) silently
// clobbers it back to off and the drift hash goes blind to it. Mirrors the
// per-sub-knob freeze: only the committed sub-knob(s) are explicit; the rest stay
// inherit.
func TestToConfigOverrides_CapturesNonDefaultReasoning(t *testing.T) {
	body := `
channels:
  - name: planning
    revision: 1
...
`
	...
	o := cfg.Channels[0].toConfigOverrides(cfg)
	require.NotNil(t, o.Reasoning, "a non-off YAML rung must be captured in the snapshot")
	require.NotNil(t, o.Reasoning.Mode)
	assert.Equal(t, ReasoningModeBid, *o.Reasoning.Mode)
...
// YAML channel declaring mode=bid must still resolve to bid after the full boot
// sequence (ResolveReasoning → ReconcileFromYAML → ResolveFromStore), not be reset
// to off by the store-overlay step.
...
`
	...
	assert.Equal(t, ReasoningModeBid, router.ReasoningFor("group:planning").Mode,
		"a YAML-declared bid rung must survive the reconcile→overlay boot round-trip")
}
...
// differently, so a hand-edit to the reasoning block is caught as drift.
