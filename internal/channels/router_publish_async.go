...
// DrainPendingFanout (the bounded shutdown drain) and WaitForPendingFanout
// (the unbounded test barrier) live in router_fanout_drain.go — split out
// when the ISSUE-0114 per-channel cascade-depth resolve pushed this file past
...
func (r *ChannelRouter) publishCommit(ctx context.Context, msg ChannelMessage, declaredType string) (*fanoutPlan, error) {
	...
	// `cascade_depth` to [0, the channel's effective cap] BEFORE the store
	// commit so `GET /messages` returns what was enforced, not the
	// publisher's claim. Defends against over-cap poisoning (NOT reset-to-0
	// — see the amendment's Future work). The cap is resolved per channel
	// (ISSUE-0114) once here and reused at the fanout-suppression check
	// below, so one publish enforces one number even against a concurrent
	// RFC 0050 apply.
	depthCap := r.maxCascadeDepthFor(msg.ChannelID)
	inboundDepth := readCascadeDepth(msg.Metadata)
	clampedDepth := clampCascadeDepth(inboundDepth, depthCap)
	...
	// channel's effective cap (captured above the clamp — ISSUE-0114). The
	// publish itself succeeded (2xx) — only the cascade is terminated. The
	// Python dispatcher cap (agents/dispatch.py) remains as defense-in-depth
