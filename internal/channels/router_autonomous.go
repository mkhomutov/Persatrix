...
// path of EVERY channel — the fanout-tail scope gate (maybeBoundedClose), the
// resolver's latch gate (resolveInteractionID), the floor head check
// (stimulusOutlivedClose) — where a plain Mutex would serialize all publishes
// on one router-global lock just to read a value written at startup and on
// the rare RFC 0050 apply.
