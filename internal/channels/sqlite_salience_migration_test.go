...
// `memberships.salience_gated` columns migration (channelStoreSchemaVersion
...
// vocabulary (`participant`/`chair`) carry `salience_gated = 1` and run the
// salience bid. The store persists the resolved Member fields verbatim; the
// disposition → (salience_gated, threshold) derivation happens at the config /
// REST boundary that still sees the disposition (see [resolveSalienceSignal]).
package channels
...
// TestSQLiteStore_SchemaV7_FreshDB_MembershipsHasSalienceColumns asserts the
// memberships table grows the two Tier B columns on a fresh database.
func TestSQLiteStore_SchemaV7_FreshDB_MembershipsHasSalienceColumns(t *testing.T) {
	...
	withDB(t, path, func(db *sql.DB) {
		...
		assert.Contains(t, cols, "salience_gated", "memberships.salience_gated missing")
	})
}
...
func TestSQLiteStore_SchemaV7_Migration_Idempotent(t *testing.T) {
	...
	withDB(t, path, func(db *sql.DB) {
		...
		// (TestSQLiteStore_SchemaV8_Migration_Idempotent) per the convention the
		// v5/v6 test headers document; this test now only asserts that a reopen
...
// `salience_gated = 0` (legacy always) — no data loss, v0.3.7 behaviour intact.
func TestSQLiteStore_Migration_V6ToV7_PreservesRows(t *testing.T) {
	...
	// A pre-v7 group channel + a membership row (no threshold / salience_gated
	// columns yet).
	_, err = db.Exec(
	...
	withDB(t, path, func(db *sql.DB) {
		...
		// hand-built v6 DB lands at channelStoreSchemaVersion (≥8 since the RFC
		// 0050 config columns migration). What this test pins is the v6→v7 Tier B
		// backfill below, which the migration chain still applies en route.
		assert.GreaterOrEqual(t, version, 7, "the v6→v7 migration ran (chain continues to latest)")

		var threshold sql.NullFloat64
		var salienceGated int
		require.NoError(t, db.QueryRow(
			`SELECT threshold, salience_gated FROM memberships
			   WHERE channel_id = 'group:planning' AND participant_id = 'alice'`).
			Scan(&threshold, &salienceGated))
		assert.False(t, threshold.Valid, "pre-v7 membership row backfilled to NULL threshold (unset)")
		assert.Equal(t, 0, salienceGated, "pre-v7 membership row backfilled to salience_gated=0 (legacy always)")
	})
	...
	assert.False(t, members[0].SalienceGated)
}

// TestSQLiteStore_CreateChannelWithMembers_RoundTripsSalienceFields proves the
...
func TestSQLiteStore_CreateChannelWithMembers_RoundTripsSalienceFields(t *testing.T) {
	path := filepath.Join(t.TempDir(), "channels.db")
	store, err := NewSQLiteStore(path, SQLiteOptions{})
	require.NoError(t, err)
	t.Cleanup(func() { _ = store.Close() })
	...
	require.NoError(t, store.CreateChannelWithMembers(context.Background(),
		...
		[]Member{
			...
			{ParticipantID: "alice", RespondPolicy: RespondAlways, SalienceGated: true},
			// A chair-like participant: salience-gated with an explicit bar.
			{ParticipantID: "bob", RespondPolicy: RespondAlways, SalienceGated: true, Threshold: &thr},
	...
	assert.True(t, byID["alice"].SalienceGated)
	assert.Nil(t, byID["alice"].Threshold, "unset threshold round-trips as nil, not 0.0")

	assert.True(t, byID["bob"].SalienceGated)
	require.NotNil(t, byID["bob"].Threshold)
	assert.Equal(t, 0.3, *byID["bob"].Threshold)

	assert.False(t, byID["carol"].SalienceGated, "legacy always member is not salience-gated")
	assert.Nil(t, byID["carol"].Threshold)
}

// TestSQLiteStore_AddMember_DerivesSalienceFromDisposition pins the REST
...
func TestSQLiteStore_AddMember_DerivesSalienceFromDisposition(t *testing.T) {
	path := filepath.Join(t.TempDir(), "channels.db")
	store, err := NewSQLiteStore(path, SQLiteOptions{})
	require.NoError(t, err)
	t.Cleanup(func() { _ = store.Close() })
	...
	assert.True(t, alice.SalienceGated, "participant is salience-gated")
	assert.Nil(t, alice.Threshold, "a plain participant carries no explicit threshold")

	bob := got("bob")
	assert.True(t, bob.SalienceGated, "chair is salience-gated")
	...
	carol := got("carol")
	assert.False(t, carol.SalienceGated, "legacy always member is not salience-gated")
