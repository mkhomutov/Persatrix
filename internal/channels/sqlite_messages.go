...
func (s *sqliteStore) PublishMessage(ctx context.Context, msg ChannelMessage) error {
	...
	// ISSUE-0130 shape (b): resolve the tenant from the PUBLISHING CONTEXT
	// and overwrite whatever the caller carried on the struct.
	//
	// Assignment, not an `if == ""` default like SessionID above, and that
	// difference is the security property: the session id is an operator
	// namespace a caller may legitimately choose, while the principal is the
	// orchestrator's own verification of who caused this message. An
	// agent-supplied principal would be a cross-tenant READ primitive the
	// moment B2 seeds the replayed event from this column — the persona binds
	// `principal_scope` from it and recall is strict equality — so the store
	// takes the ctx as the only source and the REST body has no field to
	// carry one. Same posture as the RFC 0030 interaction-id producer, which
	// likewise replaces an inbound claim rather than filling a gap.
	//
	// The context is authoritative for both writers that reach here: an
	// authenticated REST publish (authMiddleware → withRequestPrincipal), and
	// the R-2 causal re-stamp `publishCommit` applies just above this call so
	// a persona's relayed reply persists the tenant of the person who caused
	// it rather than collapsing to `local`.
	//
	// Stamping HERE rather than in `publishCommit` covers the router-nil
	// fallback ([Server.handlePublishMessage] calls the store directly) and
	// makes "the body cannot set it" structural rather than a property of
	// every caller. [PrincipalFromContext] yields "" for every
	// unauthenticated publish and under `auth.mode: disabled`; Go has no
	// `local` sentinel on the wire, so the coercion is explicit.
	msg.PrincipalID = PrincipalFromContext(ctx)
	if msg.PrincipalID == "" {
		msg.PrincipalID = DefaultPrincipalID
	}
	...
	if _, err := tx.ExecContext(ctx,
		`INSERT INTO messages (id, channel_id, sender_id, content, timestamp, thread_id, mentions, metadata, session_id, principal_id)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		msg.ID, msg.ChannelID, msg.SenderID, msg.Content, msg.Timestamp,
		threadID, mentions, metadata, msg.SessionID, msg.PrincipalID); err != nil {
