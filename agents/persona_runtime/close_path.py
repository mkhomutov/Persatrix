...
This module owns what closing ONE record means: the skips that decide
whether a closed record derives at all, the ``store_episode`` row it
writes under the record's own write axes (:mod:`.record_write_scope`),
and the background finalise it schedules.  The fans that close MANY
records from one trigger and hand each to this path live in
:mod:`.close_fan` (v0.3.16 PR D2, the ISSUE-0143 debt sweep — this file
sat at exactly its cap, and the ISSUE-0137 guard has to grow on the
``store_episode`` call below), and the boot-path sweep in
:mod:`.replay_sweep`.
"""
