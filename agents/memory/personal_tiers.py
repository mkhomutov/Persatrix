...
:class:`~agents.memory.facts.FactStore`) directly — and so the only
place the RFC 0029 PR 2 ``DeprecationWarning`` on direct external
construction fired in production.
...
def build_personal_tiers(
    ...
    *,
    ...
    PR 2 boundary ``DeprecationWarning`` does not fire — the persona
    factory is no longer a direct external constructor of the per-tier
    classes.  The returned handles are *un-initialised*: the caller
    (the persona runtime) opens and closes each tier.
