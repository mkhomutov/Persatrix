...
> **Last updated**: 2026-08-19 (v0.3.14 *One persona, many people* — ✅ [**Released**](https://github.com/mkhomutov/Persatrix/releases/tag/v0.3.14); all four phases complete. Next: a **v0.3.x** tail completing the interaction/tenant work, then the v0.4.0 train.)
> **Current phase**: **v0.3.15** — 📋 next up: the three residuals the v0.3.14 changelog already assigns to it — [ISSUE-0082](docs/issues/ISSUE-0082-orchestrator-per-request-session-principal-emission.md) **R-1** (per-speaker interaction scope, an RFC 0020 §G change) and **R-2** (agent cascades), plus [ISSUE-0130](docs/issues/ISSUE-0130-catchup-replay-rederives-memory-under-default-principal.md) shape **(b)** (persist `principal_id` on `messages`, channel store v11 → v12) — so interaction functionality is complete before v0.4.0 organizations build on it. Further v0.3.x work may precede v0.4.0; scope and sequencing land with that plan (the [Amendment 2026-08-02](docs/v0.3.x-sequencing.md#amendment-2026-08-02--v0313--v0314-the-two-release-tail-to-v040) predates the 2026-08-18 re-slot and still reads v0.3.14 → v0.4.0). Then **v0.4.0** *Agent Organizations* (RFC 0012 + RFC 0028). Prior phase: v0.3.14 *One persona, many people* — ✅ [Released 2026-08-19](https://github.com/mkhomutov/Persatrix/releases/tag/v0.3.14).
> **Current milestone**: **v0.3.14** *One persona, many people* — ✅ [Released 2026-08-19](https://github.com/mkhomutov/Persatrix/releases/tag/v0.3.14) ([plan](docs/v0.3.14-plan.md) · [release-prep plan](docs/v0.3.14-release-prep-plan.md) · [checklist](docs/v0.3.14-release-checklist.md); implementation complete and the live two-account MT **passed** — [execution report](docs/manual-tests/v0.3.14-execution-report.md), both emitted principals recorded off storage). The story: **two authenticated people talk to the same persona — beyond localhost — and neither conversation bleeds into the other's memory.** The persona-side tenant vertical has been armed and unfed since 2026-05-29 ([ISSUE-0081](docs/issues/ISSUE-0081-session-id-process-global-not-task-local.md) — `principal_id` in the storage key, strict-equality recall, the `persatrix-principal` rail); shipping auth made the caller's `participant_id` a verified claim, so this release supplies the missing **source** ([ISSUE-0082](docs/issues/ISSUE-0082-orchestrator-per-request-session-principal-emission.md) Part 2) plus the erasure residual. Cuttable: [RFC 0040](docs/rfcs/0040-agent-orchestrator-transport-unification.md) Phase 1. Prior milestone: v0.3.13 *Deferred calls closed* — ✅ [Released 2026-08-05](https://github.com/mkhomutov/Persatrix/releases/tag/v0.3.13) ([plan](docs/v0.3.13-plan.md) · [release-prep plan](docs/v0.3.13-release-prep-plan.md)): tool recalls obey the same epoch/session isolation as injected memory ([ISSUE-0118](docs/issues/ISSUE-0118-tool-recall-bypasses-epoch-session-scopes.md)), the person-identity legs ran live ([ISSUE-0121](docs/issues/ISSUE-0121-crossroom-person-identity-legs-never-run-live.md)), per-channel cascade depth ([ISSUE-0114](docs/issues/ISSUE-0114-per-channel-cascade-depth-override.md)), fold-in [ISSUE-0116](docs/issues/ISSUE-0116-fact-subject-renders-unquarantined.md). **Next milestone: v0.4.0** — Agent Organizations.
