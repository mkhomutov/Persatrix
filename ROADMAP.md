...
> **Last updated**: 2026-05-27 (v0.3.4 release-prep PR 4 — final pre-tag verification. PRs 0–3 of release prep merged: plan ([#438](https://github.com/mkhomutov/Persatrix/pull/438)), MT execution report ([#439](https://github.com/mkhomutov/Persatrix/pull/439)), README/ROADMAP + model-providers guide + provider-neutral onboarding + the knob-free provider-selection refactor + release checklist ([#440](https://github.com/mkhomutov/Persatrix/pull/440)), version bump `0.3.3`→`0.3.4` + curated `[0.3.4]` changelog ([#442](https://github.com/mkhomutov/Persatrix/pull/442)); a follow-on ([#441](https://github.com/mkhomutov/Persatrix/pull/441)) refreshed the four MT recipes to config-driven form, and a fix ([#443](https://github.com/mkhomutov/Persatrix/pull/443)) gated the full integration suite in CI + repaired the close-path tests against the unconfigured summarizer (ISSUE-0076). PR 4 (this PR) re-certified the gate sweep on the post-bump tip — `make lint` / `validate` / proto staleness / `check-licenses` / `notices` / sanitizer parity / build / Go race-cover / Rust / the alias cost-attribution gate / the bored-persona `cost-regression-gate` / the alias-provider Python slices all green, with CI green on the RC tip covering the full unit + integration suites — and flips the Version Map v0.3.4 row `🚧 Release prep` → `✅ Released`; the `v0.3.4` git tag and GitHub Release follow post-merge per [release-prep plan Phase 5](docs/v0.3.4-release-prep-plan.md). RFC 0033 stays `⚠️ Partially Implemented (Phases 1–2)`; Phase 3 (raw-ID rejection, `_infer_provider` retirement) → v0.3.5+, observed-traffic gated.)
> **Current phase**: v0.3.4 (Any Model, Any Provider — RFC 0033 Phases 1–2) — ✅ Released
> **Current milestone**: v0.3.4 released. The same agents run on any provider — Anthropic, OpenAI, a free local model (Ollama), or a $0 offline mock — selected the one standard way: a logical model alias (`quality` / `fast` / `summarizer`) whose `provider` field points at it. A vendor retirement or a provider swap is a one-line config edit; the base config ships **no default provider** (a plain `docker compose up` fails loud until one is chosen), and a missing-price guard fails closed for unpriced cloud aliases so cost stays a structural gate. Next phase is v0.3.5 (per-session memory namespacing — the F-3 root-cause fix, RFC 0031 Phases 2–4), then v0.4.0 (team/hierarchy).
