...
> **Last updated**: 2026-05-22 (v0.3.3 Phase 2 — RFC 0024 Phases-1–4 closeout open: RFC 0024 → `⚠️ Partially Implemented (Phases 1–4)`. All nine RFC 0024 PR-plan PRs (1–6, incl. 2.1/3a/3b/5.1) merged — the agent autonomy loop is structurally event-driven via `agents/event_loop.py` (per-agent `asyncio.Queue[WakeEvent]`, `SyncDispatchHandle`, `TickScheduler` thin adapter), `autonomy.timers` + per-agent `scheduled_wakes` cache, default-off `SalienceWake`, fire-and-forget channel-message dispatch, and the "bored persona costs nothing" cost-regression CI gate. Phases 5 (deprecation warning) → v0.4.0, Phase 6 (removal) → v0.5+. Version Map v0.3.3 row stays `🚧 Planning` until release-prep close per [v0.3.3-plan §ROADMAP hygiene](docs/v0.3.3-plan.md#roadmap-hygiene).)
> **Current phase**: v0.3.3 (Idle Truly Idle — RFC 0024 Phases 1–4) — 🚧 RFC 0024 implementation complete; release-prep pending
> **Current milestone**: v0.3.3 Phase 2 RFC 0024 Phases-1–4 closeout open ([#414](https://github.com/mkhomutov/Persatrix/pull/414) — `feature/v033-rfc0024-close`): RFC 0024 flips to `⚠️ Partially Implemented (Phases 1–4)`. A persona with no scheduled work and no inbound traffic now pays no SQLite recall, no `_inject_memory_context`, no provider activity, no wallet lease — the polling-loop cost-leak class is closed structurally. Next: v0.3.3 release-prep plan (master-plan Phase 3). Previous milestone: RFC 0024 PR 5.1 review-follow-up cleanups + deferred test-gap fills merged ([#413](https://github.com/mkhomutov/Persatrix/pull/413)).
