...
> **Last updated**: 2026-06-04 (v0.3.6 **release prep in progress** — [v0.3.6-plan.md](docs/v0.3.6-plan.md) Phases 2–3, executing the [v0.3.6 release-prep plan](docs/v0.3.6-release-prep-plan.md). Both v0.3.6 workstreams are fully merged: RFC 0048 Phase 1 / Slice 1 + amendments ([#496](https://github.com/mkhomutov/Persatrix/pull/496)–[#516](https://github.com/mkhomutov/Persatrix/pull/516)) and the RFC 0030 Layer 2.5 floor-control amendment ([#517](https://github.com/mkhomutov/Persatrix/pull/517)–[#521](https://github.com/mkhomutov/Persatrix/pull/521), telemetry included). Release-prep PR 0 (plan, [#522](https://github.com/mkhomutov/Persatrix/pull/522)) and PR 1 (MT execution report — `MT-CONSOLE-001` + `MT-CHANNEL-GOV-002` both **Pass live** on Anthropic, [#523](https://github.com/mkhomutov/Persatrix/pull/523)) have merged. This PR (release-prep PR 2) lands the README v0.3.6 Roadmap row, the [web console](docs/guides/web-console.md) + [channels §7](docs/guides/channels.md) + [observability §11.3](docs/observability.md) guide verification, and the [v0.3.6 release checklist](docs/v0.3.6-release-checklist.md); it flips the v0.3.6 [§Version Map](#version-map) row `🚧 Planning → 🚧 Release prep`. Version bump + changelog (PR 3) and the final pre-tag sweep + tag (PR 4 / Phase 4) cut the version. RFC 0048 stays `⚠️ Partially Implemented` (Slices 2–5 → v0.4.0+ / post-RFC 0039). The prior milestone v0.3.5 *Session-Scoped Memory* is ✅ Released ([GitHub Release](https://github.com/mkhomutov/Persatrix/releases/tag/v0.3.5), tagged 2026-06-01). Next phase after v0.3.6 is v0.4.0 (team/hierarchy).)
> **Current phase**: v0.3.6 (Web Console — RFC 0048 Phase 1 / Slice 1: Interactions + the RFC 0030 floor-control fold-in) — 🚧 implemented; **release prep in progress** (Phases 2–3 of [v0.3.6-plan.md](docs/v0.3.6-plan.md)); the [§Version Map](#version-map) row is 🚧 Release prep until the tag cuts the version (Phase 4)
> **Current milestone**: v0.3.6 release prep — both workstreams merged and both headline manual tests pass live ([#523](https://github.com/mkhomutov/Persatrix/pull/523)); the release-prep PRs (docs + checklist → version bump + changelog → final pre-tag sweep, per the [release-prep plan](docs/v0.3.6-release-prep-plan.md)) are in flight. The headline user-facing story: **open a URL and talk** — run the orchestrator with `--enable-ui`, open `http://localhost:8080/ui`, pick a persona, chat with it, and watch a channel, with zero CLI knowledge — the first vertical slice of the [RFC 0048](docs/rfcs/0048-operator-tester-web-console.md) embedded same-origin web console (Svelte SPA served from the Go orchestrator via `embed.FS`, behind a per-panel feature-toggle model in `config/ui.yaml`), folded in with the [RFC 0030 Layer 2.5 floor-control amendment](docs/rfcs/0030-amendment-floor-control-speaker-serialization.md) so a group channel reads as a coherent, ordered conversation rather than a simultaneous shout. Memory/cost/control-plane panels and authentication are deferred to later slices (v0.4.0+ / post-RFC 0039). The prior milestone — v0.3.5 *Session-Scoped Memory* (RFC 0031 Phases 2–4 + epoch axis + RFC 0033 Phase 3 co-resident) — is ✅ Released ([GitHub Release](https://github.com/mkhomutov/Persatrix/releases/tag/v0.3.5), 2026-06-01). Next phase after v0.3.6 is v0.4.0 (team/hierarchy).
...
| **v0.3.6** | Open a URL and talk to a persona — chat and watch a channel from the browser with zero CLI knowledge, via an embedded same-origin web console behind `--enable-ui` (RFC 0048 Slice 1: Interactions; later slices add memory/cost/control-plane panels) | 🚧 Release prep |
