...
> **Last updated**: 2026-06-19 (RFC 0036 recall PR 3: the audited `POST …/personas/{id}/recall` endpoint over the PR 2 scoped query, emitting the server-side `channel.recall` audit. PRs 1–2 merged; RFC 0035 ledger merged, PRs 1–4.)
...
| **v0.3.9** | Personas quote each other accurately and you can search the verbatim record of what was said, scoped to the channels each was present for (RFC 0036 + RFC 0035) | 🚧 Implementing — RFC 0035 ledger PR 1 (dormant channel-store migration v9) open; [v0.3.9 master plan](docs/v0.3.9-plan.md) (membership-scoped verbatim recall; `epoch`-filtered, session-spanning) |
