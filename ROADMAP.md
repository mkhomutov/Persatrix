...
> **Last updated**: 2026-05-20 (v0.3.2 release-prep PR 2 — README + ROADMAP + persona-agents + observability guide refresh + release checklist creation. PR 0 of release prep (#393), PR 1 manual-test execution report (#394 / re-run #397 with release gate met), and the ISSUE-0065 / ISSUE-0066 chat-error-publish + RESOURCE_EXHAUSTED follow-up fixes (#395 / #396 / #398) all merged on the `v0.3.1..HEAD` RC tip. Version Map v0.3.2 row flipped 🚧 Planning → 🚧 Release prep.)
> **Current phase**: v0.3.2 (Cost Gate + Facade Freeze — RFC 0023 full + RFC 0029 Phase 1) — 🚧 Release prep
> **Current milestone**: v0.3.2 release prep in flight — PR 0 plan ([#393](https://github.com/mkhomutov/Persatrix/pull/393)) and PR 1 MT execution report ([#394](https://github.com/mkhomutov/Persatrix/pull/394) → re-run [#397](https://github.com/mkhomutov/Persatrix/pull/397) with release gate met) merged; PR 2 (this PR) refreshes public docs and creates [`docs/v0.3.2-release-checklist.md`](docs/v0.3.2-release-checklist.md); PR 3 (version bump + curated changelog) and PR 4 (final pre-tag verification) follow per [v0.3.2-release-prep-plan.md](docs/v0.3.2-release-prep-plan.md). All five RFC 0023 LLM-call origins (workflow / chat / autonomous TICK / sub-agent / channel-message) acquire a server-issued wallet lease before issuing; RFC 0029 Phase 1 freezes the `MemoryStore` facade as the single path to agent memory ahead of the v0.4.0 Postgres split.
...
| **v0.3.2** | Every LLM call in the system passes through a wallet lease before it is issued — cost is a structural gate, not a post-hoc accountant — and the memory facade is frozen as the single path to agent memory ahead of the v0.4.0 Postgres split | 🚧 Release prep |
