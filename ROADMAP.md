...
> **Last updated**: 2026-05-30 (v0.3.5 — RFC 0031 **Phase 4 docs closeout** (= [v0.3.5 plan](docs/v0.3.5-plan.md) Phase 3): [`docs/guides/sessions.md`](docs/guides/sessions.md) ships the operator guide (resolution chain, `legacy` carve-out, split-volume `make reset` asymmetry, no-secrets-in-labels); the [channels.md](docs/guides/channels.md) / [persona-agents.md](docs/guides/persona-agents.md) `make reset` subsections carry the **scope-axes-reframed** breadcrumb — session is room continuity (not a clean slate), so `make reset` stays the run-isolation nuke until the `epoch` axis ([ISSUE-0085](docs/issues/ISSUE-0085-epoch-axis-run-isolation.md)) ships. [ISSUE-0051](docs/issues/ISSUE-0051-per-session-memory-namespacing-channels.md) is **closed** and RFC 0031 reaches **✅ Implemented** (all four phases). Version Map v0.3.5 row stays `🚧 Planning` until the release tag.)
> **Current phase**: v0.3.5 (Session-Scoped Memory — RFC 0031 Phases 2–4) — 🚧 Planning
> **Current milestone**: v0.3.5 planning opened. The release makes **persona-memory recall session-scoped**: a run reads only its own session's rows (plus the always-visible `legacy` carve-out), so concurrent conversations don't bleed and an operator isolates a run by giving it a fresh session rather than nuking volumes with `make reset` — closing the **F-3** cross-run *recall* bleed at the root. (Auto-isolating a *same-named* rerun is the deferred `epoch` axis, [ISSUE-0085](docs/issues/ISSUE-0085-epoch-axis-run-isolation.md).) Phase 2 makes default recall session-scoped (the F-3 closer, drafted in [`0031-phase2-pr-plan.md`](docs/rfcs/0031-phase2-pr-plan.md)); Phase 3 adds the `persatrix session …` operator CLI; Phase 4 ships the operator guide and closes [ISSUE-0051](docs/issues/ISSUE-0051-per-session-memory-namespacing-channels.md). RFC 0033 Phase 3 (raw-ID rejection) is a conditional co-resident, gated on the `raw_id_usage` counter reading zero across dogfood — observed state, not a date. Next phase after v0.3.5 is v0.4.0 (team/hierarchy).
