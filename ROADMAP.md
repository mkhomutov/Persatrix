...
> **Last updated**: 2026-06-04 (v0.3.6 **release-prep PR 4 — final pre-tag verification**. All release-prep PRs merged: plan ([#522](https://github.com/mkhomutov/Persatrix/pull/522)), MT execution report ([#523](https://github.com/mkhomutov/Persatrix/pull/523)), README/ROADMAP + web-console/channels guide verification + release checklist ([#525](https://github.com/mkhomutov/Persatrix/pull/525)), version bump `0.3.5`→`0.3.6` + curated `[0.3.6]` changelog ([#526](https://github.com/mkhomutov/Persatrix/pull/526)). PR 4 (this PR) re-certified the gate sweep **live on host** (Go 1.26.3 + Node 22 + Docker) on the post-bump tip — `make validate` / doc-audit / sanitizer parity / `cargo build` `0.3.6` / `cargo test` 153 / clippy / `make ui` real bundle / the full Go `./internal/... -race -cover` suite (18 pkgs `ok`, no races) incl. the **web-console + floor-control structural gates** (the flag-off concurrent-fanout regression among them) / the **alias cost-attribution gate** / the **session + epoch structural-isolation gates** (20 passed) / **Vitest 162** (up from 115 with the [#524](https://github.com/mkhomutov/Persatrix/pull/524) mention-composer tests) / the Python unit suite (2900) / ruff + Python/Rust licenses — all green, with CI carrying the Go licenses + proto staleness + full Python integration suite — and flips the Version Map v0.3.6 row `🚧 Release prep` → `✅ Released`. Both v0.3.6 headline gates (`MT-CONSOLE-001` + `MT-CHANNEL-GOV-002`) already passed **live on Anthropic** in PR 1, so no live hard-block was outstanding; the live Anthropic Docker smoke carries forward from the PR 1 run. The `v0.3.6` git tag and GitHub Release ([Phase 4](docs/v0.3.6-plan.md#phase-4--tag-and-post-release-follow-up)) are the only remaining post-merge steps. Both v0.3.6 workstreams are landed: RFC 0048 Phase 1 / Slice 1 + amendments ([#496](https://github.com/mkhomutov/Persatrix/pull/496)–[#516](https://github.com/mkhomutov/Persatrix/pull/516)) and the RFC 0030 Layer 2.5 floor-control amendment ([#517](https://github.com/mkhomutov/Persatrix/pull/517)–[#521](https://github.com/mkhomutov/Persatrix/pull/521)). RFC 0048 stays `⚠️ Partially Implemented` (Slices 2–5 → v0.4.0+ / post-RFC 0039). The prior milestone v0.3.5 *Session-Scoped Memory* is ✅ Released ([GitHub Release](https://github.com/mkhomutov/Persatrix/releases/tag/v0.3.5), tagged 2026-06-01). Next phase after v0.3.6 is v0.4.0 (team/hierarchy).)
> **Current phase**: v0.3.6 (Web Console — RFC 0048 Phase 1 / Slice 1: Interactions + the RFC 0030 floor-control fold-in) — ✅ Released
> **Current milestone**: v0.3.6 released. The headline user-facing story: **open a URL and talk** — run the orchestrator with `--enable-ui`, open `http://localhost:8080/ui`, pick a persona, chat with it, and watch a channel, with zero CLI knowledge — the first vertical slice of the [RFC 0048](docs/rfcs/0048-operator-tester-web-console.md) embedded same-origin web console (Svelte SPA served from the Go orchestrator via `embed.FS`, behind a per-panel feature-toggle model in `config/ui.yaml`), folded in with the [RFC 0030 Layer 2.5 floor-control amendment](docs/rfcs/0030-amendment-floor-control-speaker-serialization.md) so a group channel reads as a coherent, ordered conversation rather than a simultaneous shout. Both headline manual tests passed live on Anthropic ([#523](https://github.com/mkhomutov/Persatrix/pull/523)) and the PR 4 gate sweep re-certified the post-bump tip; the `v0.3.6` git tag + GitHub Release are the only remaining Phase 4 post-merge steps. Memory/cost/control-plane panels and authentication are deferred to later slices (v0.4.0+ / post-RFC 0039). The prior milestone — v0.3.5 *Session-Scoped Memory* (RFC 0031 Phases 2–4 + epoch axis + RFC 0033 Phase 3 co-resident) — is ✅ Released ([GitHub Release](https://github.com/mkhomutov/Persatrix/releases/tag/v0.3.5), 2026-06-01). Next phase after v0.3.6 is v0.4.0 (team/hierarchy).
