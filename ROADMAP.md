...
> **Last updated**: 2026-06-03 (RFC 0048 Phase 1 / Slice 1 **implemented** — docs/closeout PR of [v0.3.6-plan.md](docs/v0.3.6-plan.md) Phase 1, the final PR of the [RFC 0048 Phase 1 PR plan](docs/rfcs/0048-phase1-pr-plan.md) (6 PRs + Docker enablement). The embedded same-origin web console (chat + channel timeline behind `--enable-ui`, off by default) now ships: PRs [#496](https://github.com/mkhomutov/Persatrix/pull/496)–[#498](https://github.com/mkhomutov/Persatrix/pull/498), [#501](https://github.com/mkhomutov/Persatrix/pull/501)–[#503](https://github.com/mkhomutov/Persatrix/pull/503) merged. This PR lands the [web console guide](docs/guides/web-console.md) (with the load-bearing localhost-only / reverse-proxy security note), the README "try it in the browser" entry, [MT-CONSOLE-001](docs/manual-tests/MT-CONSOLE-001.md), and the status closeout: **adds the RFC 0048 [§RFC Master Index](#rfc-master-index) row** (`⚠️ Partially Implemented (Phase 1 / Slice 1)`, target `v0.3.6 (Slice 1) + v0.4.0+ (Slices 2–5)`) and flips RFC 0048 `🚧 Implementing → ⚠️ Partially Implemented`. The v0.3.6 [§Version Map](#version-map) row stays `🚧 Planning` until the release-prep + tag phases (Phases 2–4) cut the version. Slices 2–5 (memory inspector, isolation verifier, cost/observability, control plane) remain deferred to v0.4.0+ / post-RFC 0039. The prior milestone v0.3.5 *Session-Scoped Memory* is ✅ Released ([GitHub Release](https://github.com/mkhomutov/Persatrix/releases/tag/v0.3.5), tagged 2026-06-01). Next phase after v0.3.6 is v0.4.0 (team/hierarchy).)
> **Current phase**: v0.3.6 (Web Console — RFC 0048 Phase 1 / Slice 1: Interactions) — 🚧 Phase 1 / Slice 1 implemented; release-prep + tag (Phases 2–4) pending (the [§Version Map](#version-map) row stays 🚧 Planning until the tag cuts the version)
> **Current milestone**: v0.3.6 Phase 1 / Slice 1 implemented — the six implementation PRs + Docker enablement have merged ([#496](https://github.com/mkhomutov/Persatrix/pull/496)–[#498](https://github.com/mkhomutov/Persatrix/pull/498), [#501](https://github.com/mkhomutov/Persatrix/pull/501)–[#503](https://github.com/mkhomutov/Persatrix/pull/503)); release-prep + tag (Phases 2–4 of [v0.3.6-plan.md](docs/v0.3.6-plan.md), codename *Web Console*) are next. One user-facing story: **open a URL and talk** — a newcomer runs the orchestrator with `--enable-ui`, opens `http://localhost:8080/ui`, picks a persona, chats with it, and watches a channel, with zero CLI knowledge. This is the first vertical slice of the [RFC 0048](docs/rfcs/0048-operator-tester-web-console.md) embedded same-origin web console (Svelte SPA served from the Go orchestrator via `embed.FS`, behind a per-panel feature-toggle model in `config/ui.yaml`); it is mostly a *render-over-existing-API* problem (chat / channels / agents already exist per RFC 0002 / 0011 / 0016), so the slice lands inside the v0.3.x line. The implementation workstream was the standalone [RFC 0048 Phase 1 PR plan](docs/rfcs/0048-phase1-pr-plan.md) (6 PRs + Docker enablement, now merged); this master plan wraps it with the still-pending release-prep + tag phases. Memory/cost/control-plane panels and authentication are deferred to later slices (v0.4.0+ / post-RFC 0039). The prior milestone — v0.3.5 *Session-Scoped Memory* (RFC 0031 Phases 2–4 + epoch axis + RFC 0033 Phase 3 co-resident) — is ✅ Released ([GitHub Release](https://github.com/mkhomutov/Persatrix/releases/tag/v0.3.5), 2026-06-01). Next phase after v0.3.6 is v0.4.0 (team/hierarchy).
...
| **v0.3.6** | Open a URL and talk to a persona — chat and watch a channel from the browser with zero CLI knowledge, via an embedded same-origin web console behind `--enable-ui` (RFC 0048 Slice 1: Interactions; later slices add memory/cost/control-plane panels) | 🚧 Planning |
