...
> **Last updated**: 2026-05-31 (v0.3.5 — RFC 0031 **epoch-axis closeout** (= [v0.3.5 plan](docs/v0.3.5-plan.md) Phase 3b, [epoch PR plan](docs/rfcs/0031-epoch-pr-plan.md) PR 6): `tests/integration/test_epoch_run_isolation.py` is the F-3 **structural**-isolation gate — a rerun under a fresh `PERSATRIX_EPOCH`, same room + same `--user`, inherits no episodes / relationship trust / person-facts (the residue a fresh channel name alone cannot reach). The `make reset` breadcrumb across [channels.md](docs/guides/channels.md) / [persona-agents.md](docs/guides/persona-agents.md) / [sessions.md](docs/guides/sessions.md) is now **reframed at epoch** as the everyday run-isolation tool (superseding the Phase 4 wording that left it the nuke "until epoch ships"); `make reset` is the whole-stack nuke — all epochs across all sessions. [ISSUE-0085](docs/issues/ISSUE-0085-epoch-axis-run-isolation.md) is **closed**; the epoch axis ([#472](https://github.com/mkhomutov/Persatrix/pull/472)–[#477](https://github.com/mkhomutov/Persatrix/pull/477)) shipped. Version Map v0.3.5 row stays `🚧 Planning` until the release tag.)
> **Current phase**: v0.3.5 (Session-Scoped Memory — RFC 0031 Phases 2–4) — 🚧 Planning
> **Current milestone**: v0.3.5 planning opened. The release makes **persona-memory recall session-scoped**: a run reads only its own session's rows (plus the always-visible `legacy` carve-out), so concurrent conversations don't bleed and an operator isolates a run by giving it a fresh session rather than nuking volumes with `make reset` — closing the **F-3** cross-run *recall* bleed at the root. Auto-isolating a *same-named* rerun is the **`epoch`** axis ([ISSUE-0085](docs/issues/ISSUE-0085-epoch-axis-run-isolation.md), Phase 3b) — the **structural** half of the F-3 fix, now **shipped** in v0.3.5 (strict-equality `epoch_id` across all five tiers; a fresh `PERSATRIX_EPOCH` / `--epoch` inherits nothing). Phase 2 makes default recall session-scoped (the F-3 recall closer, drafted in [`0031-phase2-pr-plan.md`](docs/rfcs/0031-phase2-pr-plan.md)); Phase 3 adds the `persatrix session …` operator CLI; Phase 4 ships the operator guide and closes [ISSUE-0051](docs/issues/ISSUE-0051-per-session-memory-namespacing-channels.md); Phase 3b adds the epoch axis and closes [ISSUE-0085](docs/issues/ISSUE-0085-epoch-axis-run-isolation.md). RFC 0033 Phase 3 (raw-ID rejection) is a conditional co-resident, gated on the `raw_id_usage` counter reading zero across dogfood — observed state, not a date. Next phase after v0.3.5 is v0.4.0 (team/hierarchy).
...
| **v0.3.5** | Persona-memory recall is session-scoped — a run reads only its own session (plus the always-visible `legacy` rows), so concurrent conversations don't bleed and a run is isolated by giving it a fresh session instead of `make reset` (closes the F-3 cross-run *recall* bleed; auto-isolating a same-named rerun is the deferred `epoch` axis) | 🚧 Planning — [v0.3.5 plan](docs/v0.3.5-plan.md) |
