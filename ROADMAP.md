...
> **Last updated**: 2026-08-02 (v0.3.12 — ✅ **all pre-tag gates green** (release-prep PRs 0–4; live MTs executed, two live-found fixes landed, the fresh-epoch tool-recall gap slips as [ISSUE-0118](docs/issues/ISSUE-0118-tool-recall-bypasses-epoch-session-scopes.md) and the cross-room person-identity legs as [ISSUE-0121](docs/issues/ISSUE-0121-crossroom-person-identity-legs-never-run-live.md), [ISSUE-0114](docs/issues/ISSUE-0114-per-channel-cascade-depth-override.md) cut to v0.3.13 — all maintainer calls); the mechanical `v0.3.12` tag + GitHub Release are the remaining Phase-4 step. See the [release-prep plan](docs/v0.3.12-release-prep-plan.md).)
> **Current phase**: v0.3.12 (Memory that travels — RFC 0037 classification + RFC 0049 P0–1 widenings + RFC 0039 P1–2 accounts) — ✅ Released (tag = Phase 4); v0.4.0 (Agent Organizations) follows
> **Current milestone**: v0.3.12 *Memory that travels* — ✅ Released ([plan](docs/v0.3.12-plan.md), opened 2026-07-25; [release-prep plan](docs/v0.3.12-release-prep-plan.md) — PRs 0–4 landed, tag = Phase 4). The headline user-facing story: **tell a persona something in one room and it knows it in every room it belongs to — without leaking what it learned in a confidential room.** Scope: **RFC 0037** (classification lattice + protection levels + the deterministic §D egress gate + §F recall filter — the keystone; Phases 2–3 projections/tripwire cuttable) + **RFC 0049 Phases 0–1** (the L1/fact-scope/topic-predicate widening amendments, shadow-first behind an RFC 0044 golden-trace measurement gate) **+ RFC 0039 Phases 1–2** (accounts/auth — `auth.mode` opt-in, console safe beyond localhost; bundled second workstream, independently shippable/cuttable). Fold-ins: ISSUE-0106 direction (b) ✅ + ISSUE-0114 (**cut to v0.3.13** — maintainer call at release-prep PR 2). Hard rule: **no widening merges before the 0037 gate is on `main`**. Prior milestone: v0.3.11 *Conversations that run themselves* — ✅ [Released 2026-07-22](https://github.com/mkhomutov/Persatrix/releases/tag/v0.3.11) ([v0.3.11-plan.md](docs/v0.3.11-plan.md)). v0.4.0 (Agent Organizations — deliberative reasoning) follows.
