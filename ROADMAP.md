...
> **Last updated**: 2026-06-01 (v0.3.5 **release-prep PR 4 — final pre-tag verification**. All release-prep PRs merged: plan ([#487](https://github.com/mkhomutov/Persatrix/pull/487)), MT execution report ([#488](https://github.com/mkhomutov/Persatrix/pull/488)), README/ROADMAP + sessions/epochs guide verification + release checklist ([#489](https://github.com/mkhomutov/Persatrix/pull/489)), version bump `0.3.4`→`0.3.5` + curated `[0.3.5]` changelog ([#490](https://github.com/mkhomutov/Persatrix/pull/490)). PR 4 (this PR) re-certified the gate sweep on the post-bump tip — `make validate` / doc-links / `cargo build` `0.3.5` / `cargo test` 153 / clippy / the session + epoch structural-isolation gates (20 passed, the primary release-blocker) / the bored-persona `cost-regression-gate` / the alias pricing + facts/recall Python slices / ruff / Python + Rust licenses all green, with CI green on the RC tip covering the full Go race-cover + integration suites — and flips the Version Map v0.3.5 row `🚧 Release prep` → `✅ Released`. PR 4 also installed Go 1.26.3 and ran the Go gates live (full race-cover suite, sanitizer parity, Go licenses) **and ran the full `MT-MEMORY-005` (V5) dementia arc live on Anthropic `claude-sonnet-4-6`** — so **both §4 live hard-blocks are cleared**: the Go alias cost-attribution gate (both tests PASS) and the qualitative dementia arc (all 5 legs Pass — Mira/age recalled, no-phone preference honored, commitment + paraphrase + pragmatist self-consistency — plus the multi-session no-bleed re-run holds, F-3 isolation confirmed). The `v0.3.5` git tag and GitHub Release ([Phase 6](docs/v0.3.5-plan.md#phase-6--tag-and-post-release-follow-up)) are the only remaining post-merge steps. Both v0.3.5 workstreams are ✅ Implemented: RFC 0031 Phases 2–4 + the epoch run-isolation axis ([#448](https://github.com/mkhomutov/Persatrix/pull/448)–[#478](https://github.com/mkhomutov/Persatrix/pull/478)) and RFC 0033 Phase 3 ([#481](https://github.com/mkhomutov/Persatrix/pull/481)–[#486](https://github.com/mkhomutov/Persatrix/pull/486)).)
> **Current phase**: v0.3.5 (Session-Scoped Memory — RFC 0031 Phases 2–4 + epoch axis) — ✅ Released
> **Current milestone**: v0.3.5 released. **Persona-memory recall is session-scoped** — a run reads only its own session's rows (plus the always-visible `legacy` carve-out), so concurrent conversations don't bleed, closing the **F-3** cross-run *recall* bleed at the root. A session is room continuity, not a clean slate; run/test isolation is the sibling **`epoch`** axis ([ISSUE-0085](docs/issues/ISSUE-0085-epoch-axis-run-isolation.md)) — the **structural** half of the F-3 fix: a fresh `PERSATRIX_EPOCH` / `--epoch`, same room + same `--user`, inherits nothing (strict-equality `epoch_id` across all five tiers), so an operator isolates a rerun by bumping the epoch rather than nuking volumes with `make reset`. RFC 0033 Phase 3 (raw-ID rejection, `schema_version` `"0.3"`) shipped as a conditional co-resident, completing RFC 0033: `models.aliases` is the single source of truth for model identity. Both §4 live hard-blocks (the Go alias cost-attribution gate + the `MT-MEMORY-005` dementia arc on Anthropic) are cleared live; the `v0.3.5` git tag + GitHub Release are the only remaining Phase 6 post-merge steps. Next phase is v0.4.0 (team/hierarchy).
