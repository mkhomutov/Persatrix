...
> **Last updated**: 2026-06-05 (v0.3.7 in progress. New: [v0.3.7 conversation test-findings PR plan](docs/v0.3.7-test-findings-pr-plan.md) — 5 findings from live group-channel probing (safety-snippet injection false-positive, conversation-window blindness, person-facts trapped in room-scoped notes, no shared roster, broken tool metric). Prior: RFC 0034 Phase 2 group working memory landed ([0034 Phase 2 PR plan](docs/rfcs/0034-phase2-pr-plan.md), closeout); RFC 0034 → ⚠️ Partially Implemented (Phases 1–2), Phase 3 (tuning + cache LRU) remains. RFC 0030 relevance Tier A + the peer-voice prompt are the remaining realism workstreams.)
