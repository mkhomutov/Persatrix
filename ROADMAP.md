...
> **Last updated**: 2026-06-01 (v0.3.5 **post-release follow-up** — `v0.3.5` tagged and the [GitHub Release "Session-Scoped Memory"](https://github.com/mkhomutov/Persatrix/releases/tag/v0.3.5) published 2026-06-01. This PR does the post-release ROADMAP hygiene: this `Last updated` refresh, and the Merged PR History backfill for the full v0.3.5 cycle ([#445](https://github.com/mkhomutov/Persatrix/pull/445) through [#491](https://github.com/mkhomutov/Persatrix/pull/491) — the table had stopped at [#444](https://github.com/mkhomutov/Persatrix/pull/444)) plus master plan + release-prep plan + release checklist status hygiene. The Version Map v0.3.5 row and "Current phase" / "Current milestone" already flipped to ✅ Released in release-prep PR 4 ([#491](https://github.com/mkhomutov/Persatrix/pull/491)), which re-certified the gate sweep on the post-bump tip and cleared both §4 live hard-blocks — the Go alias cost-attribution gate (both tests PASS) and the full `MT-MEMORY-005` (V5) dementia arc live on Anthropic `claude-sonnet-4-6` (all 5 legs Pass + the multi-session no-bleed re-run holds, F-3 isolation confirmed). Both v0.3.5 workstreams are ✅ Implemented: RFC 0031 Phases 2–4 + the epoch run-isolation axis ([#448](https://github.com/mkhomutov/Persatrix/pull/448)–[#478](https://github.com/mkhomutov/Persatrix/pull/478)) and RFC 0033 Phase 3 ([#481](https://github.com/mkhomutov/Persatrix/pull/481)–[#486](https://github.com/mkhomutov/Persatrix/pull/486)). Next phase is v0.4.0 (team/hierarchy).)
> **Current phase**: v0.3.5 (Session-Scoped Memory — RFC 0031 Phases 2–4 + epoch axis) — ✅ Released
> **Current milestone**: v0.3.5 released. **Persona-memory recall is session-scoped** — a run reads only its own session's rows (plus the always-visible `legacy` carve-out), so concurrent conversations don't bleed, closing the **F-3** cross-run *recall* bleed at the root. A session is room continuity, not a clean slate; run/test isolation is the sibling **`epoch`** axis ([ISSUE-0085](docs/issues/ISSUE-0085-epoch-axis-run-isolation.md)) — the **structural** half of the F-3 fix: a fresh `PERSATRIX_EPOCH` / `--epoch`, same room + same `--user`, inherits nothing (strict-equality `epoch_id` across all five tiers), so an operator isolates a rerun by bumping the epoch rather than nuking volumes with `make reset`. RFC 0033 Phase 3 (raw-ID rejection, `schema_version` `"0.3"`) shipped as a conditional co-resident, completing RFC 0033: `models.aliases` is the single source of truth for model identity. Both §4 live hard-blocks (the Go alias cost-attribution gate + the `MT-MEMORY-005` dementia arc on Anthropic) cleared live; the `v0.3.5` git tag and [GitHub Release](https://github.com/mkhomutov/Persatrix/releases/tag/v0.3.5) are published (Phase 6 post-release follow-up, 2026-06-01). Next phase is v0.4.0 (team/hierarchy).
