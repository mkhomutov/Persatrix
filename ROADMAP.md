...
> **Last updated**: 2026-09-22 (a persona knows its configured peers in every channel: a peer's relationship row now reads the same in every session, and only its interaction history stays per session — [ISSUE-0165](docs/issues/ISSUE-0165-relationship-hidden-outside-its-first-session.md), [#980](https://github.com/mkhomutov/Persatrix/pull/980). **EXP-001** is pre-registered ([#969](https://github.com/mkhomutov/Persatrix/pull/969)); its harness under `evaluators/` comes next. **v0.3.16** ✅ [Released](https://github.com/mkhomutov/Persatrix/releases/tag/v0.3.16); no next plan opens before **EXP-001** and the first strategy review.)
