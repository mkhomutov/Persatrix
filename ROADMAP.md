...
> **Last updated**: 2026-05-28 (v0.3.5 — RFC 0031 Phase 2 PR 1 opened: notes-tier `session_id` coverage (migration v9) + write-path threading, bringing the last persona-memory tier (after episodes/relationships v7 and facts v8) to session parity ahead of the recall-filtering PRs. RFC 0031 RFC-Master-Index status → `🚧 Implementing`, resuming from the Phase 1 `⚠️ Partially Implemented` pause (not a status regression, per Status Hygiene). Version Map v0.3.5 row stays `🚧 Planning` until the release tag; RFC 0033 stays `⚠️ Partially Implemented (Phases 1–2)` with Phase 3 (raw-ID rejection) observed-traffic gated to v0.3.5+.)
...
| [0031](docs/rfcs/0031-per-session-namespacing-channels.md) | Per-Session Namespacing for Channels and Persona Memory — first-class Session primitive scoping `channels.db` and per-persona `memory.db`, with an operator-visible `persatrix session …` CLI; F-3 root-cause fix, succeeds the `make reset` workaround from PR 6 of the v0.3.0 channel test-findings plan. Spawned from [ISSUE-0051](docs/issues/ISSUE-0051-per-session-memory-namespacing-channels.md). | v0.3.1 (Phase 1) + v0.3.5 (Phases 2–4) | 🚧 Implementing (Phase 1 shipped v0.3.1; Phase 2 recall filtering underway in v0.3.5 — PR 1 notes-tier `session_id` coverage (migration v9) + write-path threading opening, the last persona-memory tier after episodes/relationships v7 and facts v8 to gain a session dimension. Phases 2–4 re-sequenced from v0.3.4 to v0.3.5; see [v0.3.5 plan](docs/v0.3.5-plan.md) + [Phase 2 PR plan](docs/rfcs/0031-phase2-pr-plan.md)) |
