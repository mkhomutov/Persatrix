...
> **Last updated**: 2026-07-25 (v0.3.12 plan opened — *Memory that travels*: [RFC 0037](docs/rfcs/0037-memory-confidentiality-channel-classification.md) (keystone, all phases) + [RFC 0049](docs/rfcs/0049-memory-consolidation-gradient.md) P0–1 cross-channel widenings headline; [RFC 0039](docs/rfcs/0039-user-accounts-authentication.md) P1–2 accounts/auth bundled (cuttable); ISSUE-0106(b) + ISSUE-0114 fold-ins. See [v0.3.12-plan.md](docs/v0.3.12-plan.md).)
> **Current phase**: v0.3.12 🚧 Planning → implementation (next: RFC 0037 PR 1 + RFC 0039 PR 1 in parallel once the planning PR lands; RFC 0049 slices gated on the 0037 §D gate + §F filter); v0.4.0 (Agent Organizations) follows
> **Current milestone**: v0.3.12 *Memory that travels* — 📋 Planned ([plan](docs/v0.3.12-plan.md), opened 2026-07-25). The headline user-facing story: **tell a persona something in one room and it knows it in every room it belongs to — without leaking what it learned in a confidential room.** Scope: **RFC 0037** (classification lattice + protection levels + the deterministic §D egress gate + §F recall filter — the keystone; Phases 2–3 projections/tripwire cuttable) + **RFC 0049 Phases 0–1** (the L1/fact-scope/topic-predicate widening amendments, shadow-first behind an RFC 0044 golden-trace measurement gate) **+ RFC 0039 Phases 1–2** (accounts/auth — `auth.mode` opt-in, console safe beyond localhost; bundled second workstream, independently shippable/cuttable). Fold-ins: ISSUE-0106 direction (b) + ISSUE-0114 (cuttable). Hard rule: **no widening merges before the 0037 gate is on `main`**. Prior milestone: v0.3.11 *Conversations that run themselves* — ✅ [Released 2026-07-22](https://github.com/mkhomutov/Persatrix/releases/tag/v0.3.11) ([v0.3.11-plan.md](docs/v0.3.11-plan.md)). v0.4.0 (Agent Organizations — deliberative reasoning) follows.
...
| **v0.3.12 — Memory that travels** | Tell a persona something in one room and it knows it in every other room it belongs to — a project fact taught in a DM is known in the standup, raw experience from other rooms is available (room-first-ranked) — **without leaking what it learned in a confidential room** (RFC 0037 classification + protection levels + deterministic egress gate; RFC 0049 Phases 0–1 widenings, shadow-gated). Plus verified accounts: with `auth.mode: enabled` the web console/REST surface is safe beyond localhost (RFC 0039 Phases 1–2, bundled/cuttable). | 📋 Planned — [plan opened 2026-07-25](docs/v0.3.12-plan.md) |
...
| [0037](docs/rfcs/0037-memory-confidentiality-channel-classification.md) | Memory Confidentiality & Channel Classification | The confidentiality half of the cross-channel-experience release: classification lattice + protection levels + the deterministic memory-injection egress gate, so a persona learns from a confidential channel without leaking it. **Pulled forward to v0.3.12** (with RFC 0049) per 2026-07-15, restoring its own "why this is a v0.3.x RFC" intent. | v0.3.12 | 🚧 Implementing — [PR plan](docs/rfcs/0037-pr-plan.md) |
| [0038](docs/rfcs/0038-concurrent-context-awareness-relay.md) | Concurrent-Context Awareness & Cross-Channel Relay | §B single-channel-turn guard carved forward into the RFC 0037 v0.3.12 plan (per 0037 Decision #3, 2026-07-19); §C–§E (contexts/awareness/relay) stay v0.4.0 | 📋 Proposed |
| [0039](docs/rfcs/0039-user-accounts-authentication.md) | User Accounts & Authentication | Phase 3 — account administration REST API + failed-login lockout (the Phases 1–2 foundation ships in v0.3.12 — 🚧 Implementing, [PR plan](docs/rfcs/0039-pr-plan.md)) | 📋 Proposed |
