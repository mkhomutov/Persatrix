...
> **Last updated**: 2026-06-06 (v0.3.7 in progress. F-7 Option D (person identity on the cross-room relationship tier, [ISSUE-0093](docs/issues/ISSUE-0093-person-identity-cross-room-tier.md)) is at **D2** — `store_note(contact:*)` write-through to `upsert_identity` + cross-room identity render (immediacy); D1 storage foundation prior (#553). D3 (backfill + Option-A retirement) remains. Earlier: the [v0.3.7 conversation test-findings PR plan](docs/v0.3.7-test-findings-pr-plan.md) is **complete** (#540–#548); RFC 0034 Phase 2 group working memory landed (RFC 0034 → ⚠️ Partially Implemented, Phase 3 remains). RFC 0030 relevance Tier A + the peer-voice prompt are the remaining realism workstreams.)
