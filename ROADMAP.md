...
> **Last updated**: 2026-05-10 (release-prep PR 2 — README v0.3.0 surface + Known Limitations + roadmap row flipped to 🚧 Release prep; persona/channels guide callouts for RFCs 0011 amendment / 0020 / 0021 / 0022; persona-runtime + memory-architecture diagram refresh for the interaction-lifecycle stages and the `MemoryFacade` boundary + per-channel scoping; `docs/v0.3.0-release-checklist.md` created from the v0.2.3 template — gate ledger for the v0.3.0 release tag). Prior update 2026-05-10 (release-prep PR 1 — manual-test execution report; 26 ✅ + 1 ⚠️ + 1 🔵 Baseline; release gate cleared). Prior update 2026-05-10 (RFC 0008 PR 6 close-out — RFC `✅ Implemented`). Prior update 2026-05-10 (v0.3.0 release-prep plan opened — Phase 3 PR; same PR walked back the RFC 0008 OQ #12 calibration-window gate per [v0.3.0 release-prep plan §RFC 0008 OQ #12 walkback](docs/v0.3.0-release-prep-plan.md#rfc-0008-oq-12-walkback)). Prior update 2026-05-09 (RFC 0009 PR 4 close-out — Phases 1–2 status flips to `⚠️ Partially Implemented`; `Implementation Notes (v0.3.0)` appendix records the seven divergences from the PR plan; PR 1/1b/1c/2/3 review follow-ups dispatched: typed `depthMarker` sentinel keys on reflect type rather than string content (PR #233 review NTH-3), `looksLikeSHA256` swap to `encoding/hex.DecodeString` (PR #233 review SF-7), `Emit` write-alloc reduction via two-call `Write`+`WriteByte` (PR #233 review NTH-6), generic-secret pattern consumes optional trailing quote (PR #233 review NTH-7), four new default redactor patterns: `github-token` (classic + fine-grained `gh[opusr]_…` / `github_pat_…`), `slack-token` (`xox[baprs]-…`), `stripe-key` (`[sp]k_live_…`), `gcp-private-key` (PEM BEGIN marker), exported `security.VerifyChain(path)` re-walks the per-event sha256 chain for external auditors / `persatrix audit verify` CLI (PR #233 review NTH-1), unexported `withTickerSeam` test seam removes wall-clock dependency from `TestBatchFlush_TimerTrigger` (PR #233 review SF-1) plus three coverage-gap tests (`TestWithClock_DrivesEmitTimestamps`, `TestEmit_DoesNotMutateCallerDetail`, `TestStartup_NoRedactor_TruncatedTailLeaksRaw`), `TestRedactStruct_PointerCycle_NonStringPointee_ZerosField` pins the type-erasing pointer-cap path (PR #233 review SF-5), `TestRedactStruct_OpaqueByUnexportedArray` covers `[16]byte` carriers like `uuid.UUID` before PR 3 routes tool-call argument structs through `RedactStruct` (PR #236 review L-1), `RedactStruct` representative-detail benchmark informs v0.4.0 sync.Pool decision (PR #233 review NTH-8), PR #234 N-1/N-2/N-3 nit sweep (`fmt.Sprintf` → `strconv.Itoa` in `agent_handlers.go`; `# DOCUMENTATION ONLY` header on `config/observability/audit.yaml`; `defer auditor.Close()` → `t.Cleanup` in `TestAuditLogger_RedactsBearerTokenInDetail`), PR #236 L-2/L-3 doc tightening (`isOpaqueStruct` clarified re `time.Time`'s three unexported fields; `audit_adapter.go::RecordEvent` re-attributes `context.Background()` to the post-`mu.Unlock` defer ordering), and PR #236 L-5 `AuditMetricsAbsent` alert sibling in `docs/observability.md §13.1` covers the metrics-init-failure case where `AuditSecurityClassSilent`'s `rate(...)==0` is masked by missing-series semantics. Calibration ledger filled in: 4 of 6 PRs landed >2× estimate (PR 2 at 6.7×); v0.4.0 RFC 0009 PR plan should bump the working factor to 3.5× and split aggressively when crossing ~400 estimated lines. RFC 0009 row in [RFC Master Index](#rfc-master-index) flips to `⚠️ Partially Implemented (Phases 1–2)`. `v0.3.0-plan.md` master rows 5 and 6 flip to ✅ Merged (RFC 0011 row 6 was stale at "PR 8 open" because PR 8 #304 was self-referential; corrected with all eight PR links rolled up). RFC 0011 PR 5 follow-up (2026-05-07) — on-startup catch-up fetch resolves [OQ #8](docs/rfcs/0011-channels-bridges.md#open-questions): new `agents/channel_catchup.py::replay_channel_history` issues `GET /api/v1/channels` → membership filter → `GET /api/v1/channels/{id}/messages?limit=50` per channel; orchestrator returns newest-first, fetcher reverses before replay; messages are dispatched through `agent.on_event` as `CHANNEL_MESSAGE` events with `metadata["replay_mode"] = True`. Replay-mode short-circuit lands in `agents/persona_runtime/action_loop.py::_on_event_inner` after `_sanitize_inbound_event` and before `evaluate_response_gate` — replay events ingest memory via `_store_event_episode` (skipped when `sender_id == agent_id` for defense-in-depth) and return `[DO_NOTHING]` without invoking the LLM or producing `SEND_CHANNEL_MESSAGE`. New `channel.messages.replayed{channel_id}` counter (`replay_attrs(channel_id=…)`) — separate from `channel.messages.gated` so a startup catch-up burst does not mask a real gate-suppression spike. `AgentServer.start` adds `_replay_channel_history` (called after `_self_register`, reuses the shared `aiohttp.ClientSession` opened for the publisher; iterates only over `_LLMPersonaAgent` instances; best-effort wrapper logs and continues so a flapping orchestrator cannot keep startup from completing). Watermark + per-tick recovery still deferred per OQ #8 — v0.3.0 ships at-most-once with on-startup last-N as the only catch-up trigger. Coverage: `tests/unit/python/test_replay_mode_action_loop.py` (8 tests pinning short-circuit + sanitize-then-replay ordering + self-sender ingest skip + replayed-counter behaviour), `tests/unit/python/test_channel_catchup.py` (10 tests on a loopback aiohttp orchestrator covering empty list, membership filter, oldest-first replay, default-50 + custom limit, list/history failure paths, respond-policy propagation, DM channels), `tests/unit/python/test_server_catchup_wiring.py` (4 tests on persona-only filter, shared-session contract, exception-doesn't-break-startup). RFC 0011 PR 5 fully closed; PRs 6/7/8 pending. RFC 0007 retargeted from v0.3.0 to v0.4.0: conditional/looped workflow control flow is workflow-engine plumbing that pairs with v0.4.0's sub-agent spawning (RFC 0010) and skill registry (RFC 0014) — iterative refinement and conditional branching on child-agent outputs are the cases that earn the rent. v0.3.0 stays focused on its user-facing promise (agents *talk*, negotiate, form opinions). v0.3.0 RFC count drops from six to five (0008, 0009 P1–2, 0011 internal, 0020, 0021 P1); v0.4.0 picks up RFC 0007 alongside 0009 P3–4, 0010, 0012, 0014, 0021 P2–4, 0027. RFC 0008 dep on 0007 still satisfied at v0.4.0-start since 0008 ships fully in v0.3.0. RFC 0021 P1 PR 1 merged as #256: `Clock` seam + temporal rendering pure functions — `agents/clock.py` (`Clock` Protocol, `WallClock` returning epoch seconds, `FrozenClock` test impl) + `agents/temporal/rendering.py` (`format_relative` / `format_duration` / `format_part_of_day` per RFC 0021 §B/§D); no persona-runtime wiring yet (lands in PR 2). Five low-severity follow-ups captured as ISSUE-0037 through ISSUE-0041 (FrozenClock set-vs-advance asymmetry, pytest.raises(Exception) too broad, WallClock empty-tz silent UTC fallback, `agents.clock.Clock` vs. `agents.memory.interactions.Clock` naming collision, `Clock.now()` return-type doc nit in 0021-pr-plan.md). RFC 0021 P1 PR 2 merged as #260: now-anchor + episode/relationship recency rendering wired into the persona prompt path. RFC 0021 P1 PR 3 merged as #261: review follow-ups + RFC Phase-1 close — closes ISSUE-0037 through ISSUE-0041; four follow-ups captured as ISSUE-0042 through ISSUE-0045 (empty-tz fallback test weak, v0.3.0-plan row 3 PR-date rollup missing, ROADMAP Current-milestone prose stale, FrozenClock.set docstring missing symmetrize rationale). RFC 0011 PR 4b merged as #252: channels response gate (`when_mentioned` / `always` / `never` per persona policy) + DELETE endpoints for channels and members + `channel.messages.gated{policy}` counter; closes the canonical RFC 0011 PR 4 (agent-delivery) scope across the 4a / 4b sub-PR split. RFC 0009 PR 3 merged as #253: InputSanitizer + ContextItem + `<external_data>` envelope — Go canonical pattern registry + Python mirror + `cmd/genpatterns` tool; pattern parity enforced by `make generate-sanitizer-patterns-check`; `http_request`/`file_read` tool results wrapped at the LLM-content boundary; `external-data-handling` prompt fragment appended unconditionally; deep-review tag-escape hardening: F1/M1 close/open-tag escape via `_EXTERNAL_DATA_TAG_RE`, L1 whitespace-tolerant matching, F2 `ContextSourceUser` doc + pin test, F3 `WithSanitizerLogger` best-effort drop surfacing, F4/F5/F7 low nits; all 8 findings resolved inline. RFC 0011 PR 4a-ii-β-2 merged as #251: chat-as-DM rewrite per the [RFC 0011 amendment](docs/rfcs/0011-amendment-chat-as-dm.md) — `POST /api/v1/agents/{id}/chat` migrated from the legacy `SendChatMessage` gRPC round-trip to a channels-based publish-and-await façade. New `internal/channels/waiter.go` (`replyWaiter` — O(1) correlation table keyed on `(channel_id, sender_id)`, buffered cap-1 reply channel, `ErrWaiterAlreadyRegistered` sentinel, cancel-guard against fresh-registration clobber) plus `ChannelRouter.PublishAndAwait` (registers waiter before `Publish` to close the race window, `defer cancel`, `select` on reply / timer / `ctx.Done()`); `ErrChatTimeout` sentinel mapped to HTTP 504. `internal/server/chat_handler.go` rewritten — validates `agent_id` against `resourceIDRegex`, bounds message ≤ 4 000 chars, defaults `user_id` to `"local"`, clamps timeout to `[1s, 300s]`, mints `session_id` when absent; builds `ChannelMessage{ChannelID: dm.ID, SenderID: userID, Content, Mentions: [agentID]}` and delegates to `channelRouter.PublishAndAwait`; `GetOrCreateDM` errors discriminated 400 (caller-id violations) vs 500 (storage faults) per PR #251 review M-1. **JSON contract preserved** (`chatRequest` / `chatResponse` shape unchanged — Rust CLI and existing REST clients unaffected). `chatExecutor` wiring in `cmd/orchestrator/main.go` and `agents/server_servicers.py::SendChatMessage` left intact for binary-upgrade safety (now dead code; cleanup tracked in ISSUE-0035). Coverage: `internal/channels/waiter_test.go` (7 tests) + `router_test.go` 5 new `PublishAndAwait` tests + `chat_handler_test.go` rewrite (13 fixture-based tests using real on-disk SQLite + `NoopDispatcher`). Four follow-up issues captured (ISSUE-0033 single-shot waiter multi-message reply, ISSUE-0034 fanout warn noise on non-agent recipient, ISSUE-0035 `chatExecutor` dead-but-wired cleanup, ISSUE-0036 `doc_links.py` prefer `git ls-files`). RFC 0011 PR 4a-ii-β-1 merged as #250: real Go gRPC `MessageDispatcher` (`internal/channels/grpc_dispatcher.go`) — registry-aware `ReceiveChannelMessage` invocation replacing the `NoopDispatcher{}` placeholder in `cmd/orchestrator/channels.go`; `ChannelType` derived from channel-id prefix; RFC3339Nano timestamps; `ErrAgentNotFound` warns and no-ops, non-`Healthy`/empty-`Address` returns `ErrAgentNotReady` (at-most-once / best-effort). Python `_handle_send_channel_message` rewired from in-process `EventDispatcher` to `POST /api/v1/channels/{id}/messages` via new `HTTPChannelPublisher` (10s timeout) when `channel_id` is set; in-process cascade preserved as fallback for chat-reply path until β-2. `ActionExecutor` extracted from `agents/dispatch.py` to `agents/action_executor.py` (re-exported) to keep both modules under file-size limit. Eight follow-up issues captured (ISSUE-0024 through ISSUE-0032 in `docs/issues/INDEX.md`) covering cross-process integration test, conditional `HTTPChannelPublisher` wiring, result-dict asymmetry, `selectChannelDispatcher` test gap, gRPC connection pooling, RPC-error test gap, nil-registry startup-log gap, and channel-publish OTEL spans. RFC 0011 PR 4a-ii-α merged as #249: hard rename `EventType.MESSAGE_RECEIVED` → `CHANNEL_MESSAGE` and `ActionType.SEND_MESSAGE` → `SEND_CHANNEL_MESSAGE` across all Python producers (chat ingest, persona-runtime response gate, dispatch executor, action validators, prompt assembly, state persistence, memory routing, all unit and integration tests, glossary + spec docs); v0.2 enum aliases dropped; `ActionExecutor` result dict now carries `"action_type": "send_channel_message"`; PR #231 review SF-3 closed (`PublishMessage` validates every `msg.Mentions` entry via `validateParticipantID` before INSERT). RFC 0011 PR 4a-i merged as #248: `ReceiveChannelMessage` real handler replacing PR 3 stub — additive `EventType.CHANNEL_MESSAGE` + `ActionType.SEND_CHANNEL_MESSAGE` enum members (old names retained); additive top-level `AgentEvent.thread_id`; proto-bound validation (`channel_type`/`channel_id` prefix, content 4 000-codepoint cap, mentions 10-entry cap, thread_id 128-char cap, participant-id pattern); strong-ref `_pending_dispatches` task set + `_MAX_PENDING_DISPATCHES=1000` backpressure cap; `CHANNEL_MESSAGE` wired into persona-runtime multi-turn episode path + `_format_event` + `action_loop` memory-query extraction; `agents/channel_validation.py` extracted; ISSUE-0021 ref updated. PR scope: no chat-path changes; no dispatch executor; no response gate — those land in PR 4a-ii / PR 4b. Storage Architecture Roadmap proposed — [storage-architecture-roadmap.md](docs/storage-architecture-roadmap.md) sets the personal/society storage boundary policy and the vectors-as-accelerator-only policy; spawns one v0.4.0 RFC (SA-1 — storage split, sequenced before RFC 0028 implementation) plus SA-2/SA-3/SA-4 v0.3.x addenda; RFC index extended with a Companion Discussion Documents section. RFC 0028 Agent Decision Policy Engine proposed for v0.4.0, covering manual, semi-automated, and automated decision modes for task agents and persona agents, plus integration seams with RFCs 0005/0008/0009/0011/0020/0021. RFC 0009 PR 2 merged as #244: per-agent REST/gRPC RateLimiter — sliding-window timestamps with bounded-LRU agent map (PR #232 review SF-1) + CircuitBreaker that quarantines agents after threshold violations per RFC 0009 §H + REST middleware (HTTP 429 + Retry-After) + gRPC unary interceptor (ResourceExhausted / PermissionDenied) + operator-facing `POST /api/v1/agents/{id}/unquarantine` + `SECURITY_RATE_LIMIT_*` env-var bootstrap with startup-WARN + `rate_limit.disabled` audit event when opted out. Adds five audit-event constants (`rate_limit.violated`, `rate_limit.unauthenticated_caller`, `rate_limit.agent_evicted`, `rate_limit.disabled`, `agent.quarantined`/`agent.unquarantined`). Memory Quality Roadmap ratified — [memory-quality-roadmap.md](docs/memory-quality-roadmap.md) flipped `📋 Proposed`; RFC 0026 Declarative Facts Tier scaffolded for v0.3.x; RFC 0027 Reflection-Driven Consolidation scaffolded for v0.4.0 — supersedes draft RFC 0025; MT-MEMORY-005 dementia-test scaffolded; RFC index extended with 0023–0027; v0.3.0-plan §Memory Quality Follow-Ups added. RFC 0009 PR 1c merged as #236: audit-hardening — RedactStruct unexported-non-primitive bail-out + three audit OTEL instruments. RFC 0011 PR 1 merged as #231: channel store + SQLite migration + schema rewrite per RFC 0011 §A/§B; RFC 0011 PR 2 merged as #245: channels REST endpoints + ChannelRouter + config reconciliation (§B coexistence rules; loud-fail on membership divergence) + SF-2/SF-4 closures from PR #231 review (canonical group-ID authority + nullable `name` + partial unique index) + startup-WARN trust-boundary notice for unauthenticated REST surface; RFC 0020 PR 4 merged as #229: summarization-on-close + closing-state janitor on `on_tick` cadence + `record_interaction` moved to interaction-close path); RFC 0011 PR 3 merged as #246: `ChannelMessageEvent` + `ReceiveChannelMessage` RPC added to `AgentService` proto; v0.2-era `ChannelService` + `agent_message.proto` deleted; `agents/generated/` regenerated with relative imports; stub `ReceiveChannelMessage` returns `TaskAck(success=False)`; Makefile `proto-python` target auto-rewrites absolute imports to relative form (ISSUE-0016 ✅ closed); channels-db parent-dir auto-create on startup (ISSUE-0012 ✅ closed); `channelFallbackWarnOnce` reset helper for test isolation (ISSUE-0009 ✅ closed); six follow-up issues captured as ISSUE-0018 through ISSUE-0023  
> **Current phase**: v0.3.0 (Agent Conversations — RFCs 0008, 0009 P1–2, 0011 internal, 0020, 0021 P1, 0022) — 🚧 Release prep  
> **Current milestone**: release-prep PR 2 (this PR) — README v0.3.0 section + Known Limitations + roadmap row → 🚧 Release prep, ROADMAP Current-milestone trim, persona/channels guide callouts, persona-runtime + memory-architecture diagram refresh for the interaction-lifecycle and `MemoryFacade` boundaries, and the new `docs/v0.3.0-release-checklist.md` (from the v0.2.3 template). Release-prep PR 1 (manual-test execution report) merged as [#314](https://github.com/mkhomutov/Persatrix/pull/314) on 2026-05-10 — 26 ✅ + 1 ⚠️ + 1 🔵 Baseline (MT-MEMORY-005) — gate cleared. Next PR: `feature/v030-release-prep-version-bump` (release-prep PR 3 — version bump to 0.3.0 + curated `[0.3.0]` changelog section).
...
| TBD | docs(rfc0008): PR 6 — review follow-ups absorbed + RFC close | 0008 (6/6) | 2026-05-10 |
