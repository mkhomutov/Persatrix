...
> **Last updated**: 2026-09-18 (the test that claimed a persona turn never reads with `sessions="*"` now drives a real one: on the prompt path `"*"` reaches only the facts tier, only under `cross_room: live`, and only through the RFC 0037 §D gate; test docstrings no longer call `"*"` a CLI/debug sentinel. **EXP-001** is pre-registered ([#969](https://github.com/mkhomutov/Persatrix/pull/969)); its harness under `evaluators/` comes next. **v0.3.16** ✅ [Released](https://github.com/mkhomutov/Persatrix/releases/tag/v0.3.16); no next plan opens before **EXP-001** and the first strategy review.)
