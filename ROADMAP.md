...
> **Last updated**: 2026-05-27 (v0.3.4 post-release follow-up — `v0.3.4` tagged and the [GitHub Release "Any Model, Any Provider"](https://github.com/mkhomutov/Persatrix/releases/tag/v0.3.4) published. The README offline/Ollama "not in the latest tagged release" caveats are dropped (v0.3.4 is now the latest release and ships both); ROADMAP `Last updated` refresh; Merged PR History backfill for the full v0.3.4 release-prep cycle ([#438](https://github.com/mkhomutov/Persatrix/pull/438) through [#444](https://github.com/mkhomutov/Persatrix/pull/444) — the table had stopped at [#437](https://github.com/mkhomutov/Persatrix/pull/437)); master plan + release-prep plan + release checklist status hygiene. The Version Map v0.3.4 row and "Current phase" / "Current milestone" already flipped to ✅ Released in release-prep PR 4 ([#444](https://github.com/mkhomutov/Persatrix/pull/444)). RFC 0033 stays `⚠️ Partially Implemented (Phases 1–2)`; Phase 3 (raw-ID rejection, `_infer_provider` retirement) → v0.3.5+, observed-traffic gated.)
