...
> **Last updated**: 2026-05-27 (v0.3.4 release-prep in progress: Version Map v0.3.4 → `🚧 Release prep`. Release-prep PR 1 (MT execution report, [#439](https://github.com/mkhomutov/Persatrix/pull/439)) merged. The release-prep docs PR also folds in a provider-parity design change: provider selection is now **purely config/alias-driven** — the `PERSATRIX_OFFLINE` / `PERSATRIX_OLLAMA` force-knobs (#422/#423) are removed and `mock` / `ollama` are first-class alias providers selected the same standard way as `anthropic` / `openai`, so every `make demo-*` (incl. the new `make demo-openai`) is just an alias config the overlay mounts. The base compose now plumbs every provider key optionally (no single-vendor startup guard — closes MT F-5). RFC 0033 stays `⚠️ Partially Implemented (Phases 1–2)`; Phase 3 (raw-ID rejection, `_infer_provider` retirement) → v0.3.5+, observed-traffic gated. Prior: 2026-05-26 — RFC 0033 Phases-1–2 closeout merged; RFC 0033 → `⚠️ Partially Implemented (Phases 1–2)`.)
> **Current phase**: v0.3.4 (Any Model, Any Provider — RFC 0033 Phases 1–2) — 🚧 Release prep
> **Current milestone**: v0.3.4 release-prep (master-plan Phase 4). PR 1 (MT execution report) merged; the docs PR ships the provider guide, provider-neutral onboarding (config-driven demos, `make demo-openai`, the knob-free provider-selection refactor), README/ROADMAP refresh, and the v0.3.4 release checklist. Next: version bump + changelog, then final pre-tag verification. RFC 0031 Phases 2–4 (per-session namespacing, F-3 fix) re-home to v0.3.5.
