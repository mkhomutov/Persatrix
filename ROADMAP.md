...
> **Last updated**: 2026-09-24 (code comments and the channel schema now say a governed channel's `reasoning.mode` defaults to `bid`, as it has since RFC 0051 PR 6, instead of `off` — [#990](https://github.com/mkhomutov/Persatrix/pull/990) — and that `reasoning.revise >= 1` runs the reflexion loop under `mode: plan` — [#989](https://github.com/mkhomutov/Persatrix/pull/989). **EXP-001** harness PRs 1, 2, 3a and 3b merged ([harness plan](docs/experiments/EXP-001-harness.md)). **v0.3.16** ✅ [Released](https://github.com/mkhomutov/Persatrix/releases/tag/v0.3.16); no next plan opens before **EXP-001** and the first strategy review.)
