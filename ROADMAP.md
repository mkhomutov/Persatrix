...
| **v0.3.8** | Give a group of personas a problem and watch the brainstorm **converge, terminate, and produce a readable synthesized outcome** — no pile-on, bounded cost, a real result (RFC 0030 relevance Tier B salience + `chair` disposition + natural-language addressing + reply budget + cost ceiling + end-of-interaction + interaction-summary surface), plus per-channel governance knobs you can edit live from the CLI or web console (RFC 0050) | 🚧 Release prep |
