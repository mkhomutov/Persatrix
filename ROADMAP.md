...
> **Last updated**: 2026-05-30 (v0.3.5 — RFC 0031 **Phase 3 operator CLI closeout** (Phase 3 PR plan, PR 5): the `persatrix session …` surface is complete — `/api/v1/sessions` REST registry + `new`/`list`/`archive`/`use`/`current`/`--activate` + the `--session` override — so all three session-resolution mechanisms (env → file → flag) are wired. The end-to-end operator-surface lifecycle is pinned by `tests/integration/test_session_operator_surface.py` (opt-in `-m requires_orchestrator`). The `persatrix memory recall --all-sessions` debug verb is carved out as a follow-up ([ISSUE-0086](docs/issues/ISSUE-0086-operator-all-sessions-recall-verb.md)) — the `sessions="*"` sentinel keeps no operator entry point. RFC 0031 RFC-Master-Index Phase-3 prose advanced to *shipped*; Phase 4 (operator docs + [ISSUE-0051](docs/issues/ISSUE-0051-per-session-memory-namespacing-channels.md) closeout) remains. Version Map v0.3.5 row stays `🚧 Planning` until the release tag.)
> **Current phase**: v0.3.5 (Session Isolation — RFC 0031 Phases 2–4) — 🚧 Planning
> **Current milestone**: v0.3.5 planning opened. The release closes **F-3 cross-run state bleed** at the root: every run becomes auto-isolated so a rerun reusing the same channel name + `--user` no longer inherits persona memory or channel history from a prior run — superseding the `make reset` operator workaround. Phase 2 makes default recall session-scoped (the F-3 closer, drafted in [`0031-phase2-pr-plan.md`](docs/rfcs/0031-phase2-pr-plan.md)); Phase 3 adds the `persatrix session …` operator CLI; Phase 4 ships the operator guide and closes [ISSUE-0051](docs/issues/ISSUE-0051-per-session-memory-namespacing-channels.md). RFC 0033 Phase 3 (raw-ID rejection) is a conditional co-resident, gated on the `raw_id_usage` counter reading zero across dogfood — observed state, not a date. Next phase after v0.3.5 is v0.4.0 (team/hierarchy).
...
| **v0.3.5** | Every run is auto-isolated — persona memory and channel history no longer bleed across runs that reuse a channel name + user identity (F-3 root-cause fix; supersedes the `make reset` workaround) | 🚧 Planning — [v0.3.5 plan](docs/v0.3.5-plan.md) |
...
| [0031](docs/rfcs/0031-per-session-namespacing-channels.md) | Per-Session Namespacing for Channels and Persona Memory — first-class Session primitive scoping `channels.db` and per-persona `memory.db`, with an operator-visible `persatrix session …` CLI; F-3 root-cause fix, succeeds the `make reset` workaround from PR 6 of the v0.3.0 channel test-findings plan. Spawned from [ISSUE-0051](docs/issues/ISSUE-0051-per-session-memory-namespacing-channels.md). | v0.3.1 (P1) + v0.3.5 (P2) + v0.3.x (P3–4) | 🚧 Implementing (Phase 1 shipped v0.3.1; **Phase 2 recall filtering shipped v0.3.5** — session-scoped default recall across all four persona-memory tiers (episodes/relationships v7, facts v8, notes v9), closing F-3 cross-run state bleed at the root. **Phase 3 operator CLI shipped v0.3.5** — the `persatrix session …` surface (`/api/v1/sessions` REST registry + `new`/`list`/`archive`/`use`/`current`/`--activate` + the `--session` override) is complete; all three session-resolution mechanisms (env → file → flag) now wired ([Phase 3 PR plan](docs/rfcs/0031-phase3-pr-plan.md), PRs 1–5). Phase 4 (operator docs/[ISSUE-0051](docs/issues/ISSUE-0051-per-session-memory-namespacing-channels.md) closeout) remains; see [v0.3.5 plan](docs/v0.3.5-plan.md)) |
