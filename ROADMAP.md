...
> **Last updated**: 2026-05-29 (v0.3.5 — RFC 0031 Phase 2 closeout (PR 6): recall filtering shipped across all four persona-memory tiers (`episodes`/`relationships` v7, `facts` v8, `notes` v9), closing **F-3 cross-run state bleed** at the root. The two correctness gaps recall filtering exposed in the multi-persona process — process-global session id ([ISSUE-0081](docs/issues/ISSUE-0081-session-id-process-global-not-task-local.md)) and the orchestrator never emitting a per-request `persatrix-session` ([ISSUE-0082](docs/issues/ISSUE-0082-orchestrator-per-request-session-principal-emission.md) Part 1) — landed in the same release. RFC 0031 RFC-Master-Index status `🚧 Implementing` → `⚠️ Partially Implemented` (Phases 3–4 operator CLI + docs remain); target line → `v0.3.1 (P1) + v0.3.5 (P2) + v0.3.x (P3–4)`. Version Map v0.3.5 row stays `🚧 Planning` until the release tag; RFC 0033 stays `⚠️ Partially Implemented (Phases 1–2)` with Phase 3 (raw-ID rejection) observed-traffic gated to v0.3.5+.)
