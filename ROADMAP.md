...
> **Last updated**: 2026-06-05 (v0.3.7 in progress. The [v0.3.7 conversation test-findings PR plan](docs/v0.3.7-test-findings-pr-plan.md) — all findings from live group-channel probing — is **complete** (injection false-positive, window self-awareness + sizing, cross-conversation memory honesty + cross-room person recall, sender-aware carve-out, tool metric, and the channel roster; #540–#548). Prior: RFC 0034 Phase 2 group working memory landed; RFC 0034 → ⚠️ Partially Implemented (Phases 1–2), Phase 3 (tuning + cache LRU) remains. RFC 0030 relevance Tier A + the peer-voice prompt are the remaining realism workstreams.)
