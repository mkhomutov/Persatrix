...
> **Last updated**: 2026-05-21 (v0.3.3 Phase 2 — RFC 0024 PR 1 open: EventLoop substrate + WakeEvent taxonomy + SyncDispatchHandle. `TickScheduler` is now a thin adapter over `agents/event_loop.py`; `EventDispatcher.dispatch` routes scheduler-registered targets through `event_loop.enqueue(InboundEventWake(..., handle=...))` preserving the three v0.3.2 dispatch contracts (return value, await-serialisation, queue-mediated ordering). RFC 0024 status flipped to `🚧 Implementing` on first implementation PR open per [v0.3.3-plan §ROADMAP hygiene](docs/v0.3.3-plan.md#roadmap-hygiene); Version Map v0.3.3 row stays `🚧 Planning` until release-prep close.)
> **Current phase**: v0.3.3 (Idle Truly Idle — RFC 0024 Phases 1–4) — 🚧 Implementing
> **Current milestone**: v0.3.3 Phase 2 PR 1 open — `feature/v033-rfc0024-event-loop` introduces `agents/event_loop.py` (EventLoop + WakeEvent taxonomy + SyncDispatchHandle) and rewires `TickScheduler` / `EventDispatcher.dispatch` over the new substrate. Legacy `tick_interval_seconds` continues to work via a synthesised `ScheduledWake(timer_id="legacy_tick")` at the same observable cadence as v0.3.2. RFC 0017 §F guard documented as vestigial with the RFC 0024 Phase 5/6 deletion path. Previous milestone: v0.3.3 Phase 1 RFC 0024 PR plan merged ([#405](https://github.com/mkhomutov/Persatrix/pull/405)).
...
| **v0.3.3** | A persona with no scheduled work and no inbound traffic costs nothing — no SQLite recall query, no `_inject_memory_context`, no provider activity, no wallet lease requested. The persona autonomy loop is structurally event-driven (inbound RPC / salience-triggered memory write / scheduled timer), not polled. | 🚧 Planning |
...
| [0024](docs/rfcs/0024-event-driven-scheduling.md) | Event-Driven Agent Scheduling — wake the persona on inbound RPC, salience-triggered memory write, or scheduled timer instead of a fixed-interval poll; a persona with no scheduled work and no inbound traffic costs nothing. Closes the polling-loop cost-leak class structurally instead of patching it per release. PR plan: [`0024-pr-plan.md`](docs/rfcs/0024-pr-plan.md). (Slot 0024's original "Episodic Vector Recall" reservation was deferred, gated on [MT-MEMORY-005](docs/manual-tests/MT-MEMORY-005-dementia-test.md) data; the slot was reused for this RFC — see the [RFC 0029 numbering note](docs/rfcs/0029-personal-society-storage-split.md).) | v0.3.3 (Phases 1–4) + v0.4.0 (Phase 5) + v0.5+ (Phase 6) | 🚧 Implementing |
