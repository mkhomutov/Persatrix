...
| **v0.3.9** | Personas quote each other accurately and you can search the verbatim record of what was said, scoped to the channels each was present for (RFC 0036 + RFC 0035) | 🚧 Implementing — Phase 1 complete: RFC 0035 ledger ✅ Implemented (all five PRs) and RFC 0036 recall ✅ Implemented (all six PRs: `messages_fts` index, scoped epoch-filtered `RecallMessages` query, audited `POST …/recall` endpoint, `recall_channel_messages` persona tool + `channels:recall` permission, `?as_participant` conversation-window filter, closeout); release-prep next; [v0.3.9 master plan](docs/v0.3.9-plan.md) (membership-scoped verbatim recall; `epoch`-filtered, session-spanning) |
