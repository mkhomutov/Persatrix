...
> **Last updated**: 2026-05-23 (v0.3.4 plan opening — [`docs/v0.3.4-plan.md`](docs/v0.3.4-plan.md) added. v0.3.4 re-sequenced to RFC 0033 (Provider-Agnostic Model Alias Layer) Phases 1–2 + release of the already-merged offline / Ollama provider modes, ahead of RFC 0031 Phases 2–4 which re-home to v0.3.5; canonical decision in [v0.3.x-sequencing.md §Amendment 2026-05-23](docs/v0.3.x-sequencing.md#amendment-2026-05-23--v034-carries-rfc-0033-ahead-of-rfc-0031-phases-24). Version Map gains v0.3.4 (🚧 Planning) + v0.3.5 (📋 Planned) rows; RFC Master Index 0031 / 0033 target columns pinned.)
> **Current phase**: v0.3.4 (Any Model, Any Provider — RFC 0033 Phases 1–2) — 🚧 Planning
> **Current milestone**: v0.3.4 plan opened. The release decouples agent configs from vendor model IDs: agents reference logical aliases (`quality` / `fast` / `summarizer`) resolved to `{provider, model_id, pricing}`, so a vendor retirement or provider swap is a one-line edit instead of a config sweep. RFC 0033 Phase 1 absorbs the unavoidable Anthropic Sonnet 4 → 4.6 migration (`claude-sonnet-4-20250514` retires 2026-06-15) as its first exercise; the same release ships the already-merged offline (`MockProvider`) and Ollama local-model provider modes. RFC 0031 Phases 2–4 (per-session namespacing, F-3 fix) re-home to v0.3.5 — it carries no deadline and stays mitigated by `make reset`. Next major phase is v0.4.0 (team/hierarchy).
...
| [0046](docs/rfcs/0046-budget-lease-extraction.md) | Budget-Lease Library Extraction (`persatrix-budget`) — first per-extraction RFC under RFC 0045 and the flagship one. Pure seam-cutting that carves the RFC 0023 LLM budget-lease (embeddable cost engine + gRPC wallet service + `wallet.proto` + Python client + ADK/LiteLLM adapters) into the standalone MIT `persatrix-budget` repo. Specifies the `internal/cost` split (separating the embeddable engine from the `context_package_metrics.go → executor/packaging` edge so the engine joins the dependency-direction gate), the two-layer in-process-engine vs gRPC-service packaging, the dependency-direction proof, Option-A mirror-out sync, and resolves the deferred RFC 0024 event-loop placement (kept internal/BUSL). Code moves only after RFC 0045 is Accepted and its CI gate is green. | v0.4.0+ (gated on RFC-0045 acceptance + CI gate) | 📋 Proposed |
| 0047 | Low-Coupling Batch Library Extraction — reserved per RFC 0045 §H for the prompt-safety kit + deterministic mock provider + agent/channel/workflow JSON schemas. Authored after the RFC 0046 flagship pattern is proven. | v0.4.0+ | Reserved (RFC 0045 §H) |
