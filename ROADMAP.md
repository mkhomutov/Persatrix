...
> **Last updated**: 2026-09-24 (the reasoning comments, the channel schema and the channels guide now say a governed channel's `reasoning.mode` defaults to `bid` (RFC 0051 PR 6), where the per-member `threshold` is unused, and [ISSUE-0168](docs/issues/ISSUE-0168-reasoning-rung-ignores-member-changes.md) records that a member change does not re-apply that default — [#990](https://github.com/mkhomutov/Persatrix/pull/990); `reasoning.revise >= 1` runs the reflexion loop under `mode: plan` — [#989](https://github.com/mkhomutov/Persatrix/pull/989). **EXP-001** harness PRs 1, 2, 3a and 3b merged ([harness plan](docs/experiments/EXP-001-harness.md)). **v0.3.16** ✅ [Released](https://github.com/mkhomutov/Persatrix/releases/tag/v0.3.16); no next plan opens before **EXP-001** and the first strategy review.)
> **Current phase**: **v0.4.0** *Agent Organizations* — 📋 **Planned**, gated with no plan open: per the [sequencing Amendment 2026-09-12](docs/v0.3.x-sequencing.md#amendment-2026-09-12--close-v0316-small-then-measure-before-any-train-opens), no v0.3.17 or v0.4.0 plan opens before **EXP-001** ([the pre-registered experiment](docs/project-strategy.md#4-the-decision-framework)) is published and the first [strategy review](docs/project-strategy.md#10-review-cadence-and-log) (2026-11-30 at the latest) logs its result; that review's amendment decides what opens, and whether it is v0.4.0. The work in between is the amendment's own next steps: three mechanism PRs (the External evidence check [#964](https://github.com/mkhomutov/Persatrix/pull/964), the file-size gate change [#967](https://github.com/mkhomutov/Persatrix/pull/967), the one-document patch release [#968](https://github.com/mkhomutov/Persatrix/pull/968)), then EXP-001: its [pre-registration](docs/experiments/EXP-001-preregistration.md) ([#969](https://github.com/mkhomutov/Persatrix/pull/969)), the harness under `evaluators/`, the run and the published result. Prior phase: **v0.3.16** *The persona knows who is listening* — ✅ [Released 2026-09-16](https://github.com/mkhomutov/Persatrix/releases/tag/v0.3.16) ([plan](docs/v0.3.16-plan.md) · [PR plan](docs/v0.3.16-pr-plan.md) · six [scope locks](docs/v0.3.16-scope-locks.md) · [release-prep plan](docs/v0.3.16-release-prep-plan.md) · [baseline](docs/v0.3.16-release-baseline.md) · [checklist](docs/v0.3.16-release-checklist.md) · [execution report](docs/manual-tests/v0.3.16-execution-report.md)); all eleven implementation PRs merged — D1, D2, A1, A2, E1, B1, B2, K1, [C1](https://github.com/mkhomutov/Persatrix/pull/945), [C2](https://github.com/mkhomutov/Persatrix/pull/947) and [A3](https://github.com/mkhomutov/Persatrix/pull/950) (the verdict-gated flip to `live`, merged 2026-09-14) — the live arc ran once, as release-prep [PR 1](https://github.com/mkhomutov/Persatrix/pull/954) (2026-09-15, all five legs, Leg 5 under both auth modes — pass); closed small per the Amendment 2026-09-12 — D3… cut, K1 (`MEMORY_BUDGET_TOKENS` in `optimization.yaml`) rode as a cuttable fold-in; original scope ratified by the [sequencing Amendment 2026-08-19](docs/v0.3.x-sequencing.md#amendment-2026-08-19--v0315--v0316-attribution-and-audience-before-the-v040-train): [ISSUE-0132](docs/issues/ISSUE-0132-memory-egress-gate-blind-to-room-audience.md) audience-scoped egress, the operator-observability pair ([ISSUE-0122](docs/issues/ISSUE-0122-relationship-tier-emits-no-provenance.md) / [ISSUE-0108](docs/issues/ISSUE-0108-reasoning-reason-note-no-operator-egress.md)), and [RFC 0044](docs/rfcs/0044-eval-set-golden-traces.md) Phase 2 (paired with [ISSUE-0142](docs/issues/ISSUE-0142-ci-never-runs-golangci-lint.md)) — preceded by the two path splits of the [ISSUE-0143](docs/issues/ISSUE-0143-debt-sweep-26-files-at-size-cap.md) debt sweep. Prior phase: v0.3.15 *Who said what* — ✅ [Released 2026-09-06](https://github.com/mkhomutov/Persatrix/releases/tag/v0.3.15).
> **Current milestone**: **EXP-001** — 📋 the pre-registered experiment: five arms on one provider and model, its decision rules and [pre-registration](docs/experiments/EXP-001-preregistration.md) merged before the run and the result published with transcripts whichever way it falls ([strategy §4](docs/project-strategy.md#4-the-decision-framework)); it gates the v0.4.0 train. Prior milestone: **v0.3.16** *The persona knows who is listening* — ✅ [Released 2026-09-16](https://github.com/mkhomutov/Persatrix/releases/tag/v0.3.16) ([plan](docs/v0.3.16-plan.md) · [scope locks](docs/v0.3.16-scope-locks.md) · [release-prep plan](docs/v0.3.16-release-prep-plan.md) · [checklist](docs/v0.3.16-release-checklist.md); opened 2026-09-08, all eleven implementation PRs merged as of 2026-09-14, the live arc **passed** 2026-09-15 — [execution report](docs/manual-tests/v0.3.16-execution-report.md)). The story: **the persona weighs who is in the room before it speaks from memory — and an operator can read why it decided what it decided.** Audience landed shadow-first as an AND-condition on the RFC 0037 §D gate and flipped to `live` on a measured verdict; the debt sweep split the two at-cap files this release had to edit; **no store migration**. Prior milestone: v0.3.15 *Who said what* — ✅ [Released 2026-09-06](https://github.com/mkhomutov/Persatrix/releases/tag/v0.3.15).
...
| [0029](docs/rfcs/0029-personal-society-storage-split.md) | Personal/Society Storage Split (SA-1 from [storage-architecture-roadmap.md](docs/storage-architecture-roadmap.md); originally filed as 0025, renumbered to preserve the 0025→0027 supersession edge) | v0.3.2 (Phase 1) + v0.4.0 (Phases 2–6) | ⚠️ Partially Implemented (Phase 1; PRs 1 [#370](https://github.com/mkhomutov/Persatrix/pull/370), 2 [#372](https://github.com/mkhomutov/Persatrix/pull/372), 3 [#373](https://github.com/mkhomutov/Persatrix/pull/373), 4 [#375](https://github.com/mkhomutov/Persatrix/pull/375), 5 [#376](https://github.com/mkhomutov/Persatrix/pull/376) merged — `MemoryStore` facade promotion + `MemoryFacade` alias shim + personal/society boundary lint rule + `persona_runtime`/`sub_agents`/RFC 0026 call-site sweep + recall-latency regression gate. Phases 2–6 deferred to v0.4.0.) |
...
|---------|---------|--------|
| `internal/state/` | Workflow/step state tracking | ✅ Complete (in-memory store, 98.9% coverage). Planned: a SQLite store that survives a restart, checkpoints, and export and import (not scheduled), plus additions from RFC 0007, RFC 0009 Phase 4 and RFC 0014, slotted for v0.4.0 — see the [package comment](internal/state/state.go) |
...
| `internal/security/` | Audit logger, secret redactor, rate limiter and its circuit breaker, REST/gRPC middleware, gRPC panic recovery ([ISSUE-0059](docs/issues/ISSUE-0059-grpc-server-no-panic-recovery-interceptor.md), v0.3.2), input sanitizer | ⚠️ Partially Implemented — RFC 0009 Phases 1–2 shipped in v0.3.0, with gaps that the [package comment](internal/security/security.go) lists. The orchestrator does not call the input sanitizer (the agents run a Python copy) or `VerifyChain`. Phases 3–4, including agent identity tokens, are slotted for v0.4.0. |
...
| `agents/tools/registry.py` | Tool discovery and registration | ✅ Complete (decorator + registry). Not built: the permission, rate-limit and audit steps its tool wrapper marks TODO (built-in tools check their own permissions), and RFC 0009's argument-schema check (Phase 3) and `irreversible` flag (Phase 4), slotted for v0.4.0 |
...
| `agents/tools/sandbox.py` | Filesystem path restriction (PathValidator) | ✅ Complete (100% coverage). Planned, not built: `ResourceLimiter` and `OutputSizeLimiter` (RFC 0009 Phase 3, slotted for v0.4.0) |
...
- Authentication — sessions are local and caller-supplied. Human logins came in v0.3.12 (RFC 0039, off by default); agent identity tokens are RFC 0009 Phase 4, slotted for v0.4.0
...
| [0009](docs/rfcs/0009-security-sandboxing.md) | Security & Sandboxing | Phases 1–2 (audit, rate limiting, sanitization) | ⚠️ Partially Implemented (Phases 1–2 — Phases 3–4 deferred to v0.4.0) |
...
Audit logging and rate limiting are foundational safety infrastructure with no RFC 0011/0020 dependency. They can develop concurrently and are integrated progressively (rate limiting into channel REST endpoints in RFC 0011 Phase 1; input sanitization of incoming channel messages in Phase 3, done by the agents rather than in channel message storage). Phases 3–4 (identity tokens, HITL gates) are prerequisites for sub-agent spawning and are deferred to v0.4.0.
...
4. If an RFC closes part-way — some phases shipped, the rest moved to a later version — change its status to `⚠️ Partially Implemented` here **and** in the RFC file.
5. Move completed components from "TODO stub" / "🔲 pending" → "✅ Complete" in component tables. When an RFC closes part-way, a component still marked "🚧 In progress" becomes "✅ Complete", "⚠️ Partially Implemented", or "🔲 TODO stub" for the part moved to a later version. `make roadmap-status-check` (pre-commit and CI) fails while a **Component Status** row says less than the RFC it names.
6. Update the **RFC Master Index** table status.
