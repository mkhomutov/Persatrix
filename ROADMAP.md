...
> **Last updated**: 2026-06-01 (v0.3.5 **release prep** — Track A/B sequenced in the [release-prep plan](docs/v0.3.5-release-prep-plan.md)). The MT execution report ([#488](https://github.com/mkhomutov/Persatrix/pull/488)) landed: every session + epoch structural-isolation gate green on the RC tip, the `persatrix session …` operator surface verified live, and the F-3 recall-isolation promise confirmed on persisted rows ([report](docs/manual-tests/v0.3.5-execution-report.md)). This release-prep PR (PR 2) refreshes the README + ROADMAP, verifies the [sessions](docs/guides/sessions.md) / [epochs](docs/guides/epochs.md) operator guides + the `channel.dispatch` epoch/session span attributes ([observability.md §10.2](docs/observability.md)), and adds the [v0.3.5 release checklist](docs/v0.3.5-release-checklist.md). Both v0.3.5 workstreams are landed: RFC 0031 Phases 2–4 + the epoch run-isolation axis ([#448](https://github.com/mkhomutov/Persatrix/pull/448)–[#478](https://github.com/mkhomutov/Persatrix/pull/478)) and RFC 0033 Phase 3 ([#481](https://github.com/mkhomutov/Persatrix/pull/481)–[#486](https://github.com/mkhomutov/Persatrix/pull/486)) — both ✅ Implemented. Version Map v0.3.5 → 🚧 Release prep; it flips to ✅ Released at the tag (Phase 6).
> **Current phase**: v0.3.5 (Session-Scoped Memory — RFC 0031 Phases 2–4 + epoch axis) — 🚧 Release prep
> **Current milestone**: v0.3.5 release prep underway. The release makes **persona-memory recall session-scoped** — a run reads only its own session's rows (plus the always-visible `legacy` carve-out), so concurrent conversations don't bleed, closing the **F-3** cross-run *recall* bleed at the root. A session is room continuity, not a clean slate; run/test isolation is the sibling **`epoch`** axis ([ISSUE-0085](docs/issues/ISSUE-0085-epoch-axis-run-isolation.md)) — the **structural** half of the F-3 fix: a fresh `PERSATRIX_EPOCH` / `--epoch`, same room + same `--user`, inherits nothing (strict-equality `epoch_id` across all five tiers), so an operator isolates a rerun by bumping the epoch rather than nuking volumes with `make reset`. RFC 0033 Phase 3 (raw-ID rejection, `schema_version` `"0.3"`) shipped as a conditional co-resident, completing RFC 0033: `models.aliases` is the single source of truth for model identity. Both workstreams are ✅ Implemented; the remaining release-prep work (docs refresh, version bump, final pre-tag verification) is sequenced in the [release-prep plan](docs/v0.3.5-release-prep-plan.md). Next phase after v0.3.5 is v0.4.0 (team/hierarchy).
...
| **v0.3.5** | Persona-memory recall is session-scoped — a run reads only its own session (plus the always-visible `legacy` rows), so concurrent conversations don't bleed and a run is isolated by giving it a fresh session instead of `make reset` (closes the F-3 cross-run *recall* bleed; auto-isolating a same-named rerun is the `epoch` axis, also shipped) | 🚧 Release prep — [v0.3.5 plan](docs/v0.3.5-plan.md) |
