...
> **Last updated**: 2026-06-28 (RFC 0052 *autonomous agent-only channels* filed as the next realism-rung candidate — *Conversations that run themselves*, the human-out-of-the-loop capstone of the realism arc — bundled with RFC 0053 (Gemini + watsonx.ai providers) so the flagship demo is a four-vendor human-free brainstorm; alongside the v0.3.11 *safe remote console* (RFC 0039) candidate; → v0.4.0 *Agent Organizations*. See [v0.3.x-sequencing Amendment 2026-06-28](docs/v0.3.x-sequencing.md#amendment-2026-06-28--add-the-autonomous-agent-only-channel-as-the-v03x-realism-capstone).)
> **Current phase**: v0.3.10 (Conversations worth posting into — RFC 0051 reasoning before posting) — ✅ Released
> **Current milestone**: v0.3.10 *Conversations worth posting into* — ✅ Released. The headline user-facing story: **personas think before they speak** — a persona privately decides whether a turn is worth a post and what it should add, staying silent (with a reason) when it has nothing to contribute and composing under a private plan when it does. RFC 0051 generalizes the shipped RFC 0030 Tier-B salience bid into a structured `{ should_post, plan }` verdict on the same leased `fast`-model seam (idle stays free; the trace is walled and audit-only), tunable per channel via a `reasoning` knob on the RFC 0050 config surface — semantic silence is on by default once a channel is governed (`mode: bid`), the considered plan-threaded compose (`mode: plan`) and the critic→revise reflexion loop (`revise: 1|2`) are one-rung-up per-channel opt-ins, and a one-flip `mode: off` restores the prior RFC 0030 scalar gate. The headline MT (`MT-REASON-001`) passed live on Anthropic — 80% semantic suppress-rate under `bid`, plan-threaded considered posts with the private trace walled out of the transcript and both peers' recall, and the one-flip kill switch ([execution report](docs/manual-tests/v0.3.10-execution-report.md)). The final pre-tag gate sweep re-ran green live on host (Go `./internal/... -race -cover` 19 pkgs no-races incl. the `off→bid` go-live + kill-switch + capability-gate + reconcile-survives-boot + proto-stamp suites; the Python deliberation/plan/reflexion + privacy-wall no-leak integration + the RFC 0034 P3 cache/instrument suites; Rust + Vitest; ruff/mypy/clippy/go-vet; the RFC 0045 `imports-check` + `archpolicy` gate), **no store migrates this release** (channel store stays v10; persona-memory untouched), and the offline Docker smoke confirmed the wiring carries through CLI + web; only the mechanical `v0.3.10` tag + GitHub Release ([Phase 4](docs/v0.3.10-plan.md#phase-4--tag-and-post-release-follow-up)) remain. Scope: RFC 0051 Phases 1–3 + 5 (Phase 4 native extended-thinking deferred); RFC 0039 accounts/auth stays parallel → v0.3.11 candidate. See [v0.3.10-plan.md](docs/v0.3.10-plan.md) and the [v0.3.x-sequencing Amendment 2026-06-23](docs/v0.3.x-sequencing.md#amendment-2026-06-23--add-v0310-reasoning-before-posting-as-the-next-realism-rung). v0.4.0 (Agent Organizations — deliberative reasoning) follows.
...
| [0052](docs/rfcs/0052-autonomous-agent-channels.md) | Autonomous Agent-Only Channels — a channel that convenes a roster of personas on a topic and runs a productive, **human-free** discussion that converges, terminates, and yields a readable synthesis: the human-out-of-the-loop capstone of the v0.3.7→v0.3.10 realism arc and the project's best pre-v0.4.0 adoption demo. Reuses the shipped channel/governance/scheduling/reasoning seams (RFC 0011/0030/0024/0050/0051/0020) — **assembly plus three new pieces**: *self-convening* (open without a human turn), *anti-collapse cadence* (counter-pressure to the bias-to-silence defaults, scoped to autonomous channels so human-channel defaults are untouched), and a *mandatory cost cap* (no human circuit-breaker on an unattended channel). Scoped to the **brainstorm** rung; the reason-toward-a-decision version rides v0.4.x RFC 0028. Sequenced by [v0.3.x-sequencing Amendment 2026-06-28](docs/v0.3.x-sequencing.md#amendment-2026-06-28--add-the-autonomous-agent-only-channel-as-the-v03x-realism-capstone). | v0.3.x (next realism rung — candidate) | 📋 Proposed |
| [0053](docs/rfcs/0053-gemini-watsonx-providers.md) | Gemini and watsonx.ai LLM Providers — add Google Gemini and IBM watsonx.ai as first-class providers, the second concrete dogfood of the [RFC 0033 §H](docs/rfcs/0033-model-alias-layer.md#h-multi-provider-extensibility) multi-provider extensibility seam (one provider class + one `create_provider` branch + priced alias entries each — the same "Any Model, Any Provider" axis as v0.3.4). Brings the configurable cloud roster to four vendors (Anthropic / OpenAI / Gemini / watsonx.ai) plus local Ollama + offline mock. Non-local → priced (the missing-price guard keeps the RFC 0023 budget gate live); `make demo-gemini` / `demo-watsonx` + compose parity. **Bundled with RFC 0052** because its motivating consumer is that rung's flagship demo — a four-vendor human-free brainstorm. Independently shippable (RFC 0052 runs on any single provider). Sequenced by [v0.3.x-sequencing Amendment 2026-06-28](docs/v0.3.x-sequencing.md#amendment-2026-06-28--add-the-autonomous-agent-only-channel-as-the-v03x-realism-capstone). | v0.3.x (bundled with RFC 0052 — candidate) | 📋 Proposed |
