...
> **Last updated**: 2026-06-21 (RFC 0035 membership-interval ledger closeout → ✅ Implemented; all five PRs merged. RFC 0036 recall implementation complete, PRs 1–5; closeout pending.)
> **Current phase**: v0.3.9 (Conversations you can mine — RFC 0035 membership-interval ledger + RFC 0036 verbatim message recall) — 🚧 Implementing
> **Current milestone**: v0.3.9 *Conversations you can mine* — 🚧 Implementing (RFC 0035 membership-interval ledger ✅ Implemented — all five PRs merged: migration v9, read surface, transactional write hooks, operator inspection endpoint, closeout. RFC 0036 recall implementation complete, PRs 1–5 merged — dormant channel-store migration v10 `messages_fts` FTS index, the membership-scoped, epoch-filtered `RecallMessages` search query, the audited `POST …/recall` endpoint, the `recall_channel_messages` persona tool + the `channels:recall` permission, and the `?as_participant` conversation-window membership filter; PR 6 closeout pending). The headline user-facing story: **personas quote each other accurately, and a human can search the verbatim record of what was said — scoped to the channels each persona was present for**. A new `recall_channel_messages` persona tool searches the verbatim text of past conversations server-side ([RFC 0036](docs/rfcs/0036-persona-message-recall.md) FTS5 over `messages`), joined against an append-only membership-interval ledger ([RFC 0035](docs/rfcs/0035-channel-membership-interval-ledger.md)) so a persona added → removed → re-added recalls *both* stints and neither the pre-join period nor the removal gap. Recall is `epoch_id`-hard-filtered (run isolation) and spans sessions within the epoch; it is audited, permission-gated (`channels:recall`), and delimiter-escaped against prompt injection. The same membership filter is retrofitted onto the live RFC 0034 conversation window. This closes the v0.3.x conversation-realism-and-usefulness arc; the next phase is **v0.4.0** (Agent Organizations — deliberative reasoning).
...
| **v0.3.9** | Personas quote each other accurately and you can search the verbatim record of what was said, scoped to the channels each was present for (RFC 0036 + RFC 0035) | 🚧 Implementing — RFC 0035 ledger ✅ Implemented (all five PRs); RFC 0036 recall implementation merged (PRs 1–5: `messages_fts` index, scoped epoch-filtered `RecallMessages` query, audited `POST …/recall` endpoint, `recall_channel_messages` persona tool + `channels:recall` permission, `?as_participant` conversation-window filter), PR 6 closeout pending; [v0.3.9 master plan](docs/v0.3.9-plan.md) (membership-scoped verbatim recall; `epoch`-filtered, session-spanning) |
