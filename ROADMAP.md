...
> **Last updated**: 2026-05-21 (v0.3.3 master plan opened — `docs/v0.3.3-plan.md` added; RFC Master Index row 0024 corrected from the stale Episodic-Vector-Recall reservation to Event-Driven Agent Scheduling at `v0.3.3 | 📋 Proposed` per [v0.3.x-sequencing.md §ROADMAP hygiene](docs/v0.3.x-sequencing.md#roadmap-hygiene) — the slot-title flip the sequencing-amendment PR [#311](https://github.com/mkhomutov/Persatrix/pull/311) ordered did not land at the time and was missed by later status sweeps. Reserved-scope breadcrumb preserved as a parenthetical, matching the RFC 0023 row shape.)
> **Current phase**: v0.3.3 (Idle Truly Idle — RFC 0024 Phases 1–4) — 🚧 Planning
> **Current milestone**: v0.3.3 master plan opened ([docs/v0.3.3-plan.md](docs/v0.3.3-plan.md), this PR — Phase 0). Goal: invert the persona autonomy loop from polling to event-driven so a persona with no scheduled work and no inbound traffic costs nothing (no SQLite recall query, no `_inject_memory_context`, no provider activity, no wallet lease requested). Cost-regression CI gate from [RFC 0024 §Test Strategy](docs/rfcs/0024-event-driven-scheduling.md#test-strategy) is the release-blocker acceptance. Previous phase: v0.3.2 (Cost Gate + Facade Freeze — RFC 0023 full + RFC 0029 Phase 1) — ✅ released 2026-05-20.
...
| [0024](docs/rfcs/0024-event-driven-scheduling.md) | Event-Driven Agent Scheduling — wake the persona on inbound RPC, salience-triggered memory write, or scheduled timer instead of a fixed-interval poll; a persona with no scheduled work and no inbound traffic costs nothing. Closes the polling-loop cost-leak class structurally instead of patching it per release. (Slot 0024's original "Episodic Vector Recall" reservation was deferred, gated on [MT-MEMORY-005](docs/manual-tests/MT-MEMORY-005-dementia-test.md) data; the slot was reused for this RFC — see the [RFC 0029 numbering note](docs/rfcs/0029-personal-society-storage-split.md).) | v0.3.3 | 📋 Proposed |
