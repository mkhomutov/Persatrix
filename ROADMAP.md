...
> **Last updated**: 2026-08-01 (v0.3.12 — 🚧 **Release prep**: implementation complete; the live release-gating MTs executed ([#796](https://github.com/mkhomutov/Persatrix/pull/796)) — eval-replay 5/5, `MT-AUTH-001` all six legs, `MT-PERSONA-CONFIDENTIALITY-001` pass, `MT-MEMORY-CROSSROOM-001` headline pass with the fresh-epoch leg Accepted-with-known-gap ([ISSUE-0118](docs/issues/ISSUE-0118-tool-recall-bypasses-epoch-session-scopes.md), doesn't gate the tag) — two live-found fixes landed. [ISSUE-0114](docs/issues/ISSUE-0114-per-channel-cascade-depth-override.md) cut to v0.3.13 and the cross-room MT's new person-identity legs deferred to the next memory-touching release ([ISSUE-0121](docs/issues/ISSUE-0121-crossroom-person-identity-legs-never-run-live.md)) — both maintainer calls. Remaining: version bump + final pre-tag sweep + tag. See the [release-prep plan](docs/v0.3.12-release-prep-plan.md).)
> **Current phase**: v0.3.12 🚧 Release prep (RFCs 0037 + 0049 P0–1 + 0039 P1–2 ✅ all implemented; live MTs executed); v0.4.0 (Agent Organizations) follows
> **Current milestone**: v0.3.12 *Memory that travels* — 🚧 Release prep ([plan](docs/v0.3.12-plan.md), opened 2026-07-25; [release-prep plan](docs/v0.3.12-release-prep-plan.md)). The headline user-facing story: **tell a persona something in one room and it knows it in every room it belongs to — without leaking what it learned in a confidential room.** Scope: **RFC 0037** (classification lattice + protection levels + the deterministic §D egress gate + §F recall filter — the keystone; Phases 2–3 projections/tripwire cuttable) + **RFC 0049 Phases 0–1** (the L1/fact-scope/topic-predicate widening amendments, shadow-first behind an RFC 0044 golden-trace measurement gate) **+ RFC 0039 Phases 1–2** (accounts/auth — `auth.mode` opt-in, console safe beyond localhost; bundled second workstream, independently shippable/cuttable). Fold-ins: ISSUE-0106 direction (b) ✅ + ISSUE-0114 (**cut to v0.3.13** — maintainer call at release-prep PR 2). Hard rule: **no widening merges before the 0037 gate is on `main`**. Prior milestone: v0.3.11 *Conversations that run themselves* — ✅ [Released 2026-07-22](https://github.com/mkhomutov/Persatrix/releases/tag/v0.3.11) ([v0.3.11-plan.md](docs/v0.3.11-plan.md)). v0.4.0 (Agent Organizations — deliberative reasoning) follows.
...
| **v0.3.12 — Memory that travels** | Tell a persona something in one room and it knows it in every other room it belongs to — a project fact taught in a DM is known in the standup, raw experience from other rooms is available (room-first-ranked) — **without leaking what it learned in a confidential room** (RFC 0037 classification + protection levels + deterministic egress gate; RFC 0049 Phases 0–1 widenings, shadow-gated). Plus verified accounts: with `auth.mode: enabled` the web console/REST surface is safe beyond localhost (RFC 0039 Phases 1–2, bundled/cuttable). | 🚧 Release prep — [release-prep plan](docs/v0.3.12-release-prep-plan.md) |
