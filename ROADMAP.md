...
> **Last updated**: 2026-06-20 (RFC 0036 recall PR 4: the `recall_channel_messages` persona tool + the distinct `channels:recall` permission + per-row §F delimiter-escape. Server-side recall PRs 1–3 merged; RFC 0035 ledger merged, PRs 1–4.)
...
| **v0.3.9** | Personas quote each other accurately and you can search the verbatim record of what was said, scoped to the channels each was present for (RFC 0036 + RFC 0035) | 🚧 Implementing — RFC 0035 ledger merged (PRs 1–4); RFC 0036 recall Phase 1 server-side merged (PRs 1–3), PR 4 (`recall_channel_messages` persona tool + `channels:recall` permission) open; [v0.3.9 master plan](docs/v0.3.9-plan.md) (membership-scoped verbatim recall; `epoch`-filtered, session-spanning) |
