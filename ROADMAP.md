...
> **Last updated**: 2026-09-18 (code comments no longer say the persona prompt path never reads across sessions with `sessions="*"`: since RFC 0049 PR 4 the live facts recall does, behind the §D gate, and the shadow passes run only in `shadow` mode — [#971](https://github.com/mkhomutov/Persatrix/pull/971). **EXP-001** is pre-registered ([#969](https://github.com/mkhomutov/Persatrix/pull/969)); its harness under `evaluators/` comes next. **v0.3.16** ✅ [Released](https://github.com/mkhomutov/Persatrix/releases/tag/v0.3.16); no next plan opens before **EXP-001** and the first strategy review.)
