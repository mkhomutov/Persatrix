...
> **Last updated**: 2026-09-23 (the sessions guide and two manual tests now keep a conversation in one session by sending that session on every CLI call; starting the persona under `PERSATRIX_SESSION_ID` scopes no channel turn — [#981](https://github.com/mkhomutov/Persatrix/pull/981). **EXP-001** is pre-registered ([#969](https://github.com/mkhomutov/Persatrix/pull/969)); its harness under `evaluators/` comes next. **v0.3.16** ✅ [Released](https://github.com/mkhomutov/Persatrix/releases/tag/v0.3.16); no next plan opens before **EXP-001** and the first strategy review.)
