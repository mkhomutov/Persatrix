...
| [0045](docs/rfcs/0045-open-core-extraction-policy.md) | Open-Core Library Extraction Policy — foundational policy for extracting selected leaf subsystems into standalone MIT repositories (budget lease, prompt-safety kit, mock provider, schemas) while the integrated product and the memory moat stay BUSL-1.1. Fixes the license boundary, the MIT-must-not-import-BUSL dependency-direction invariant + its CI gate, the monorepo-canonical/mirror-out sync default, DCO governance, and the core-plus-adapters repo structure that every per-extraction RFC inherits. Moves no code itself. | v0.3.x (policy + CI gate) + v0.4.0+ (per-extraction RFCs) | 🔨 Draft |
