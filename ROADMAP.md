...
> **Last updated**: 2026-05-25 (RFC 0033 PR 2 — factory integration. `create_provider` now returns `(provider, physical_model)` and consumes the alias resolver: a `models.aliases` entry is authoritative for the provider, a raw vendor ID still passes through (§E) but fires a one-shot deprecation warning and the per-agent `persatrix.llm.alias.raw_id_usage` counter (the Phase 3 gate signal). Configs still carry raw IDs (PR 3 migrates them), so observable routing is unchanged. RFC 0033 RFC Master Index row → 🚧 Implementing (first PR wiring the resolver into the runtime); v0.3.4 Version Map row stays 🚧 Planning. Prior: 2026-05-23 — v0.3.4 plan opening; v0.3.4 re-sequenced to RFC 0033 Phases 1–2 + the already-merged offline / Ollama provider modes, ahead of RFC 0031 Phases 2–4 which re-home to v0.3.5 ([v0.3.x-sequencing.md §Amendment 2026-05-23](docs/v0.3.x-sequencing.md#amendment-2026-05-23--v034-carries-rfc-0033-ahead-of-rfc-0031-phases-24)).)
