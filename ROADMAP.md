...
> **Last updated**: 2026-05-22 (v0.3.3 release-prep PR 2 — docs refresh open: README roadmap row + Cost Warning flipped to released wording, persona-agents `autonomy.timers` / event-driven-loop section, and the new `docs/v0.3.3-release-checklist.md` land ahead of the version bump. Version Map v0.3.3 row flips `🚧 Planning` → `🚧 Release prep`. RFC 0024 stays `⚠️ Partially Implemented (Phases 1–4)` — release-prep does not re-touch it.)
> **Current phase**: v0.3.3 (Idle Truly Idle — RFC 0024 Phases 1–4) — 🚧 Release prep
> **Current milestone**: v0.3.3 release-prep PR 2 (docs refresh + release checklist) open per [release-prep plan §Track A](docs/v0.3.3-release-prep-plan.md#pr-2--readme--roadmap--persona-agents-guide-refresh--release-checklist); release-prep PR 0 (plan [#415](https://github.com/mkhomutov/Persatrix/pull/415)) and PR 1 (MT execution report [#416](https://github.com/mkhomutov/Persatrix/pull/416)) merged. Next: version bump + curated `[0.3.3]` changelog (release-prep PR 3). Previous milestone: RFC 0024 Phases-1–4 closeout merged ([#414](https://github.com/mkhomutov/Persatrix/pull/414)).
...
| **v0.3.3** | A persona with no scheduled work and no inbound traffic costs nothing — no SQLite recall query, no `_inject_memory_context`, no provider activity, no wallet lease requested. The persona autonomy loop is structurally event-driven (inbound RPC / salience-triggered memory write / scheduled timer), not polled. | 🚧 Release prep |
