...
| **v0.3.12** | Tell a persona something in one room and it **knows it in every other room it belongs to** — a project fact taught in a DM is known in the standup, raw experience from other rooms is available (room-first-ranked) — **without leaking what it learned in a confidential room**: rooms carry a classification, every memory carries a protection level, and a deterministic egress gate filters what memory may inform each reply (RFC 0037 + RFC 0049 Phases 0–1, live). Plus **verified accounts**: opt-in `auth.mode: enabled` puts the web console and REST surface behind password login with revocable sessions, operator/user roles, and a verified chat identity — safe beyond localhost over HTTPS (RFC 0039 Phases 1–2) | 🚧 Release prep |
