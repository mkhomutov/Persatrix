...
| **v0.3.15** | **Memory a persona derives from a shared room names the person it came from** — and keeps that name across the orchestrator hop. A group room's close-time summary and facts land in the speaking person's tenant rather than whoever closed it, an agent cascade keeps the originating tenant, a restart replay re-derives under the tenant that spoke, and derived rows carry the **speaker**, so hearsay is distinguishable from testimony ([ISSUE-0082](docs/issues/ISSUE-0082-orchestrator-per-request-session-principal-emission.md) R-1/R-2, [ISSUE-0130](docs/issues/ISSUE-0130-catchup-replay-rederives-memory-under-default-principal.md), [ISSUE-0131](docs/issues/ISSUE-0131-derived-memory-has-no-speaker-attribution.md)) | ✅ [Released](https://github.com/mkhomutov/Persatrix/releases/tag/v0.3.15) |
| **v0.3.16** | **The persona weighs who is in the room before it speaks from memory.** A fact taught in a DM is withheld from any room that adds someone who was not there — audience is an AND-condition on the RFC 0037 §D egress gate, on by default (`memory.egress.audience: live`; `shadow` is the rollback lever), and the persona's channel-recall tool obeys the same rule ([ISSUE-0132](docs/issues/ISSUE-0132-memory-egress-gate-blind-to-room-audience.md), [ISSUE-0158](docs/issues/ISSUE-0158-recall-filter-audience-blind.md)). An operator can read why it decided what it decided: the identity tier's admission line under the provenance switch ([ISSUE-0122](docs/issues/ISSUE-0122-relationship-tier-emits-no-provenance.md)) and the verbatim silence `reason_note` at `DEBUG` ([ISSUE-0108](docs/issues/ISSUE-0108-reasoning-reason-note-no-operator-egress.md)). Go lint and the golden-trace evals are merge gates ([RFC 0044](docs/rfcs/0044-eval-set-golden-traces.md) Phase 2). No store migration — drop-in both ways | ✅ [Released](https://github.com/mkhomutov/Persatrix/releases/tag/v0.3.16) |
...
current release's
[§ Known Gaps](docs/v0.3.16-release-checklist.md#6-known-gaps-to-document-in-release-notes)
