...
| **v0.3.10** | Watch personas **think before they speak** — on an open-floor turn a persona privately decides whether the turn is worth a post, staying silent (with a reason) when it would only agree, restate, or pile on (semantic silence, on by default once a channel is governed) and — one opt-in rung up, per channel — composing considered, plan-threaded posts instead of reflexive ones, with an optional critic→revise reflexion loop; the deliberation runs on the same leased `fast`-model seam (idle stays free) and its private trace is walled — never a channel message, never persisted, never visible to a peer (RFC 0051 reasoning before posting, Phases 1–3 + 5) | 🚧 Release prep |
